...
bool dtype_is_bf16(const std::string& s) { return s == "bf16" || s == "bfloat16"; }
bool dtype_is_fp32(const std::string& s) { return s == "fp32" || s == "float32" || s == "float"; }
...
int64_t derive_E(int64_t l1_bytes, int sizeof_elt, double target_frac) {
  ...
  int64_t e_max =
      static_cast<int64_t>(target_frac * static_cast<double>(l1_bytes) / 16.0 / static_cast<double>(sizeof_elt));
...
int64_t derive_Sk(int64_t l1_bytes, int sizeof_elt, double target_frac) {
  ...
  int64_t sk_max = static_cast<int64_t>(target_frac * static_cast<double>(l1_bytes) / static_cast<double>(per_sk));
...
void format_bytes(int64_t n, char* buf, size_t buflen) {
  if (n >= 1024 * 1024) {
    std::snprintf(buf, buflen, "%.2f MiB", static_cast<double>(n) / (1024.0 * 1024.0));
  } else if (n >= 1024) {
    std::snprintf(buf, buflen, "%.2f KiB", static_cast<double>(n) / 1024.0);
  } else {
...
template <class MK>
std::map<std::string, double> dispatch_one(const std::string& dtype, int64_t E, int64_t Sk, int64_t iters,
                                           int64_t warmup) {
...
std::map<std::string, double> dispatch(const std::string& impl, const std::string& dtype, int64_t E, int64_t Sk,
                                       int64_t iters, int64_t warmup) {
#if FUSED_CPP_MK_ENABLE_BASELINE
  if (impl == "baseline") {
    return dispatch_one<::fused_cpp::sdpa_microkernels::MK_Baseline>(dtype, E, Sk, iters, warmup);
  }
#endif
#if FUSED_CPP_MK_ENABLE_SCALAR
  if (impl == "scalar") {
    return dispatch_one<::fused_cpp::sdpa_microkernels::MK_Scalar>(dtype, E, Sk, iters, warmup);
  }
#endif
...
void print_table(const std::string& dtype, const std::string& impl, int64_t E, int64_t Sk, int sizeof_elt,
                 int64_t l1_bytes, const std::map<std::string, double>& r) {
  ...
  std::printf(
      ...
      dtype.c_str(), impl.c_str(), static_cast<long long>(E), static_cast<long long>(Sk), buf_q,
      100.0 * static_cast<double>(ws_qkt) / static_cast<double>(l1_bytes), buf_p,
      100.0 * static_cast<double>(ws_pv) / static_cast<double>(l1_bytes));
  ...
  std::printf("  %-10s%4s%4s%8s%14s%12s\n", "op", "M", "N", "K", "us/iter", "GFLOPS");
  for (const auto& row : rows) {
    std::printf("  %-10s%4d%4d%8lld%14.4f%12.2f\n", row.name, row.M, row.N, static_cast<long long>(row.K), row.us,
                row.gflops);
  }
  std::printf("\n");
}
...
int main(int argc, char** argv) {
  ...
  const int64_t l1_bytes = opts.l1_bytes_override > 0 ? opts.l1_bytes_override : cache_bytes[0];
  const int sizeof_elt = dtype_is_bf16(opts.dtype) ? 2 : (dtype_is_fp32(opts.dtype) ? 4 : 0);
  if (sizeof_elt == 0) {
    std::fprintf(stderr, "unknown dtype '%s'; expected bf16 or fp32\n", opts.dtype.c_str());
    return 2;
  }

  const int64_t E = opts.E > 0 ? opts.E : derive_E(l1_bytes, sizeof_elt, opts.target_frac);
  const int64_t Sk = opts.Sk > 0 ? opts.Sk : derive_Sk(l1_bytes, sizeof_elt, opts.target_frac);
  ...
  std::printf(
      ...
      static_cast<long long>(l1_bytes), l1_str, opts.target_frac, static_cast<long long>(opts.iters),
