# Agent Performance References

Read this file only when comparing benchmark results with peak/sanity reference
data or investigating hardware-specific throughput on the configured remote
machines.

## Arm-codex-internal / Arm-codex Peak FLOPs Reference

The following numbers are `Arm-codex-internal` / `Arm-codex` single-core peak
measurements unless noted otherwise. Use them as sanity checks for SDPA and
microkernel benchmark results on that machine.
...
instruction-level performance tests are known to be inaccurate or unstable, so
treat this section as reference data only. Use it for rough sanity checks and
relative comparisons, not as authoritative peak throughput.
