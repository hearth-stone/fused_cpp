...
def prepare(
    ...
    *,
    ...
    if weight_int8.dim() != 2:
        raise RuntimeError(
            f"i8gemm.prepare: weight_int8 must be 2D [N, K], got {weight_int8.dim()}D"
        )
    if weight_int8.dtype != torch.int8:
        raise RuntimeError(
            f"i8gemm.prepare: weight_int8 dtype must be torch.int8, got {weight_int8.dtype}"
        )
    if weight_scale.numel() not in (1, int(weight_int8.shape[0])):
        raise RuntimeError(
            "i8gemm.prepare: weight_scale must be scalar or shape [N]"
        )
...
def dynamic_scaled_mm(
    ...
    *,
    ...
    if x.shape[-1] != packed.k:
        raise RuntimeError(
            f"i8gemm.dynamic_scaled_mm: x last dim must be K={packed.k}, got {x.shape[-1]}"
        )
    if x.dtype not in (torch.float32, torch.bfloat16):
        raise RuntimeError(
            f"i8gemm.dynamic_scaled_mm: x dtype must be float32/bfloat16, got {x.dtype}"
        )
    if out_dtype not in (torch.float32, torch.bfloat16):
        raise RuntimeError(
            f"i8gemm.dynamic_scaled_mm: out_dtype must be float32/bfloat16, got {out_dtype}"
        )
    if bias is not None:
        if bias.dim() != 1 or int(bias.shape[0]) != packed.n:
            raise RuntimeError(
                f"i8gemm.dynamic_scaled_mm: bias must be shape [{packed.n}]"
            )
...
def mm(
    ...
    *,
    ...
    return dynamic_scaled_mm(
        x, packed, bias=bias, out_dtype=out_dtype, nthreads=nthreads
    )
