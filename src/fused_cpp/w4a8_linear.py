...
_INT_MM = getattr(torch, "_int_mm", None)
_INT_MM_AVAILABLE: bool | None = None

# 环境变量开关：当被设置为真值（1/true/yes/on，任意大小写）时，``w4a8_linear``
# 跳过实际的反量化与 GEMM 计算，直接返回一个形状正确的全零 bfloat16 张量。
# 用途：在 profile / 形状采集等场景下避免昂贵的真实计算。
_SKIP_COMPUTE_ENV: str = "FUSED_CPP_W4A8_SKIP"
...
def _supports_int_mm() -> bool:
    """当前 PyTorch 环境是否提供可调用的 ``torch._int_mm``。"""
    global _INT_MM_AVAILABLE  # pylint: disable=global-statement

    if _INT_MM_AVAILABLE is not None:
        return bool(_INT_MM_AVAILABLE)

    int_mm = _INT_MM
    if int_mm is None:
        _INT_MM_AVAILABLE = False
        return False

    a = torch.zeros((1, 16), dtype=torch.int8)
    b = torch.zeros((16, 1), dtype=torch.int8)
    try:
        out = int_mm(a, b)
    except RuntimeError:
        _INT_MM_AVAILABLE = False
        return False
    _INT_MM_AVAILABLE = out.dtype == torch.int32 and out.shape == (1, 1)
    return bool(_INT_MM_AVAILABLE)
...
def _int8_gemm(
    ...
    *,
    ...
    """执行 ``int8 x int8 -> int32`` GEMM，优先使用 ``torch._int_mm``。

    :param x_q: ``[M, K] int8`` 激活矩阵。
    :param w_q: ``[K, N] int8`` 权重矩阵。
    :param use_int_mm: 为 ``True`` 且环境支持时，优先使用 ``torch._int_mm``。
    :returns: ``[M, N] int32`` 累加结果。
    """
    x_contig = x_q.contiguous()
    w_contig = w_q.contiguous()
    int_mm = _INT_MM
    if use_int_mm and int_mm is not None and _supports_int_mm():
        try:
            return int_mm(x_contig, w_contig)
        except RuntimeError:
            pass
    return torch.matmul(x_contig.to(torch.int32), w_contig.to(torch.int32))
...
def w4a8_linear(
    ...
    :param scales:  float16 或 bfloat16 张量，形状 ``[K // group_size, N]``。
    :param bias:    可选 bias，形状 ``[N]``，输出 dtype 为 bfloat16。
    :returns:       bfloat16 张量，形状 ``[M*, N]``。
    """
