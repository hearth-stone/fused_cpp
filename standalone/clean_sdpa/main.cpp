...
int main(int argc, char** argv) {
  ...
  clean_sdpa::AlignedVector<at::BFloat16> q(static_cast<size_t>(q_size));
  clean_sdpa::AlignedVector<at::BFloat16> k(static_cast<size_t>(k_size));
  clean_sdpa::AlignedVector<at::BFloat16> v(static_cast<size_t>(v_size));
  clean_sdpa::AlignedVector<float> out(static_cast<size_t>(o_size), 0.0f);

  std::mt19937 rng(1234);
  std::normal_distribution<float> dist(0.0f, 1.0f);
  for (auto& x : q) x = at::BFloat16(dist(rng));
  for (auto& x : k) x = at::BFloat16(dist(rng));
  for (auto& x : v) x = at::BFloat16(dist(rng));
