...
inline double elapsed_ms(TimePoint start) { return std::chrono::duration<double, std::milli>(now() - start).count(); }

inline void add_elapsed(double* field, TimePoint start) { *field += elapsed_ms(start); }
...
#define FUSED_CPP_PROFILE_START(name) ::fused_cpp::profile::TimePoint name = ::fused_cpp::profile::now()
