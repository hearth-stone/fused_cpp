...
static std::vector<int32_t> integer_allocation(const std::vector<double>& weights, long total) {
  ...
  std::sort(order.begin(), order.end(), [&](int a, int b) { return frac[a] > frac[b]; });
  for (long i = 0; i < leftover; ++i) out[order[i % n]]++;
  return out;
}

static std::vector<int32_t> gen_routes(const std::string& dist, int E, long total, int active_k, int hot_k,
                                       double hot_frac) {
  if (dist == "uniform") {
...
static std::string check_plan(const PlanResult& r, const Workload& w, const CostModel& cm) {
  std::unordered_map<int32_t, int32_t> routes_of;
  for (int a = 0; a < w.num_active; ++a) routes_of[w.expert_ids[a]] = w.routes[a];

  if (r.num_teams() != w.num_active) return "team count != active count";

  std::unordered_map<int32_t, int> seen;
  for (int j = 0; j < r.num_teams(); ++j) seen[r.team_expert_ids[j]]++;
  if (static_cast<int>(seen.size()) != w.num_active) return "expert appears zero or duplicated";
  for (auto& kv : seen)
  ...
  if (execute != r.estimated_execute_cost_ns) return "recomputed execute != reported";
  return "";
}
...
int main(int argc, char** argv) {
  ...
  for (const auto& c : cases) {
    ...
    std::vector<int32_t> routes = gen_routes(c.dist, E, total_routes, c.active_k, c.hot_k, c.hot_frac);
    for (int C : core_list) {
      if (only_cores && only_cores != C) continue;
      Workload w0 = prepare_workload(routes.data(), E, C, cm);
      Timing tp = bench(
          [&]() {
            volatile auto w = prepare_workload(routes.data(), E, C, cm);
            (void)w;
          },
          200, 2000);

      if (!csv)
        printf("\n[dist=%s cores=%d active=%d]  prepare=%s\n", c.dist, C, w0.num_active, fmt_ns(tp.median).c_str());
      for (PlanKind k : kinds) {
        ...
        int iters = (k == PlanKind::GREEDY_MARGINAL_GAIN || k == PlanKind::ENUMERATE_CORE_GROUPS) ? 2000 : 20000;
        Timing t = bench(
            [&]() {
              volatile auto rr = run_planner(k, w0);
              (void)rr;
            },
            std::max(50, iters / 10), iters);
        double total = tp.median + t.median;
        if (csv) {
          printf("%s,%d,%d,%s,%.1f,%.1f,%.1f,%d,%lld\n", c.dist, C, w0.num_active, plan_kind_name(k), tp.median,
                 t.median, total, r.num_waves(), (long long)r.estimated_execute_cost_ns);
        } else {
          printf("  %-26s plan=%s  +prep=%s  waves=%-3d exec=%s\n", plan_kind_name(k), fmt_ns(t.median).c_str(),
                 fmt_ns(total).c_str(), r.num_waves(), fmt_ns((double)r.estimated_execute_cost_ns).c_str());
        }
      }
    }
  }
