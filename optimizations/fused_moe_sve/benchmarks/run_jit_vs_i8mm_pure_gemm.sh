...
BUILD_DIR="${BUILD_DIR:-/tmp/fused_cpp_jit_pure_gemm}"
CXX="${CXX:-g++}"
SVE_BITS="${FUSED_CPP_MOE_SVE_VECTOR_BITS:-128}"
TARGET="${BUILD_DIR}/bench_jit_vs_i8mm_pure_gemm_sve${SVE_BITS}"

mkdir -p "${BUILD_DIR}"

SOURCES=(
  "${SCRIPT_DIR}/bench_jit_vs_i8mm_pure_gemm.cpp"
  ...
  "${ROOT}/refs/i8gemm/lib/bf16gemm_sve.S"
)
DEPENDENCIES=(
  "${SOURCES[@]}"
