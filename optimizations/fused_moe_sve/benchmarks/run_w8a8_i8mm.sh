...
SOURCE="${ROOT_DIR}/optimizations/fused_moe_sve/benchmarks/bench_w8a8_i8mm.cpp"
TARGET="${BUILD_DIR}/bench_w8a8_i8mm_sve${SVE_BITS}"
...
for source_name in "${I8_SOURCES[@]}"; do
  ...
  if [[ ! -f "${object_path}" || "${source_path}" -nt "${object_path}" ]]; then
    cc -O2 -fPIC -march=armv8.6-a+sve+i8mm -msve-vector-bits="${SVE_BITS}" \
