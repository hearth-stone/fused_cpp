        ...
        if (arg == "--stage") {
            ...
            if (options.stage_name == "w13") {
                ...
                throw std::invalid_argument(
                    "--stage must be w13, w2, or w2_bf16");
            }
        } else if (arg == "--m") {
            ...
            options.low_threads =
                parse_int(value("--low-threads"), "low threads");
        } else if (arg == "--epoch-rows") {
            options.epoch_rows =
                parse_int(value("--epoch-rows"), "epoch rows");
        } else if (arg == "--warmup") {
            options.warmup = parse_int(value("--warmup"), "warmup");
        } else if (arg == "--iters") {
            options.iterations =
                parse_int(value("--iters"), "iterations");
        } else if (arg == "--copies") {
            options.copies = parse_int(value("--copies"), "copies");
        } else if (arg == "--cpu-start") {
            options.cpu_start =
                parse_nonnegative_int(value("--cpu-start"), "cpu start");
        } else if (arg == "--check") {
            options.check_only = true;
        } else if (arg == "--help" || arg == "-h") {
            std::cout
                << "Usage: bench_elastic_nsplit [options]\n"
...
struct Buffers {
    ...
    void* output(elastic::Stage stage) {
        return stage == elastic::Stage::kW2F32
                   ? static_cast<void*>(output32.data())
                   : static_cast<void*>(output16.data());
    }

    const void* output(elastic::Stage stage) const {
        return stage == elastic::Stage::kW2F32
                   ? static_cast<const void*>(output32.data())
...
size_t output_bytes(const Options& options) {
    const size_t element_bytes =
        options.stage == elastic::Stage::kW2F32 ? sizeof(float)
                                                 : sizeof(uint16_t);
...
elastic::Problem make_problem(const Options& options, Buffers& buffers,
                              int n_tile) {
    ...
    problem.k = options.k;
    problem.n = options.n;
    problem.ldc = options.stage == elastic::Stage::kW13 ? options.n / 2
                                                         : options.n;
    problem.n_tile = n_tile;
    return problem;
}

void assert_same(const std::vector<std::byte>& reference, const Buffers& buffers,
                 const Options& options, const char* variant) {
    const auto* actual =
        static_cast<const std::byte*>(buffers.output(options.stage));
    ...
    throw std::runtime_error(std::string("correctness mismatch in ") + variant +
                             " at output byte " + std::to_string(first));
}

void run_correctness(const Options& options, elastic::Executor& executor,
                     Buffers& buffers, int n_tile,
    ...
    std::memcpy(reference.data(), buffers.output(options.stage),
                reference.size());
    ...
    assert_same(reference, buffers, options,
                "elastic_strict_epoch_low_to_high");
}
...
struct Variant {
    ...
    Variant(std::string variant_name,
            std::function<elastic::RunResult(const elastic::Problem&)>
                variant_run)
        : name(std::move(variant_name)), run(std::move(variant_run)) {}
};

double percent_over(double value, double reference) {
    return (value / reference - 1.0) * 100.0;
}

double low_row_fraction(const std::vector<elastic::EpochSpec>& plan,
                        int low_threads, int m) {
    int rows = 0;
...
void print_result_json(const Options& options, const Variant& variant,
                       int n_tile, double flops) {
    const double gflops = flops / variant.stats.median_seconds / 1.0e9;
    std::cout << "RESULT_JSON {\"variant\":\"" << variant.name
              << "\",\"stage\":\"" << options.stage_name << "\",\"m\":"
              << options.m << ",\"k\":" << options.k << ",\"n\":"
              << options.n << ",\"threads\":" << options.threads
              << ",\"low_threads\":" << options.low_threads
              << ",\"epoch_rows\":" << options.epoch_rows
              << ",\"n_tile\":" << n_tile << ",\"median_ms\":"
              << variant.stats.median_seconds * 1.0e3 << ",\"min_ms\":"
              << variant.stats.min_seconds * 1.0e3 << ",\"mean_ms\":"
              << variant.stats.mean_seconds * 1.0e3 << ",\"gflops\":"
              << gflops << "}\n";
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (options.m % 12 != 0 || options.epoch_rows % 12 != 0 ||
            options.k % 8 != 0 || options.n % n_tile != 0) {
            throw std::invalid_argument(
                "M/epoch must align to 12, K to 8, and N to runtime n_tile");
        }
        const int n_tiles = options.n / n_tile;
        if (options.threads > n_tiles) {
            throw std::invalid_argument(
                "high thread count exceeds the number of SVE N tiles");
        }

        std::cout << std::fixed << std::setprecision(4);
        std::cout << "stage=" << options.stage_name << " M=" << options.m
                  << " K=" << options.k << " N=" << options.n
                  << " n_tile=" << n_tile << " lanes=" << options.low_threads
                  << "->" << options.threads
                  << " epoch_rows=" << options.epoch_rows
        ...
        const auto fixed_low = elastic::make_epoch_plan(
            options.m, options.epoch_rows, options.low_threads,
            options.low_threads, 0.0);
        const auto fixed_high = elastic::make_epoch_plan(
            options.m, options.epoch_rows, options.threads, options.threads,
            0.0);
        const auto elastic_plan = elastic::make_epoch_plan(
            options.m, options.epoch_rows, options.low_threads, options.threads,
            0.5);

        run_correctness(options, executor, buffers.front(), n_tile, fixed_low,
                        fixed_high, elastic_plan);
        ...
        variants.push_back({"static_high", [&](const elastic::Problem& p) {
                                return executor.run_static(p, options.threads);
                            }});
        variants.push_back({"static_low", [&](const elastic::Problem& p) {
                                return executor.run_static(p,
                                                           options.low_threads);
                            }});
        variants.push_back(
            {"epoch_fixed_high", [&](const elastic::Problem& p) {
                 return executor.run_epoch_fixed(p, options.threads,
                                                 options.epoch_rows);
             }});
        variants.push_back(
            {"phase_claim_high", [&](const elastic::Problem& p) {
                 return executor.run_phase_claim(p, fixed_high);
             }});
        variants.push_back(
            {"phase_claim_low", [&](const elastic::Problem& p) {
                 return executor.run_phase_claim(p, fixed_low);
             }});
        variants.push_back(
            {"elastic_phase_low_to_high", [&](const elastic::Problem& p) {
                 return executor.run_phase_claim(p, elastic_plan);
             }});
        variants.push_back(
            {"strict_epoch_claim_high", [&](const elastic::Problem& p) {
                 return executor.run_epoch_claim(p, fixed_high);
             }});
        variants.push_back(
            {"strict_epoch_claim_low", [&](const elastic::Problem& p) {
                 return executor.run_epoch_claim(p, fixed_low);
             }});
        variants.push_back(
            {"elastic_strict_epoch_low_to_high",
             [&](const elastic::Problem& p) {
                 return executor.run_epoch_claim(p, elastic_plan);
             }});
        ...
        auto run_round = [&](int round, bool measured) {
            ...
            for (size_t offset = 0; offset < count; ++offset) {
                const size_t variant_index =
                    (static_cast<size_t>(round) + offset) % count;
                Buffers& current = buffers[invocation % buffers.size()];
                ++invocation;
                elastic::Problem problem =
                    make_problem(options, current, n_tile);
                const elastic::RunResult result =
                    variants[variant_index].run(problem);
                if (measured) {
        ...
        for (Variant& variant : variants) {
            ...
            const double gflops = flops / variant.stats.median_seconds / 1.0e9;
            std::cout << std::left << std::setw(24) << variant.name << std::right
                      << std::setw(10) << variant.stats.median_seconds * 1.0e3
                      << std::setw(10) << variant.stats.min_seconds * 1.0e3
                      << std::setw(12) << gflops << '\n';
        }
        ...
        const Stats& elastic_phase =
            find_stats("elastic_phase_low_to_high");
        const Stats& strict_high = find_stats("strict_epoch_claim_high");
        const Stats& strict_low = find_stats("strict_epoch_claim_low");
        const Stats& elastic_strict =
            find_stats("elastic_strict_epoch_low_to_high");
        const double low_fraction =
            low_row_fraction(elastic_plan, options.low_threads, options.m);
        const double predicted_static =
            low_fraction * static_low.median_seconds +
            (1.0 - low_fraction) * static_high.median_seconds;
        const double predicted_phase =
            low_fraction * phase_low.median_seconds +
            (1.0 - low_fraction) * phase_high.median_seconds;
        const double predicted_strict =
            low_fraction * strict_low.median_seconds +
            (1.0 - low_fraction) * strict_high.median_seconds;

        const double fragmentation = percent_over(
            epoch_fixed_high.median_seconds, static_high.median_seconds);
        const double phase_claim_overhead = percent_over(
            phase_high.median_seconds, epoch_fixed_high.median_seconds);
        const double strict_claim_overhead = percent_over(
            strict_high.median_seconds, epoch_fixed_high.median_seconds);
        const double elastic_phase_vs_static = percent_over(
            elastic_phase.median_seconds, predicted_static);
        const double elastic_phase_vs_phase = percent_over(
            elastic_phase.median_seconds, predicted_phase);
        const double elastic_strict_vs_strict = percent_over(
            elastic_strict.median_seconds, predicted_strict);

        std::cout << "\nfragmentation_over_static_pct=" << fragmentation
                  << "\nphase_claim_over_epoch_fixed_pct="
                  << phase_claim_overhead
                  << "\nstrict_claim_over_epoch_fixed_pct="
                  << strict_claim_overhead
                  << "\nelastic_phase_over_piecewise_static_pct="
                  << elastic_phase_vs_static
                  << "\nelastic_phase_over_piecewise_phase_pct="
                  << elastic_phase_vs_phase
                  << "\nelastic_strict_over_piecewise_strict_pct="
                  << elastic_strict_vs_strict
                  << "\nlow_row_fraction=" << low_fraction << '\n';
        for (const Variant& variant : variants) {
            print_result_json(options, variant, n_tile, flops);
        }
        std::cout << "SUMMARY_JSON {\"stage\":\"" << options.stage_name
                  ...
                  << ",\"phase_claim_over_fixed_pct\":"
                  << phase_claim_overhead
                  << ",\"strict_claim_over_fixed_pct\":"
                  << strict_claim_overhead
                  << ",\"elastic_phase_over_piecewise_static_pct\":"
                  << elastic_phase_vs_static
                  << ",\"elastic_phase_over_piecewise_phase_pct\":"
                  << elastic_phase_vs_phase
                  << ",\"elastic_strict_over_piecewise_strict_pct\":"
                  << elastic_strict_vs_strict << ",\"low_row_fraction\":"
                  << low_fraction << "}\n";
