...
extern "C" {
void moe_sve_w13_silu_poly5_packc_m12_rows_opt(
    const uint16_t*, const uint16_t*, uint16_t*, uint16_t*,
    const gemm_params_t*);
void moe_sve_w2_packed_m12(const uint16_t*, const uint16_t*, float*,
...
void run_range(const Problem& p, int row_begin, int rows, int n_begin,
               int n_cols) {
    ...
    const int64_t b_offset =
        static_cast<int64_t>(n_begin / p.n_tile) * p.k * p.n_tile;
    const uint16_t* a = p.packed_a + static_cast<int64_t>(row_begin) * p.k;
    const uint16_t* b = p.packed_b + b_offset;

    if (p.stage == Stage::kW13) {
        auto* c = static_cast<uint16_t*>(p.output) +
                  static_cast<int64_t>(row_begin) * p.ldc +
    ...
    for (int row = 0; row < rows; row += 12) {
        ...
        if (p.stage == Stage::kW2F32) {
            auto* c = static_cast<float*>(p.output) +
                      static_cast<int64_t>(row_begin + row) * p.ldc + n_begin;
...
void run_lane(const Problem& p, int row_begin, int rows, int lanes, int lane) {
    ...
    run_range(p, row_begin, rows, tile_range.begin * p.n_tile,
              tile_range.size * p.n_tile);
}
...
uint64_t make_cursor(uint32_t epoch, uint32_t lane) {
    return (static_cast<uint64_t>(epoch) << 32) | lane;
}
...
class Executor::Impl {
public:
    Impl(int workers, int cpu_start)
        : worker_count_(workers), cpu_start_(cpu_start) {
        if (workers <= 0) {
...
Executor::Executor(int workers, int cpu_start)
    : impl_(std::make_unique<Impl>(workers, cpu_start)) {}
...
RunResult Executor::run_epoch_fixed(const Problem& problem, int lanes,
                                    int epoch_rows) {
    check_problem(problem);
    check_lanes(problem, lanes);
    if (lanes > impl_->workers()) {
...
RunResult Executor::run_phase_claim(const Problem& problem,
    ...
    for (const EpochSpec& epoch : epochs) {
        if (epoch.row_begin != expected_row || epoch.rows <= 0 ||
            epoch.row_begin % 12 != 0 || epoch.rows % 12 != 0) {
            throw std::invalid_argument(
                "epoch plan must be contiguous and 12-row aligned");
        }
        check_lanes(problem, epoch.lanes);
        if (epoch.lanes > impl_->workers()) {
            throw std::invalid_argument(
                "epoch lane count exceeds resident workers");
        }
    ...
    auto next_task =
        std::make_unique<std::atomic<int>[]>(static_cast<size_t>(phase_count));
    ...
    const double seconds = impl_->run([&](int tid) {
        ...
        while (true) {
            const uint32_t observed =
                phase_index.load(std::memory_order_acquire);
            ...
            if (tid >= phase.lanes) {
                ...
                while (phase_index.load(std::memory_order_acquire) ==
                       observed) {
                    cpu_relax(spins);
                }
                continue;
            ...
            const int task_index =
                next_task[observed].fetch_add(1, std::memory_order_relaxed);
            ...
            for (const EpochSpec& epoch : phase.epochs) {
                run_lane(problem, epoch.row_begin, epoch.rows, epoch.lanes,
                         task_index);
            }
            if (!final_phase &&
                remaining.fetch_sub(1, std::memory_order_acq_rel) == 1) {
                const uint32_t next_phase = observed + 1;
                remaining.store(phases[next_phase].lanes,
...
RunResult Executor::run_epoch_claim(const Problem& problem,
                                    const std::vector<EpochSpec>& epochs) {
    check_problem(problem);
    if (epochs.empty()) {
        throw std::invalid_argument("epoch plan must not be empty");
        ...
        while (true) {
            ...
            if (remaining.fetch_sub(1, std::memory_order_acq_rel) == 1) {
                ...
                if (next_epoch < epoch_count) {
                    remaining.store(epochs[next_epoch].lanes,
                                    std::memory_order_relaxed);
                }
                cursor.store(make_cursor(next_epoch, 0),
...
std::vector<EpochSpec> make_epoch_plan(int m, int epoch_rows, int low_lanes,
    ...
    if (low_lanes <= 0 || high_lanes <= 0 || low_fraction < 0.0 ||
        low_fraction > 1.0) {
    ...
    for (int row = 0; row < m; row += epoch_rows) {
        result.push_back(
            EpochSpec{row, std::min(epoch_rows, m - row), high_lanes});
    }
