...
def run_benchmark(args: argparse.Namespace) -> list[dict]:
    ...
    print("=" * 80)
    print(f"ACL GEMM Configure 开销基准测试")
    print(f"  K={args.K}, N={args.N}, dtype={args.dtype}, "
          f"fast_math={args.fast_math}")
    ...
    create_us = bench_create_handler(
        args.K, args.N, dtype, args.fast_math, num_iters=min(args.num_iters, 50),
    )
    ...
    for M1, M2 in m_pairs:
        avg_us = bench_alternating_m(
            handler, M1, M2, args.K, dtype, args.num_iters,
        )
        ...
        print(f"  M={M1}<->{M2}: {avg_us:.1f} μs/次, "
              f"reconfigure 开销 ≈ {reconf_overhead:.1f} μs")
        results.append({
            "场景": f"alternating_M",
            "M": f"{M1}<->{M2}",
            "平均耗时(μs)": f"{avg_us:.1f}",
    ...
    avg_us_small = bench_sequential_different_m(
        handler, moe_m_values_small, args.K, dtype,
    ...
    print(f"  8 个不同 M {moe_m_values_small}: {avg_us_small:.1f} μs/次, "
    ...
    avg_us_large = bench_sequential_different_m(
        handler, moe_m_values_large, args.K, dtype,
    ...
    print(f"  64 个不同 M [1..64]: {avg_us_large:.1f} μs/次, "
    ...
    print("-" * 80)
    for r in results:
        print(f"{r['场景']:<35s} {r['M']:<15s} "
              f"{r['平均耗时(μs)']:<15s} {r['reconfigure开销(μs)']:<15s}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--dtype", type=str, default="bf16", choices=["fp32", "bf16"],
        help="数据类型（默认 bf16）",
    )
    parser.add_argument(
        "--fast-math", action="store_true", default=False,
        help="启用 fast_math 低精度加速路径",
    )
    parser.add_argument(
        "--m-values", type=int, nargs="+", default=[1, 4, 16, 32, 64, 128],
        help="场景 1/2 中测试的 M 值列表（默认 1 4 16 32 64 128）",
    )
    parser.add_argument(
        "--num-iters", type=int, default=200,
