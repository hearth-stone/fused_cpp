...
def test_async_exact_stage_ranges_are_forwarded(monkeypatch) -> None:
    captured: list[tuple] = []
    ...
    for w13_ranges, w2_ranges in ((1, 1), (2, 4)):
        ...
        assert captured[-1][-3:] == (w13_ranges, w2_ranges, None)


def test_exact_stage_ranges_are_forwarded_by_all_entrypoints(monkeypatch) -> None:
    ...
    assert captured["normal"][-3:] == (w13_ranges, w2_ranges, None)
    assert captured["scheduled"][-3:] == (w13_ranges, w2_ranges, None)
    assert captured["async"][-3:] == (w13_ranges, w2_ranges, None)
...
def test_stage_ranges_reject_invalid_values(monkeypatch, value, error) -> None:
    monkeypatch.setattr(bf16_tiled, "_HAS_BF16_TILED_FUSED_MOE", True)
    ...
    with pytest.raises(error, match="w13_ranges"):
        bf16_tiled.fused_moe_bf16_tiled(
            hidden,
            weights,
            torch.ones((1, 1), dtype=torch.float32),
...
def test_async_exact_stage_ranges_preserve_results() -> None:
    ...
    def run(w13_ranges: int, w2_ranges: int) -> torch.Tensor:
    ...
    reference = run(1, 1)
    for w13_ranges, w2_ranges in ((2, 1), (4, 1), (4, 2), (4, 4)):
        candidate = run(w13_ranges, w2_ranges)
...
def test_multithread_stage_ranges_preserve_w2_owner_scatter(monkeypatch, bridge: str) -> None:
    ...
    def run(stage_ranges: int) -> torch.Tensor:
        if bridge == "scheduled":
            return bf16_tiled.fused_moe_bf16_tiled_scheduled(
                ...
                num_threads=threads,
                skip_weighted=True,
                w13_ranges=stage_ranges,
                w2_ranges=stage_ranges,
            )
        return bf16_tiled.fused_moe_bf16_tiled_async(
            hidden,
            packed,
