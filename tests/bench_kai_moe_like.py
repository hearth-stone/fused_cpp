    ...
    def _get_weight(self, k: int, n: int, seed: int) -> torch.Tensor:
        ...
        if key not in self._weight_ref:
            ...
            self._weight_ref[key] = torch.randn(
                k, n, dtype=torch.float32, generator=gen,
            )
...
def _run_sequential(
    ...
    pool = (
        _fcpp.KAIThreadPool(list(range(total_threads)))
        if total_threads > 1 else None
    )
    try:
...
def _run_grouped(
    ...
    """
    assert total_threads % group_splits == 0, (
        "total_threads 必须能被 group_splits 整除"
    )
    ...
    def _worker(wid: int) -> None:
        ...
        worker_pool = (
            _fcpp.KAIThreadPool(list(range(threads_per_handler)))
            if threads_per_handler > 1 else None
        )
        try:
            ...
            while True:
                ...
                t0 = time.perf_counter()
                _ = fused_cpp_mod.kai_gemm(
                    handler, x, output_dtype=out_dtype, pool=worker_pool,
                )
                t1 = time.perf_counter()
    ...
    workers = [
        threading.Thread(target=_worker, args=(i,), name=f"kai-worker-{i}")
        for i in range(group_splits)
    ]
...
def _bench_one(
    ...
    threads_per_handler = (
        total_threads if strategy == "sequential"
        else total_threads // group_splits
    )
    ...
    if strategy == "sequential":
        ...
        def _one_run() -> Tuple[float, List[float]]:
            return _run_sequential(
                specs, inputs, cache, total_threads, seed, out_dtype,
                fused_cpp_mod,
            )
        ...
        def _one_run() -> Tuple[float, List[float]]:
            return _run_grouped(
                specs, inputs, _cache_factory, total_threads, group_splits,
                seed, out_dtype, fused_cpp_mod,
            )
    else:
...
def _write_csv(
    ...
    with open(path, "w", newline="", encoding="utf-8") as fp:
        ...
        writer.writerow([
            "strategy", "total_threads", "group_splits", "threads_per_handler",
            "num_gemms", "total_flops", "wall_time_ms", "gflops",
        ])
        for r in results:
            writer.writerow([
                r.strategy, r.total_threads, r.group_splits,
                r.threads_per_handler, r.num_gemms,
                f"{r.total_flops:.3e}", f"{r.wall_time_ms:.3f}",
                f"{r.gflops:.2f}",
            ])
        writer.writerow([])
        ...
        for idx, s in enumerate(specs):
            ...
            for r in results:
                row.append(
                    f"{r.per_gemm_ms[idx]:.3f}" if idx < len(r.per_gemm_ms) else ""
                )
            writer.writerow(row)
...
def _write_xlsx(
    ...
    headers = [
        "strategy", "total_threads", "group_splits", "threads_per_handler",
        "num_gemms", "total_flops", "wall_time_ms", "gflops", "speedup_vs_seq",
    ]
...
def _print_brief_summary(results: List[StrategyResult]) -> None:
    ...
    print()
    print("=" * 96)
    print(f"{'strategy':<14}{'threads':>10}{'x':>6}{'t/handler':>12}"
          f"{'wall_ms':>12}{'GFLOP/s':>12}{'speedup':>12}")
    print("=" * 96)
    seq = next((r for r in results if r.strategy == "sequential"), None)
    base_ms = seq.wall_time_ms if seq else None
    for r in results:
        speedup = (
            f"{base_ms / r.wall_time_ms:.2f}x"
            if base_ms and r.wall_time_ms > 0 else "-"
        )
        print(
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--total-threads", type=int, default=80,
                        help="总可用线程数（应与 taskset 绑定的核心数一致），默认 80")
    parser.add_argument("--group-splits", nargs="+", type=int,
                        default=[1, 2, 4, 5, 8, 10, 20],
                        help="grouped 策略的分组数列表，必须整除 total_threads；"
                             "值为 1 时会 skip（与 sequential 等价）")
    parser.add_argument("--include-sequential", action="store_true", default=True,
                        help="是否包含 sequential 策略（默认启用）")
    parser.add_argument("--no-sequential", dest="include_sequential",
                        action="store_false",
                        help="关闭 sequential 策略")
    ...
    parser.add_argument("--output-csv", default="bench_kai_moe_like.csv",
                        help="CSV 输出路径")
    parser.add_argument("--output-xlsx", default="bench_kai_moe_like.xlsx",
                        help="Excel 输出路径（需要 openpyxl）")
    return parser
...
def main() -> int:
    ...
    if not (kai_ok and is_aarch64):
        print("[ERROR] KleidiAI 后端不可用（需要 AArch64 且已启用 KleidiAI 构建）",
              file=sys.stderr)
        return 2
    ...
    for x in args.group_splits:
        if x < 1 or args.total_threads % x != 0:
            print(f"[ERROR] group_splits 中的 {x} 无法整除 total_threads={args.total_threads}",
                  file=sys.stderr)
            return 3
    ...
    print(f"[INFO] 共 {len(specs)} 个 GEMM，total_threads={args.total_threads}，"
          f"dtype={args.dtype}")
    ...
    if args.include_sequential:
        ...
        r = _bench_one(
            "sequential", 1, specs, inputs, args.total_threads,
            args.warmup, args.repeat, args.seed, out_dtype, fused_cpp,
        )
    ...
    for x in args.group_splits:
        if x == 1:
            ...
            continue
        print(f"[INFO] 运行 grouped(x={x}) 策略 "
              f"({args.total_threads // x} threads/handler) ...")
        r = _bench_one(
            "grouped", x, specs, inputs, args.total_threads,
            args.warmup, args.repeat, args.seed, out_dtype, fused_cpp,
        )
        print(f"       wall={r.wall_time_ms:.2f} ms, {r.gflops:.2f} GFLOP/s")
        results.append(r)
