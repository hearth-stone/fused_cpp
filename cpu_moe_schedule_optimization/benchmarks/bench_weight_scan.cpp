...
Options parse_args(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (arg == "--cpu-ids") {
      ...
      options.stream_bytes = static_cast<std::size_t>(std::stod(value()) * static_cast<double>(1ULL << 20));
    } else if (arg == "--passes") {
      ...
      std::cout << "Usage: bench_weight_scan --cpu-ids 0-95 "
  ...
  if (options.stream_bytes < kCacheLine || options.stream_bytes % kCacheLine != 0) {
...
void pin_thread(int cpu_id) {
  ...
  if (error != 0) {
    throw std::runtime_error("pthread_setaffinity_np failed: " + std::string(std::strerror(error)));
  }
#else
  (void)cpu_id;
#endif
...
class Barrier {
 public:
  explicit Barrier(int participants) : participants_(participants), remaining_(participants) {}
...
WorkerRange worker_range(int worker, const std::vector<int>& shape, std::size_t lines_per_stream) {
  int begin = 0;
  for (int group = 0; group < static_cast<int>(shape.size()); ++group) {
    ...
    if (worker < end) {
      ...
      const std::size_t local_begin = static_cast<std::size_t>(local) * base + std::min<std::size_t>(local, extra);
      return {
...
Result run_point(const Options& options, int groups) {
  ...
  for (int worker = 0; worker < cores; ++worker) {
    workers.emplace_back([&, worker] {
      ...
      for (std::size_t line = range.first_line; line < range.first_line + range.line_count; ++line) {
        data[line * (kCacheLine / sizeof(std::uint64_t))] = static_cast<std::uint64_t>(line + 1);
      }
      ...
      for (int iteration = 0; iteration < iterations; ++iteration) {
        ...
        for (int pass = 0; pass < options.passes; ++pass) {
          for (std::size_t line = range.first_line; line < range.first_line + range.line_count; ++line) {
            local += volatile_data[line * (kCacheLine / sizeof(std::uint64_t))];
          }
        }
        finish_barrier.wait();
      }
  ...
  const double scanned_bytes = static_cast<double>(total_bytes) * options.passes;
  return {
      groups,
...
int main(int argc, char** argv) {
  try {
    ...
    for (int groups : options.groups) {
      ...
      std::cout << result.groups << ' ' << result.min_team_threads << ' ' << result.max_team_threads << ' '
                << result.working_set_bytes / static_cast<double>(1ULL << 20) << ' ' << result.median_ms << ' '
                << result.p10_ms << ' ' << result.p90_ms << ' ' << result.bandwidth_gbs << '\n';
    }
    if (!options.output_csv.empty()) {
      ...
      for (const Result& result : results) {
        output << result.groups << ',' << result.min_team_threads << ',' << result.max_team_threads << ','
               << options.stream_bytes << ',' << result.working_set_bytes << ',' << options.passes << ','
               << result.median_ms << ',' << result.p10_ms << ',' << result.p90_ms << ',' << result.bandwidth_gbs
               << '\n';
      }
    }
    return 0;
