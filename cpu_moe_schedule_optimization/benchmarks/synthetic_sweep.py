...
def run_case(
    ...
    baseline = plan_by_kind(plans, PlanKind.SORTED_TOKEN_BALANCED_1T)
    if baseline is None:
        raise RuntimeError("balanced baseline plan missing from planner set")
    ...
    total_speedup = baseline.estimated_total_cost_ns / best_total.estimated_total_cost_ns
    exec_speedup = baseline.estimated_execute_cost_ns / best_execute.estimated_execute_cost_ns
    ...
    return {
        ...
        "baseline_total_ns": baseline.estimated_total_cost_ns,
        "baseline_execute_ns": baseline.estimated_execute_cost_ns,
        ...
        "execute_speedup_vs_baseline": exec_speedup,
        "total_speedup_vs_fixed": (
            fixed.estimated_total_cost_ns / best_total.estimated_total_cost_ns
        ),
        "execute_speedup_vs_fixed": (
            fixed.estimated_execute_cost_ns / best_execute.estimated_execute_cost_ns
        ),
...
def print_summary(rows: Sequence[Dict[str, object]]) -> None:
    header = (
        ...
        f"{'baseline':<24} {'best_total':<24} {'total':>10} {'vs_base':>8} "
        f"{'auto':<24} {'regret':>7} "
    ...
    for row in rows:
        ...
        print(
            ...
            f"{str(row['baseline_kind']):<24} "
            f"{str(row['best_total_kind']):<24} "
            f"{format_ns(int(row['best_total_ns'])):>10} "
            f"{float(row['total_speedup_vs_baseline']):>8.2f} "
