...
| 2026-09-21 | v1.119 | 2T 缺陷定位到机理：并非 2T 特有，也不在窗口项。E12（9 个新层、三个家族仅改窗口几何）三条冻结读数全部为否，手工搭的 2T 重计划预测良好（1.03--1.13）；546 个已测计划的元分析显示误差由"紧密打包 + 2T 份额"携带（利用率≥0.88 时无 2T 为 1.093、2T 份额≥0.2 为 1.218，而窗口信用 0→0.25 只带来 1 个点）。窗口表自己的满载同宽网格给出机理：每核 live B 切片 = 12 MiB/t 对私有 L2 1.25 MiB，无窗口时四个宽度扩张相差 0.16--0.56 且窄 lane 最差（M=96：2T 1.57、4T 1.26、8T 1.15、16T 1.10，正是 4.8x/2.4x/1.2x/0.6x 溢出序），加 1-tile 窗口（live 切片 128 KiB）后相差降到 0.03--0.13、四个宽度同为 1.05--1.13；M=12 只有一个 panel、无跨 panel 复用可失，故窗口无用而各宽度同为 1.5--1.8。模型的曲线以(目标宽度, 其他核数)为参数，无法表达同核数不同切片，误差恰为切片可解释的部分。修正方向：争用状态改以每核 live 切片/私有 L2 参数化；标定数据已存在（溢出 0.6--4.8x 与 <1 两段），须在新层上先验证。本条只记录诊断，未改模型、表、planner 默认或搜索空间。报告：`optimizations/fused_moe_sve/results/narrow_lane_l2_mechanism_20260921.md`。 |
