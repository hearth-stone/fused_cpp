...
$R>1$ 时禁用 stripe 级 `bulk_m`，因为它假设一次调用覆盖 worker 的整条 stripe；
窗口下每个 window 都是一次独立调用。`first_panel_prefetch` 因 E2E 无稳定收益且高并发、
长 route 可回退，已经从运行时和 JIT 生成器移除，仅在 Git 历史与实验报告中保留。
