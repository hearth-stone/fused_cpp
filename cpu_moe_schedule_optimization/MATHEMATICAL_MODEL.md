...
| 2026-09-21 | v1.119 | 2T 缺陷定位到机理：并非 2T 特有，也不在窗口项。E12（9 个新层、三个家族仅改窗口几何）三条冻结读数全部为否，手工搭的 2T 重计划预测良好（1.03--1.13）；546 个已测计划的元分析显示误差由"紧密打包 + 2T 份额"携带（利用率≥0.88 时无 2T 为 1.093、2T 份额≥0.2 为 1.218，而窗口信用 0→0.25 只带来 1 个点）。窗口表自己的满载同宽网格给出机理：一个 40 核 LLC 域(70 MiB)并发持有的权重在整条带下为 480 MiB/t（2T 240M=3.4x、4T 120M=1.7x、8T 60M=0.9x、16T 24M=0.3x），无窗口时四个宽度扩张相差 0.16--0.56 且窄 lane 最差（M=96：2T 1.57、4T 1.26、8T 1.15、16T 1.10，正是 4.8x/2.4x/1.2x/0.6x 溢出序），加 1-tile 窗口后每域并发足迹一律约 5 MiB（0.07x，与宽度无关），相差降到 0.03--0.13、四个宽度同为 1.05--1.13；窗口 PMU（v1.103：DRAM 读降 27--75%、L2 refill 仅降 0--9%）指向 LLC 而非私有 L2；M=12 只有一个 panel、无跨 panel 复用可失，故窗口无用而各宽度同为 1.5--1.8。模型的曲线以(目标宽度, 其他核数)为参数，无法表达同核数不同切片，误差恰为切片可解释的部分。修正方向：争用状态改以 LLC 域的并发 live 权重足迹参数化；标定数据已存在（溢出 0.3--3.4x 与 0.07x 两段），须在新层上先验证。本条只记录诊断，未改模型、表、planner 默认或搜索空间。报告：`optimizations/fused_moe_sve/results/narrow_lane_cache_mechanism_20260921.md`。 |
| 2026-09-21 | v1.120 | 2T 价值判定：关闭。用满载网格的实测每 expert 核时间给出工作量下界——允许 2T 27.88 ms、仅 ≥4T 28.13 ms、而现有 ≥4T 参照实测 28.25 ms，即 2T 只把下界改善 0.82%，参照已在含 2T 下界之上 1.3%。E13 在 E5 的 18 个层上实测六个手搭变体，全部比参照慢 15--20%、无一层取胜（同形状的 4T 对照 +15.23%，说明差距主要来自手搭形状而非宽度）。另证伪一条由机理推出的放置规则：把 routes≤16 的 expert 排除出 2T lane 反而慢 5.10%（失衡代价大于 LLC 溢出代价）。结论：2T 继续排除在搜索空间外，不为 2T 做模型修正；机理的价值保留在解释窗口表与模型整体偏差上。 |
| 2026-09-21 | v1.121 | P9（足迹探针）：窗口扫 w13 ∈ {1,2,4,8,16,32}、W2 钉 4 tile、宽度 2/4/8/16、M 24--720。匹配 ρ 时各宽度扩张一致到 0.008--0.083（中位约 0.04），而匹配宽度、放任足迹时（整条带行）跨宽度相差达 0.40（M=96：1.48 对 1.08）——**状态变量是足迹不是 lane 宽度**；但 D 不是 ρ 的单变量函数，冻结的 0.05 容差在 16 个可比格子中 4 个未达（最大 0.083，集中在 16T 小 M 与 4T 大 M）。D 随 ρ 单调上升且从最小足迹起就平滑上升（2T、M=96：ρ 0.07→2.29 对应 1.04→1.48），无容量拐点；混淆方向有利：窗口越大 A 重扫越少，故足迹效应至少有这么大。E5 参照计划按孤立时间加权有 78.8% 的工作在 ρ≤0.2、10.2% 在 ρ>1。本条只记录测量，未改模型。 |
| 2026-09-21 | v1.122 | 候选 v14：模拟器按 LLC 域累计 live 权重，用 P9 探针格标定的 c(ρ,M)=实测扩张/本模型扩张 乘在合成扩张上，并按域繁忙比例淡入（孤立任务不变）；标定不含任何整计划时间。描述性检验（588 个已测计划，均未参与标定）：实测/预测 1.090→1.061、绝对偏差中位 0.090→0.061、含 2T≥20% 的计划 1.112→1.031。E14 在 9 个新层（每层 6 个计划，其中两个分别在 v11/v14 下同起点同种子各搜 45 s）验收：W1 水平过（1.075→1.055，+0.021 对门槛 0.02）、**W2 排序不过**（Spearman 中位 0.94→0.89）、**W3 选择不过**（regret@1 中位/最大 0.00%/0.00% 对 1.89%/4.16%），且 lns_v14 实测比 lns_v11 慢 1.89%、0/9 取胜。按冻结规则 **v14 不采纳，v11 仍为参照模型**；机理（P9 的足迹状态变量）保持成立，失败的是"把同宽满载格子的比值乘进异构计划"这一具体形式。 |
| 2026-09-21 | v1.123 | 第二台机器（C9g：Neoverse-V3、2×96 核 NUMA、每节点 96 MiB L3、SVE-128 故 n_tile=8）在 TP4/TP2 两个形状上的实测。足迹机理跨机器跨形状成立：整条带相对最佳窗口的惩罚按 ρ 对齐（ρ=1 时 1.30/1.32、ρ=2 时 1.98/2.06、ρ=4 时 2.97/3.12，两个形状每 expert 权重差一倍仍落同一条曲线），**ρ 是状态变量、宽度不是**；水平是机器属性（codex 在 ρ=2.29 只有 1.47）。但**最优宽度不跨机器**：C9g 上开表窗口后 M≤192 一律 2T 最快（M=96 时 9.44 ms 对 16T 的 11.90），而 codex 上各宽度相差约 1%——E13 关闭 2T 的结论只对 codex 成立。每 expert 核时间 6.4 对 15.84 核·ms（约 2.5 倍效率）；TP2 每单位工作量比 TP4 省约 13%。该机解析标定已建（服务探针 + 96 个孤立训练点 + 按宽度开销表）：planner 工作区（宽度≤16、M≥24）MAPE 2.0%/4.5%，`M<宽度` 区高估至 42%（物理项按每核计工作量，非负开销无法抵扣），M 刚过 12 行 panel 边界处符号翻转（−16%/−29%，模型的 panel 量化与 kernel 不一致）。事件模型探针曲线未移植，故该机争用侧尚不可用。报告：`optimizations/fused_moe_sve/results/second_machine_c9g_20260921.md`。 |
| 2026-09-21 | v1.124 | 三项离线/静态结论，均未改模型、表、planner 默认或搜索空间（两台机器均不可达，无任何构建与实测）。(a) C9g 的 W8A8 缺陷是**向量长度假设**而非竞态：`i8gemm_k_hybrid`/`narrow`/`narrow2` 的 `HSTORE_PAIR` 每行固定写 64 字节，而 N 步进为 `svcntb()/2` 列，仅在 256-bit 下相等；128-bit 机器上多写 8 列并越过相邻线程的 N 条带，末尾越界 32 字节即堆破坏。宽度 1 干净是因为它走 NEON `nld` 系；ASAN 干净是因为越界发生在手写汇编中（不被插桩），该证据不能反证越界。最小修法为 `svcntb() != 32` 时强制走 packed 路径，**未落地**（无法编译与验证）。(b) C9g 解析标定的 panel 残差不是 panel 量化错误，而是**缺少每 panel 项**：kernel 每个 M12 panel 重流一次权重，实测每 panel 495 µs 中约 210 µs 与行数无关；无参数恒等式 `cost(16)=cost(4)+cost(24)-cost(12)` 在 1T/2T 内差 0.5%/0.2%，而标定模型差 −9.4%/−2.8%。`phase_model._formula_iso` 与 `tiso_roofline.panel_histogram` 已有该结构，`analytic_model` 的 B 流量只按 cold/steady 分档，故缺一次权重流；`m12_effective_rows` 的尾表与 kernel 分派一致，唯一真实分歧是取模基数（模型 `routes % 12`、m8 分派 `routes % 8`），在现有 profile 点上不可见。(c) 546 个已测计划的误差分解：层间水平只占方差 15.3%，**层内计划间占 84.7%**——层内近似恒定的修正（足迹项即属此类）按定义只能改水平，这解释了 v12--v14 为何都改善水平而排序不动；候选项应先按层内变异筛选。层内残差与模型自算的 `window_credit`(r=+0.672)、`loading_share`(−0.527) 最相关，符号表明**窗口信用在计划间偏高**；`0.61*window_credit − 2.94*loading_share` 在留一层交叉验证下 Spearman 中位 0.817→0.917、regret@1 均值 1.65%→0.27%，留一实验集下 E5 由 0.642/5.72% 改善到 0.875/0.07% 且无实验集明显变差，系数在所有折上稳定（+0.41..+0.74）。按 E14 的先例**不采纳**，E15 设计（判据偏重排序与选择）已冻结于 lab 记录。报告：`optimizations/fused_moe_sve/results/offline_findings_20260921.md`。 |
| 2026-09-21 | v1.125 | 窗口表解析改为**机器封闭**。`AMAZON_C5_192C_TP4_F512_V5` 原本没有 `machine_ids`，按 `matches_machine` 的空元组语义对任意机器都匹配，于是**任何 H=4096、F=512、n_tile=8 的机器都会继承这张在 AmazonC5192Cores NUMA0 上测的表**——C9g 的 TP4 正是这个形状，而它自己的网格选出的窗口不同（v1.123）。本次给该表补上其两个 NUMA0 标定的 `machine_ids`，并新增 `_validate_registry`：注册表里的每张表都必须声明所测机器，否则导入即失败。模块不变量随之从"未标定的 shape 保持整条带"扩展到"未标定的机器也保持整条带"，不再有按 shape 的兜底。公式、schema、表内容、planner 默认搜索与标定资产均不变，改变的只是哪台机器能解析到哪张表。影响面：(a) 未注册机器由"继承别人的表"变为整条带——方向正确（宁可无窗口，不可用错窗口）；(b) `ContentionCostModel` 只有 `policy` 没有 `calibration`，machine_id 恒为 `None`，因此经验 profile 路径此后恒为整条带，此前它是按 shape 拿到 C5 表的，同一缺陷；(c) C5 机器本身若用 quick 标定，其 machine_id 形如 `{node}-rank-sve-jit-quick-v1`，不在已注册的两个 id 中，故该机器的 quick 路径也会失去窗口，待下次使用该机器时补录。(b)(c) 与"把训练 profile 并入一键标定"一并记入 `TODO.md` 的"One-click calibration and deployment portability"。新增测试：注册表不变量（含直接构造的 policy 仍可用、只拒绝注册）、C5 表只在其两个 id 上解析、C9g TP4/TP2 与未知机器均解析为 `None`。 |
| 2026-09-22 | v1.126 | 两台机器恢复后的复验，并**修正 v1.124(a) 的一半结论**。代码按用户要求全量镜像本地（不在远端建 commit），两机重建通过（codex 增量 91 s；C9g 为重装后的裸机，按记录的 recipe 重新 bootstrap 到 torch 2.8.0，gcc 15.2 下构建通过）。验证：native hot-wide planner 首次在 256-bit 机器上链接并绑定成功；窗口表机器封闭端到端成立（C9g 的 TP4/TP2 两个形状现在都解析为 `None`，此前 TP4 会静默继承 Amazon C5 的表）；`test_moe_stage_window_plan` 在 codex 的 t=3/6/12 挂 6 个而在 C9g **199 全过**，证实其成因是 legacy 的 `n_split_range` 按构建常量 `kKernelTile=8` 分块、而 stage window plan 按运行时 n_tile 分块（C9g 的 n_tile 恰为 8 故重合；逐宽度实测：所有 2 的幂宽度全 match，仅 3/6/12 不同；生产 supported_widths 全为 2 的幂，不受影响）；`test_moe_analytic_model` 的 5 个 shadow window 失败在两机同名复现，且把 analytic_model.py 与测试换成 HEAD 版本后依旧，确认与机器及用户未提交的 WideTeamPressureCalibration WIP 均无关。**W8A8 修正**：六个形状每个单独进程实测，**256-bit 上 6/6 正确、128-bit 上 6/6 堆破坏**——缺陷随向量长度而非线程数，这一点比静态推断更强；但其中三个形状走 packed NEON 路径、根本不执行 `HSTORE_PAIR` 却同样破坏，故 v1.124 记的『固定 16 列存储』只解释 hybrid 那部分，**不是缺陷全貌**，据此提出的 `svcntb() != 32` 强制走 packed 的修法**无效**（该路径本身在 128-bit 下也坏），未落地。待定位的 VL 假设候选与二分顺序记于 `tmp/w8a8_vector_length_20260921/` 与 TODO。另记：全量 `tests/` 在两机均以 SIGABRT 终止于 SDPA 套件，属另一条 gate。本条不改模型、表、planner 默认或搜索空间。 |
| 2026-09-22 | v1.127 | i8gemm SVE 内核的向量长度缺陷已修复并双向验证（不触及模型、表、planner 默认或搜索空间）。先用只链接 `i8gemm_sve.S` + `i8gemm_hybrid.S` 的独立单测确认缺陷在库内而非调用方：向内核要 `N = svcntb()/2 = 8` 列，两个内核族都写到第 15 列。修法在 store 宏内、汇编期选择：`DEINT_QUADS` 与 `HSTORE_PAIR` 改为**每 128 位向量长度发两个 quad**（原为无条件四个），由 `FUSED_CPP_MOE_SVE_VECTOR_BITS` 驱动——该宏由构建探测、`csrc/moe/common/backend.cpp:281` 对运行线程校验，且 `setup.py:186` 本就把 define 传给 native 源，故无需改构建。512 位改为汇编期 `#error`（bias 模式的四个 quad 偏置占 z8--z11、`LOAD_SCALED_TILE_AUX` 也固定读四个 scale quad，推广到八个需重排寄存器预算且无机器可验；报错优于静默少写半个 tile）。验证：**256 位下两文件反汇编与原版逐字相同**（对 Arm-codex 是 no-op，无需等其恢复）；128 位下单测由写到第 15 列变为第 7 列；C9g 上 `test_moe_w8a8` 由堆破坏变为 **7 passed**，四个形状对动态量化参考的最大绝对误差 2.861e-06(H=64) 与 1.007e-03(H=2048)，**与 Arm-codex 的数值完全一致**；C9g MoE 套件 298 passed、kernel 80 passed/1 skipped，无回归。未改且已记录：`DEINT_SCALED_QUADS`/`LOAD_SCALED_TILE_AUX`（后者在 128 位下会读越 scale/bias 数组）与 `i8gemm_msplit_k.S` 带同样缺陷，但 W8A8 MoE 路径不经过它们，无从验证，故不盲改。遗留：`refs/` 在 .gitignore 中，该补丁无法提交，交付方式待定（补丁内容已记入报告）。 |
| 2026-09-22 | v1.128 | 标定流程修正：**服务探针不得只跑一次**。起因是 C9g 重装后做新旧实例对照——三次重复把噪声与真实差异分开后发现，`gemm_core_flops` 在 4T/96T、`matrix_flops` 在 1T 的三次重复离散达 45--47%，而 `l2_bytes` 各宽度只散 0.4--1.9%、`dram_bytes`/`llc_bytes` 不到 2%——**是特定资源在特定宽度上不可复现，不是整体噪声**。三次中的一次直接产出退化标定：所有拟合 overhead 为 0、训练 MAPE 20.3% 对另两次的 10.2--10.7%、planner 工作区 MAPE 7.8%/8.5% 对 2.3%/4.7%；且该退化标定构建无报错、服务率看着正常，链路不给任何提示。`build_analytic_calibration.py` 改为接受多个探针文件并**逐点取中位数**（`median_service_probe`，校验 kind 与 machine.id 一致、不修改输入、单文件仍可用但告警）；写入 `docs/agent_benchmark_hygiene.md` 新增节。用中位数重建 C9g：tp4 planner 区 2.3%/最差 9.4%、tp2 4.7%/27.6%，恢复到 2026-09-21 的 2.0%/4.5% 水平。两条对既有证据的影响：(a) 本规则之前的全部标定资产（含 Arm-codex 的 v9 等）都是单样本，C9g 的 09-21 资产经复核落在样本间散布内属好样本，但那是运气不是流程，定量引用前应复核；(b) **用原始服务率做跨运行/跨机器的验收判据是错的**——C9g 的 `l2_bytes` 可复现地高 13--21%，但拟合出的 expert_fixed/route_ns/stage_scale 全部落在各自的样本间散布内，该差异根本没传导到模型；应比较拟合参数与 planner 区残差，而非探针本身。本条不改模型公式、表、planner 默认或搜索空间。 |
| 2026-09-22 | v1.129 | C9g 的跨 NUMA 干扰基线（E10 在第二台机器上的复现），并修正 idle 闸门。被测作业为该机已验证的满载网格 12 格（宽度 2/4/8 × routes 96/192 × W13 {整条带,1 tile}，TP4），背景为 E10 原样的 `background.py` 置于 node 1（96 线程）。**结论：E10 的规则原样转移**——外部节点跑 BF16 矩阵乘 −0.06%/+0.15%（12 格中 4 格变慢，无可测效应），跑内存流式 **+1.06%/+0.90%（12/12 全部变慢，最小 +0.12%）**；codex 对应为 +0.01%/+0.24% 与 +0.83%/+1.84%，单节点流式代价量级相同。机理提示：C9g 是 96 核共享一片 96 MiB LLC（codex 为 40 核/70 MiB）、每域足迹为其 2--4 倍，跨节点代价却相同，说明该干扰在**节点间通路（内存控制器/互连）**而非 LLC，与决定窗口价值的足迹机理是两回事。**副产物（更重要）**：首尾两个 idle session 相差 0.68%，与待测效应同量级；追查后发现并非机器属性——四个背对背的干净 idle session 显示该机跨 session 下界为**每格离散中位 0.49%、session 中位差 ±0.1%**（codex 0.18%），而那个 0.68% 来自**第一个 idle session 紧接在我方上一个网格运行结束后 27 秒启动**，`wait_idle` 因 1 分钟负载均值已降到 0.39 而放行，此时 5 分钟均值仍为 3.10。对干净基线重算，离群的是该 session（+0.54%，格间 −1.95%~+1.53%）而非其它任何 session（均在 ±0.15%）。修正：新增 `scripts/wait_idle.sh`，要求 1 分钟与 5 分钟负载均值同时落定并持续一段冷却期，超时返回非零；`docs/agent_benchmark_hygiene.md` 的 Machine Sharing 节补 C9g 列与该闸门。报告：`optimizations/fused_moe_sve/results/numa_interference_c9g_20260922.md`。本条不改模型、表或 planner 默认。 |
| 2026-09-22 | v1.130 | Panel 探针（v1.124 预注册的实验，在为区分而选的 route 数上实测，C9g 新实例、中位数标定、每 pass 前过 `wait_idle`，两形状各两遍，重复误差 0.01--1.36%）。**主命题不确定，模型不改**：零参数恒等式 `cost(q)=a+P·floor(q/12)+tail(q%12)` 在 routes 17/20/36 上两形状两遍全部落在 1.6% 内（接近重复误差），但在 **routes 13 上四遍同号高估**（TP4 +2.0/+3.6%、TP2 +4.9/+4.9%），最差 3.6%/4.9% 落在事先冻结的 3--5% 不确定带内，故按规则不采纳 per-panel 项。同批显示现有按行速率模型在 routes 13 上错 −30.1%/−33.7%、routes 20 上错 −8.2%/−12.0%，即恒等式不完美并不能救按行速率：两者在 13 处都错，方向相反且量级悬殊。**副命题已定**：尾表的取模基数是 `routes % 8` 而非模型用的 `routes % 12`——在 routes 9/10 上 `%12` 错 +11.4%/+11.3%（把九行任务凑成 kernel 从不执行的整十二行 panel），`%8` 为 −5.2%/−5.7%；routes 17/18 上 `%8` 为 −0.7%/−0.9% 而 `%12` 为 +5.6%/+5.4%；13/14 两者难分。证实了 `packc_w13_tail_dispatch` 的静态读法。该修正为 planner 可见的 cost model 改动，且 routes 9/10 低于 planner 工作区（宽度≤16、M≥24），故尚无实测后果，另行决策。两表在 routes 9/10 上都不对（`%8` 仍低 5%），改基数只把误差从 +11% 移到 −5%。本轮全部在 1 线程，4--16 线程区未触及。报告：`results/panel_term_c9g_20260922.md`。 |
| 2026-09-22 | v1.131 | C9g 三项实验。**(1) 窗口表跨实例复现**：新实例按同一设计重测网格、同一条 09-19 冻结规则组表，TP4 35 格中 28 格、TP2 30 格选择相同，时间尺度 r 漂移中位 0.001、最大 0.029；12 处差异中 10 处是 1-tile 与 2-tile 之争且 r 几乎不动，2 处实质差异所在格子窗口收益本就不足 3%。窗口表是机器型号的属性，不是个体实例的。**(2) per-panel 项继续不采纳**：routes 25/26/37/38 把两个冻结读法都证伪——不是尾形（TP2 两行尾与一行尾失配同样大），也不是随 panel 数增长（误差反而递减）；绝对值上是近似固定的偏置（TP4 一行尾约 +19 µs、TP2 约 +85 µs），百分比下降只因总耗时从 760 µs 涨到 1746 µs。TP4 只有一行尾受影响而 TP2 两种都受影响，该不对称未解释。**(3) 2T 拒绝是对的，且非边际**：18 个新层上，放开 2T 与仅 ≥4T 出的计划 18/18 逐字相同（宽度搜索从不选 2T）；强制 48 个 2T lane 0/18 取胜、中位 +157.5%（最好 +48.0%、最差 +257.2%）；强制 24 个 4T lane 对照也 2/18、中位 +35.7%，说明是强制窄同构宽度之过而非 2T 特有。**据此撤回 v1.123 的一条推论**：『最优宽度不跨机器、E13 关闭 2T 只对 codex 成立』不成立——同构满载网格给每个 expert 相同 M 且各 lane 均衡，是 2T 的最佳情况；真实层路由倾斜（203--249 活跃 expert、头部很重），96 核切 48 条 2T lane 后 makespan 被最重 lane 绑死，失衡代价压过 cache 收益，与 E13 在 codex 上证伪『小 expert 不上 2T』时发现的是同一件事。**同构满载网格只为该工作负载排宽度，不能外推到真实层。** 报告：`results/c9g_width_and_panel_20260922.md`，并在 09-21 报告对应段落加了撤回说明。本条不改模型公式、表或 planner 默认。 |
| 2026-09-22 | v1.132 | 协议：**跨向量长度复用实验脚本前先查 packed-B tile 假设**。成因是两个常数在 VL=256 上数值重合：NEON quad 恒为 8 个 int32 列（硬件常数），SVE packed-B tile 为 `svcntb()/2`（VL 128 → 8、256 → 16、512 → 32）；256 位上 tile 是 quad 的两倍，128 位上两者相等，所以混用二者的代码在 256 位机器上偶然正确。一天内在 C9g 上撞到三次：legacy 的 `n_split_range` 用 quad 切 N 而窗口计划用 tile（v1.126）、vendored i8gemm 的 SVE store 写死 16 列而 N 步进为 `svcntb()/2`（v1.127）、plan benchmark 因写死 `n_tile == 16` 拒跑。三条规则写入 `docs/agent_benchmark_hygiene.md` 新增节：(a) tile 一律从构建读 `fused_moe_bf16_tiled_backend_n_tile()`，不得用字面量——仓库的几何测试正是这么做的，所以在第二个向量长度上一字未改即通过；(b) **编码了 tile 计数的产物必须声明其所依据的 tile，消费方校验不符即拒**——Plan V2 的窗口按 tile 计数，把 n_tile 8 的计划拿到 tile 16 的构建上跑会静默地把每个窗口的物理尺寸翻倍且不报错，所以单纯删掉写死的检查比保留它更危险；(c) 按危险程度分类：拒跑型只是不便，`full_stage_geometry(..., n_tile=16)` 这类字面量会静默算错，优先处理。现存 16 个实验室脚本写死 16，已按三类记入 TODO；被跟踪的生产代码干净（唯一两处是 codex 窗口表声明自身测量条件）。本条不改模型、表或 planner 默认。 |
| 2026-09-22 | v1.133 | **更正 v1.130 的副命题**：那里说『尾表取模基数是 routes%8，模型用的 routes%12 在 routes 9/10 上错 11%』，比的是 `static_bucketed` 策略的表——而实际使用的标定是 `xbyak_exact_m`，`_exact_m=True` 时 `m12_effective_rows(q) == q`，模型对行数根本不取整。按模型真正计费的行数重算（1 线程，两形状两遍）：平均 |误差| exact 2.3%/3.4%、`%8` 2.4%/3.6%、`%12` 5.4%/5.9%；routes 9/10 上 exact 与 `%8` 同为 −5.2%/−5.7%，`%12` 才是 +11.4%/+11.3%。**活跃路径已是三者中最好，不存在待修的取模缺陷**，原先『取模基数待决策』一项撤销。真正存留的残差是：**routes 9/10 被所有候选（含 exact）低估 5.2%--5.7%**，且任何行数表都解释不了它，因为 exact 与 `%8` 在这两点计费相同。教训：比较模型与实测前，先确认该标定激活的是哪条策略——`m_tail_policy` 有两种，行为完全不同。已在 `results/panel_term_c9g_20260922.md` 加更正节。 |
| 2026-09-22 | v1.134 | **注册 C9g 的 stage window 表** `AMAZON_C9G_192C_TP4_F512_N8_V1`（machine_ids 限定 `amazon_c9g_192c_2numa_96c_sve128_tp4`，TP4 H=4096 F=512 backend tile 8，6 个 route band 覆盖 17--720，1--16 与 >720 保持整条带，宽度 2--32 之外未标定）。依据不是网格而是整计划：18 个新层、同一 quick planner 与同一中位数标定、**唯一差别是窗口策略**，两 session，窗口 **18/18 取胜、中位 −2.07%**（最好 −7.56%、最差 −1.24%，每层都单独越过 1% 门槛），两 session 重复误差 0.00%--0.16%，即最小增益仍是最大重复误差的八倍；判据（≥14/18 且中位 ≥1%）收集前冻结，1% 门是对着该机 0.1% 的跨 session 下界与 0.49% 的每格离散定的。对比 codex 的 E11 整计划 1.69%：同量级、此处略高，与该机每域足迹更大一致。**网格的 3--4 倍不外推到整计划**——计划的时间摊在许多 expert 与 route 数上，多数落在窗口收益最大的带之外，且 makespan 由最慢 lane 决定而非平均格子。表由本实例网格按 09-19 未改规则组成，并跨实例复现（TP4 35 格中 28 格、TP2 30 格选择相同，r 漂移中位 0.001）。该表与 Amazon C5 表**形状完全相同**（H=4096/F=512/tile 8，因 128 位 SVE 按 8 切片），仅靠 `machine_ids` 区分——若无 2026-09-21 加入的机器封闭（v1.125），此次注册无法进行：C9g 会静默继承 C5 的表，C5 的机器也会继承 C9g 的。限制：仅 TP4；TP2 表已组好且跨实例复现 30/35，但无整计划验证，不注册。仅 quick planner，未跑参照搜索。报告：`results/c9g_window_table_20260922.md`。 |
| 2026-09-23 | v1.135 | **注册 C9g 的 TP2 窗口表** `AMAZON_C9G_192C_TP2_F1024_N8_V1`（machine_ids 限定 `..._tp2`，H=4096 F=1024 tile 8）。整计划验证：六个 TP4 验证未用过的新层、同一方法与同一条冻结判据，窗口 **18/18 取胜、中位 −4.82%**（最好 −8.81%、最差 −1.11%），是 TP4 的 −2.07% 的 2.3 倍——与足迹机理给出的方向一致（TP2 每 expert 权重两倍，同宽度下每域足迹两倍）；最差那层仅略过 1% 门，已在报告注明。**并密集扫描 routes 1--30 定位到一个贯穿性结构：孤立耗时是周期为 2 的阶梯**——奇数→偶数几乎免费（+0.3~+2.7 µs），偶数→奇数约 +50 µs，即内核按**行对**计费（smmla/bfmmla 的 2×2 结果形状）；另有 12→13 与 24→25 各跳约 258/263 µs（TP2 约 544/528，约两倍，与权重两倍一致），那正是新 M12 panel 的权重重流。把有效行数由 `q` 换成 `2·ceil(q/2)`，routes 3--30 的平均 |误差| 由 2.47%/2.25% 降到 **0.12%/0.10%**，最差点由 17% 降到 0.6%（routes 13 由 −16.9% 到 +0.0%）。**该结构统一了本会话分头追查的三个残差**：routes 9/10 低估 5.2--5.7%（9 按 10 收费）、routes 13 的固定偏置（13 按 14 收费）、以及『TP4 只有一行尾受影响而 TP2 两种都受影响』的不对称（一行尾=奇数、两行尾=偶数，TP4 的两行尾本就正确）。**per-panel 项本身是对的**——恒等式在 routes 13 失配是因为它用 `cost(1)` 给一行尾定价，而 routes 2 比 routes 1 还便宜 24 µs（m1 与 m2 是不同内核）。前三轮之所以没看出来：每轮挑的 route 数混了奇偶（13/17 奇、20/36 偶），偶数点吻合被读成『恒等式基本对』，奇数点的偏差被归给 panel 结构——**周期为 2 的阶梯对不控制奇偶的采样不可见**。`2·ceil(q/2)` 目前只是候选，按 v14 的教训须在本轮未覆盖的 route 数上做预注册验证后才谈采纳；且仅 1 线程。报告：`results/c9g_window_table_20260922.md`、`results/row_pairs_c9g_20260923.md`。 |
| 2026-09-23 | v1.136 | **更正 v1.135 的行对采纳前提**：那里『模型按 `q` 计费、换成 `2·ceil(q/2)` 使误差 2.47%→0.12%』中的 `q` 基线是报告里写的插值代理，并非模型。C9g planner 使用的 `AnalyticMoeCostModel` 早已按行对计费——`kernel_panels` 对 exact-M 尾取 `compute_rows = 2·ceil(r/2)`，出厂标定给出 T(9)=T(10)、T(13)=T(14)、T(25)=T(26)；返回 `q` 的 `m12_effective_rows` 在解析模型中无调用方，只有表驱动 `ContentionCostModel` 按 `q` 计费，且仅在 profile 缺奇数尾桶时才有影响。**无行数改动可采纳。**阶梯本身成立：预注册 pairs 轮 1 线程 9/9（事后 2--16 线程同样成立；其宽度规则比较的是 cost(50)−cost(49) 这一被预测为零的量，即噪声，撤回）；预注册 triplet 轮（同一 M12 panel 内 q−1/q/q+1，12 个新奇数 q，1--16 线程，三遍）的冻结判决为**不可检验**——噪声门用了跨遍的格值极差，含整遍偏移（TP4 1T 两遍差 2.1%），而遍内差分会抵消它；事后改用遍内台阶及其跨遍离散，各宽度每个可检验三元组均 r≤0.25（中位 −0.013~+0.088）。同批数据暴露另一误差：出厂模型在 panel 内每多两行的增量是实测的 1.5--3 倍（TP4 1T 81 对 54 µs、TP2 2T 81 对 25 µs），属斜率误差而非行对，并入多 panel 偏差一并处理。报告：`results/row_pairs_c9g_20260923.md` 更正节。 |
| 2026-09-23 | v1.137 | **C9g packed-B 保留率实测，且不是多 panel 低估的解**。C9g 解析标定从未跑 `ANALYTIC_MODEL.md` 要求的 packed-B 重复扫描探针，沿用构建器默认值（有效比例 0.75、miss floor 0、2/4 MiB 处 miss 1，即 1.5 MiB 以下完全保留）。实测（CPU 0、5 个独立进程、M12/M120 差分 L2 refill）：floor 15.2%、拐点 0.63×L2、2 MiB 64.5%、4 MiB 85.4%，与 Arm-codex 2026-08-31 的 17.4%/0.662/68.0%/79.8% 相近。按收集前冻结的判据，用注册流程重建两份标定（不带保留率时逐字节复现出厂标定），在 434 个未参与训练的孤立点上评分：TP4 MAE 7.66%→7.84%、TP2 10.55%→12.58%（TP2 8T 8.8%→19.4%），≥8T 下 routes 13--30 的有符号偏置 −10.6/−17.6% 基本不变——**拒绝**，数值只记录不使用。它排除的是：宽宽度下多 panel 的低估（TP4 8T routes 13 −27.5%、16T −19.3%；反号点恰是 B 复用 miss 归零处）**不是** panel 间 B 掉出 L2——单核重读 1.1 MiB 窗口每次复用只 miss 约 1/5 的行，而实测的新增 panel 开销像是整份 B 按 LLC 速率。待归因的候选：薄 panel 仍按行数无关地发射整条 B stripe 的 load、TP4 16--48T 每新增 panel 约 25--30 µs 的宽度无关残差、以及 panel 内两行增量被高估 1.5--3 倍。归因先于任何模型改动，排在一键标定之后。报告：`results/c9g_b_retention_20260923.md`。 |
| 2026-09-23 | v1.138 | **一键标定达到研究标定精度（标定语义，公共 API 增量）**。旧 `calibrate_moe_planner_quick` 在 C9g 上三次空闲运行、434 个留出孤立点：TP4 平均 |误差| 28.7/20.7/31.4%、TP2 23.5/17.2/30.2%，研究标定为 7.66%/10.55%。两个原因：算子 overhead 恒为零（宽宽度低估最多，TP4 96T −75%），以及单次 5 秒服务探针不可复现（同一空闲机 1T 误差 −16/−17/+51%）。改动：(1) 服务探针默认跑三次、逐点取中位数（`service_repeats=3`，复用 `median_service_probe`；quick 版本号 2）；(2) 新增 `train_quick_operator_overheads`：按给定专家形状、以研究训练档相同口径（8 个 expert 背靠背、3+11 次）在 routes 1/4/12/48/192/2040 × 全部支持宽度上测孤立时间，先在 12/192/2040 上拟合公共 `(expert_fixed, route, stage_scale)`（`fit_operator_residuals`），再逐宽度拟合 `(expert_fixed, route)`（新 `fit_width_overheads`，即实验室 `fit_by_width` 移入 `build_analytic_calibration`，两形状逐项复现）；(3) `enable_moe_planner_quick(train_overheads=True)` 默认在机器标定后训练、两步都成功后才写出并安装，并在测量前拒绝已存在的输出。解析结构不变，只是把原本为零的 overhead 参数按形状标定；训练 routes 是研究训练 routes 的子集，留出集不受影响。验证（冻结目标：每次都在研究标定 2 个百分点内、三次互差 ≤3 个百分点）：TP4 7.85/7.83/7.71%、TP2 10.55/8.86/8.72%，通过；耗时 TP4 49 s（机器 15.5 + 训练 33.4）、TP2 70 s。限制：只验证孤立精度，未比较 v2 标定与研究标定在真实层上的计划选择；仅 C9g；训练结果绑定专家形状，并行策略改变形状时需重训（约 1 分钟）。报告：`results/c9g_one_click_calibration_20260923.md`。 |
| 2026-09-23 | v1.139 | **一键标定的计划选择与研究标定等价——旧一键标定也是**。18 个未用过的真实层/形状（请求 008/016/022 × 层 2/9/15/25/36/40），同一 `plan_quick`（宽度 4/8/16/32）、同一 C9g 注册窗口表，唯一差别是标定：research、v2（生成计划前选定的第一次端到端运行）、v1（旧的未训练单探针文件，对照）。36 层中 34 层宽度一致，但各变体的 LPT 分配都与 research 不同。窗口验证的 `bench.py` 原样、两 session，research 自身重复误差中位 0.12%/0.06%。v2 相对 research：TP4 中位 −0.04%（−0.25~+0.24%，10/18 更快）、TP2 −0.12%（−0.48~+0.17%，12/18），冻结判据（中位 ≤+0.5% 且无层慢 >2%）**等价**。对照 v1：TP4 +0.20%（−0.18~+0.99%，2/18 更快）、TP2 −0.02%——同样等价。**孤立误差 20--31% 的标定选出的计划与研究标定只差约 0.2%**：quick planner 只在四个同构宽度中选一个并按 LPT 分配，同向的宽度误差不改变排序。结论：此 planner 的计划选择对标定稳健，v2 的精度在这些层上只值 TP4 约 0.2%；在绝对时间主导决策处（full search、fast planner、跨 rank/共享 expert 平衡）精度应更重要，未测。报告：`results/c9g_one_click_calibration_20260923.md` 计划选择节。 |
