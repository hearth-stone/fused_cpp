...
#### 9.29 team 分割几何（`kM` / `kN`）与 $\omega$ 的适用域
...
由此有一个结构性缺口但生产不可达：tile 数少于线程数的 range 仍可沿 M 切以用满线程
（即 `kM`），但 `choose_moe_gemm_split` 只按 $(stage,M,t)$ 选轴、从不查窗口。由于表的
最小值是 1 tile/线程，$g=t$ 个 tile 恒可整除，该缺口触碰不到。
