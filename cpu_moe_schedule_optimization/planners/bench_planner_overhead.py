...
def measure_kernel(bridge, ids, wt):
    ...
    cpu = i32(bridge["thread_cpu_ids"]); nt = bridge["num_threads"]
    def run(): return fused_moe_bf16_tiled_async(x, packed, wt, ids, *args,
...
print("%-18s %6s %8s %9s %9s | %9s %9s" %
      ("config", "active", "cold_us", "warm_us", "kern_ms", "coop_ms", "shape"))
for name, tokens, E, hot in CONFIGS:
    ids, wt = routing(tokens, E, hot)
    counts = route_counts(ids, E)
    pm = PlannedMoE(model, 8)
    b_cold = pm.plan_for(counts); cold = pm.last["planner_overhead_ns"] / 1e3
    b_warm = pm.plan_for(counts); warm = pm.last["planner_overhead_ns"] / 1e3
    shape = pm.last["shape"]
    kern = measure_kernel(b_warm, ids, wt)
    coop = measure_kernel(pl.to_async_bridge(pl.score_shape(counts, (8,))[1]), ids, wt)
    print("%-18s %6d %8.1f %9.1f %9.3f | %9.3f  %s" %
          (name, len(counts), cold, warm, kern, coop, str(shape)))
...
for name, tokens, E, hot in [("decode-8e", 16, 8, 0), ("decode-64e", 64, 64, 0),
    ...
    print("%-16s hit_rate=%3.0f%%  mean_overhead=%7.1fus  cache_shape_matches_full=%3.0f%%" %
