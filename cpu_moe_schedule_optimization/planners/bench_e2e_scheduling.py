...
CFG = [("tk2 E8 uniform", 2048, 8, 2, 0), ("tk2 E8 hotspot", 2048, 8, 2, 2),
       ("tk6 E8 uniform", 2048, 8, 6, 0), ("tk6 E64 uniform", 2048, 64, 6, 0),
       ("tk6 E64 hotspot", 2048, 64, 6, 8)]
print("%-18s %6s %9s %8s %9s %8s   %s" %
      ("config", "close", "default", "coop_a", "planned", "plan_us", "shape"))
for name, tokens, E, tk, hot in CFG:
    ids, tw = routing(tokens, E, tk, hot); x = bf16(tokens, H)
    counts = route_counts(ids, E)
    pm.plan_for(counts); b = pm.plan_for(counts)  # warm
    ...
    def run_def(): return fused_moe_bf16_tiled(x, fw, tw, ids, num_threads=8, activation="silu", silu_poly_degree=5)
    def mk(br):
        a = (i32(br["task_expert_ids"]), i32(br["task_core_begins"]), i32(br["task_threads"]),
             i32(br["task_dep_offsets"]), i32(br["task_deps"]))
        return lambda: fused_moe_bf16_tiled_async(x, fw, tw, ids, *a,
            thread_cpu_ids=i32(br["thread_cpu_ids"]), num_threads=br["num_threads"], activation="silu")
    run_pl = mk(b); run_coop = mk(bc)
    close = torch.allclose(run_def().float(), run_pl().float(), atol=5e-2, rtol=5e-2)
    d = med(run_def); c = med(run_coop); p = med(run_pl)
    print("%-18s %6s %9.1f %8.1f %9.1f %8.1f   %s" %
          (name, str(bool(close)), d, c, p, plan_us, str(pm.last["shape"])))
