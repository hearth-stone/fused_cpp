...
def measure(config):  # config = [(routes, threads), ...]
    routes = [r for (r, _) in config]
    shape = [t for (_, t) in config]
    K = len(config)
    nthreads = sum(shape)
    begins, c = [], 0
    for t in shape:
...
for cfg in CONFIGS:
    meas = measure(cfg)
    ph = model.phase_makespan(cfg)
    sc = model.scalar_makespan(cfg)
    phe = (ph - meas) / meas * 100
    sce = (sc - meas) / meas * 100
    ph_errs.append(abs(phe))
    sc_errs.append(abs(sce))
    print("%-40s %9.3f %9.3f %9.3f %+8.1f %+8.1f" % (str(cfg), meas / 1e6, ph / 1e6, sc / 1e6, phe, sce))
