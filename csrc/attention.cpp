...
extern at::Tensor gather_kv_cache(at::Tensor kv_cache, at::Tensor block_table, int64_t seq_len, int64_t block_size);
...
py::object varlen_attention(at::Tensor q, at::Tensor k, at::Tensor v, at::Tensor cu_seqlens_q, at::Tensor cu_seqlens_k,
  ...
  // Input validation
  TORCH_CHECK(q.dim() == 3, "varlen_attention: q must be 3-D [total_tokens, num_heads, qk_head_dim], got ", q.dim(),
              "-D");
  TORCH_CHECK(k.dim() == 3, "varlen_attention: k must be 3-D [total_tokens, num_heads, qk_head_dim], got ", k.dim(),
              "-D");
  TORCH_CHECK(v.dim() == 3, "varlen_attention: v must be 3-D [total_tokens, num_heads, v_head_dim], got ", v.dim(),
              "-D");
  TORCH_CHECK(cu_seqlens_q.dim() == 1, "varlen_attention: cu_seqlens_q must be 1-D");
  TORCH_CHECK(cu_seqlens_k.dim() == 1, "varlen_attention: cu_seqlens_k must be 1-D");
  ...
  TORCH_CHECK(q.size(1) == k.size(1), "varlen_attention: q and k num_heads mismatch (", q.size(1), " vs ", k.size(1),
              ")");
  TORCH_CHECK(q.size(2) == k.size(2), "varlen_attention: q and k head_dim mismatch (", q.size(2), " vs ", k.size(2),
              ")");
  TORCH_CHECK(v.size(1) == k.size(1), "varlen_attention: v and k num_heads mismatch (", v.size(1), " vs ", k.size(1),
              ")");
  ...
  auto output = at::zeros({total_q_tokens, num_heads, v_head_dim}, q.options());

  // Optionally allocate LSE: [num_heads, total_q_tokens]
  at::Tensor lse;
  if (return_softmax_lse) {
    lse = at::full({num_heads, total_q_tokens}, -std::numeric_limits<float>::infinity(), q.options().dtype(at::kFloat));
  }
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    ...
    if (return_softmax_lse) {
      ...
      if (causal) {
        ...
        attn_scores =
            at::where(causal_mask, attn_scores, at::full_like(attn_scores, -std::numeric_limits<float>::infinity()));
      }
      ...
      output_i = at::scaled_dot_product_attention(q_i, k_i, v_i,
                                                  /*attn_mask=*/{},
                                                  /*dropout_p=*/0.0,
                                                  /*is_causal=*/causal,
                                                  /*scale=*/scale);
    ...
    output.slice(0, q_start, q_end) = output_i[0].slice(-1, 0, v_head_dim).transpose(0, 1);
  }
...
at::Tensor forward_decode(at::Tensor q_nope_proj, at::Tensor q_pe, at::Tensor kv_cache, at::Tensor block_table,
  ...
  // Input validation
  TORCH_CHECK(q_nope_proj.dim() == 3, "forward_decode: q_nope_proj must be 3-D [B, N, kv_lora_rank], got ",
              q_nope_proj.dim(), "-D");
  TORCH_CHECK(q_pe.dim() == 3, "forward_decode: q_pe must be 3-D [B, N, qk_rope_head_dim], got ", q_pe.dim(), "-D");
  TORCH_CHECK(kv_cache.dim() == 3, "forward_decode: kv_cache must be 3-D [num_blocks, block_size, head_size]");
  TORCH_CHECK(block_table.dim() == 2, "forward_decode: block_table must be 2-D [B, max_blocks]");
  TORCH_CHECK(seq_lens.dim() == 1, "forward_decode: seq_lens must be 1-D [B]");
  TORCH_CHECK(q_nope_proj.size(0) == q_pe.size(0), "forward_decode: q_nope_proj and q_pe batch size mismatch");
  TORCH_CHECK(q_nope_proj.size(1) == q_pe.size(1), "forward_decode: q_nope_proj and q_pe num_heads mismatch");
  ...
  auto output = at::zeros({batch_size, num_heads, kv_lora_rank}, q_nope_proj.options());
  ...
  for (int64_t b = 0; b < batch_size; ++b) {
    ...
    auto gathered_kv = gather_kv_cache(kv_cache, block_table[b], seq_len, block_size);
    ...
    auto attn_scores = (at::matmul(q_nope_proj[b], kv_c.t()) + at::matmul(q_pe[b], k_pe_seq.t())) * scale;
