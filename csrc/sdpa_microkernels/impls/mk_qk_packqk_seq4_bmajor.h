#pragma once
// ── 微内核 impl：MK_QkPackqkSeq4Bmajor ──────────────────────────────────────────
//
...
// `vld1q_u16`（各 4 条独立 load），验证 seq4 的 bmajor 调度变体。
//
// 与 packk_seq 的关键差别：
...
// through 到 baseline。
//
// 编译期开关：FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ4_BMAJOR，默认 1。
...
#ifndef FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ4_BMAJOR
#define FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ4_BMAJOR 1
#endif

namespace fused_cpp::sdpa_microkernels {

#if FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ4_BMAJOR
struct MK_QkPackqkSeq4Bmajor {
  static constexpr const char* kName = "qk_packqk_seq4_bmajor";
  ...
  static inline void qkt_8x8(const at::BFloat16* Q, int64_t q_row_stride, const at::BFloat16* K, int64_t k_row_stride,
    ...
    gemm_qkt_microkernel_8x8_bf16_packqk_seq4_bmajor_inner(q_packed_buf.data(), Q, q_row_stride, k_packed_buf.data(), K,
                                                           k_row_stride, E, scale, scores_buf);
#else
...
#endif  // FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ4_BMAJOR
