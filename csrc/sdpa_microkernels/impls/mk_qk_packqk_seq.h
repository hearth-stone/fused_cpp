...
// `vld1q_u16_x4`，是「Q + K 双侧都已是 BFMMLA-friendly 连续布局」时的
// 理论上限实现。
//
// 与 packk_seq 的关键差别：
...
//     vcombine → 1 vld1q_u16_x4 + 4 vreinterpret，cacheline 利用率
...
// through 到 baseline。
//
// 编译期开关：FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ，默认 1。
...
#ifndef FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ
#define FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ 1
#endif

namespace fused_cpp::sdpa_microkernels {

#if FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ
struct MK_QkPackqkSeq {
  static constexpr const char* kName = "qk_packqk_seq";
  ...
  static inline void qkt_8x8(const at::BFloat16* Q, int64_t q_row_stride, const at::BFloat16* K, int64_t k_row_stride,
    ...
    gemm_qkt_microkernel_8x8_bf16_packqk_seq_inner(q_packed_buf.data(), Q, q_row_stride, k_packed_buf.data(), K,
...
#endif  // FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ
