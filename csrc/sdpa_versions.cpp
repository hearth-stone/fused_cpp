...
at::Tensor scaled_dot_product_attention_versioned(
    ...
    TORCH_CHECK(query.dim() == 4,
                "sdpa_versioned: query must be 4-D [B, N, L, E], got ",
                query.dim(), "-D");
    TORCH_CHECK(key.dim() == 4,
                "sdpa_versioned: key must be 4-D [B, N, S, E], got ",
                key.dim(), "-D");
    TORCH_CHECK(value.dim() == 4,
                "sdpa_versioned: value must be 4-D [B, N, S, Ev], got ",
                value.dim(), "-D");
    TORCH_CHECK(query.size(0) == key.size(0) && query.size(0) == value.size(0),
                "sdpa_versioned: batch size mismatch among Q/K/V (",
                query.size(0), ", ", key.size(0), ", ", value.size(0), ")");
    TORCH_CHECK(query.size(1) == key.size(1) && query.size(1) == value.size(1),
                "sdpa_versioned: num_heads mismatch among Q/K/V (",
                query.size(1), ", ", key.size(1), ", ", value.size(1), ")");
    TORCH_CHECK(key.size(2) == value.size(2),
                "sdpa_versioned: K and V seq_len mismatch (",
                key.size(2), " vs ", value.size(2), ")");
    TORCH_CHECK(query.size(3) == key.size(3),
                "sdpa_versioned: Q and K head_dim mismatch (",
                query.size(3), " vs ", key.size(3), ")");
    TORCH_CHECK(!enable_gqa,
                "sdpa_versioned: enable_gqa=true is not supported");

    if (dropout_p != 0.0) {
        TORCH_WARN("sdpa_versioned: dropout_p=", dropout_p,
    ...
    if (kernel_fn == nullptr) {
        ...
        TORCH_CHECK(false,
                    "sdpa_versioned: version='", version,
                    "' is not registered; available versions: [",
                    available, "]");
    }
    ...
    double scale_val = scale.has_value()
        ? scale.value()
        : 1.0 / std::sqrt(static_cast<double>(E));
