...
void check_shape(int64_t B,
    ...
    if (B <= 0 || N <= 0 || L <= 0 || S <= 0 || E <= 0 || Ev <= 0) {
        throw std::invalid_argument(
            "sdpa_c_api: B/N/L/S/E/Ev must all be positive");
    }
}

void check_ptr(const void* ptr, const char* name) {
    if (ptr == nullptr) {
        throw std::invalid_argument(std::string("sdpa_c_api: ") + name +
                                    " must be non-null");
    }
}
...
bool is_contiguous_bntd(const fused_cpp_sdpa_strides& s,
                        int64_t N,
                        int64_t T,
                        int64_t D) {
...
const Scalar* materialize_bntd(const void* src_void,
                               const fused_cpp_sdpa_strides& strides,
                               int64_t B,
                               int64_t N,
                               int64_t Tdim,
                               int64_t D,
        ...
        for (int64_t n = 0; n < N; ++n) {
            for (int64_t t = 0; t < Tdim; ++t) {
                const int64_t src_base =
                    b * strides.b + n * strides.n + t * strides.t;
                Scalar* dst_row = dst + (((b * N + n) * Tdim + t) * D);
                if (strides.d == 1) {
                    std::memcpy(dst_row, src + src_base,
                                static_cast<size_t>(D) * sizeof(Scalar));
                } else {
...
void scatter_bntd_float(const float* src,
                        float* dst,
                        const fused_cpp_sdpa_strides& strides,
                        int64_t B,
                        int64_t N,
    ...
    if (is_contiguous_bntd(strides, N, T, D)) {
        if (dst != src) {
            std::memcpy(dst, src, static_cast<size_t>(B * N * T * D) *
                                      sizeof(float));
        }
        return;
    }
        ...
        for (int64_t n = 0; n < N; ++n) {
            for (int64_t t = 0; t < T; ++t) {
                ...
                const int64_t dst_base =
                    b * strides.b + n * strides.n + t * strides.t;
                if (strides.d == 1) {
                    std::memcpy(dst + dst_base, src_row,
...
int forward_strided_impl(const char* version,
                         int dtype,
                         const void* q,
                         const void* k,
                         const void* v,
    ...
    if (sdpa_find_kernel(version_name) == nullptr) {
        std::string msg = "sdpa_c_api: unknown SDPA version '" + version_name +
                          "'; available: [";
    ...
    if (sdpa_dtype == SdpaDtype::kFloat32) {
        ...
        p.q_ptr = materialize_bntd<uint16_t>(q, q_strides, B, N, L, E,
                                             q_storage);
        p.k_ptr = materialize_bntd<uint16_t>(k, k_strides, B, N, S, E,
                                             k_storage);
        p.v_ptr = materialize_bntd<uint16_t>(v, v_strides, B, N, S, Ev,
                                             v_storage);
        sdpa_dispatch(version_name, p);
    }
...
const char* fused_cpp_sdpa_last_error(void) {
...
int fused_cpp_sdpa_forward_strided(
    const char* version,
    int dtype,
    const void* q,
    const void* k,
    ...
    float scale) {
    try {
        return forward_strided_impl(version, dtype, q, k, v, attn_mask, out,
                                    B, N, L, S, E, Ev, q_strides, k_strides,
...
int fused_cpp_sdpa_forward_ggml_f32(const char* version,
                                    const float* q,
                                    const float* k,
                                    const float* v,
                                    float* out,
    ...
    return fused_cpp_sdpa_forward_strided(
        version, FUSED_CPP_SDPA_DTYPE_F32, q, k, v, nullptr, out, B, N, L, S,
        E, Ev, q_strides, k_strides, v_strides, out_strides, is_causal, scale);
}
