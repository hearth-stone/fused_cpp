...
void CheckIntTensor(const at::Tensor& tensor, const char* name) {
  ...
  TORCH_CHECK(tensor.scalar_type() == at::kInt || tensor.scalar_type() == at::kLong, name,
              " must be torch.int32 or torch.int64, got ", tensor.scalar_type());
}
...
int64_t ReadIndex2d(const at::Tensor& tensor, int64_t i, int64_t j) {
  if (tensor.scalar_type() == at::kInt) {
    return static_cast<int64_t>(tensor.data_ptr<int32_t>()[i * tensor.stride(0) + j * tensor.stride(1)]);
  }
...
inline void CopyBf16Row(const c10::BFloat16* src, int64_t src_stride, c10::BFloat16* dst, int64_t dst_stride,
                        int64_t head_dim) {
  if (head_dim <= 0) {
    return;
  }
...
#if defined(__ARM_FEATURE_SVE)
inline void CopyAddI32ContiguousSve(const int32_t* src, int32_t* dst, int64_t len, int32_t addend) {
...
inline void StoreI32ArangeContiguousSve(int32_t* dst, int64_t len, int32_t base) {
  const int64_t vl = static_cast<int64_t>(svcntw());
...
inline void CopyGatherRow(const GatherRegion& region, int64_t row_idx) {
  ...
  const int64_t physical_block = ReadIndex2d(*region.block_table, region.batch_idx, block_in_seq);
  TORCH_CHECK(physical_block >= 0 && physical_block < region.k_size0, "block_table[", region.batch_idx, ", ",
              block_in_seq, "] out of range: ", physical_block, " for k_cache.size(0)=", region.k_size0);

  const auto* src = region.k_ptr + physical_block * region.k_s0 + pos_in_block * region.k_s1;
  auto* dst = region.out_ptr + region.batch_idx * region.out_s0 + (region.offset + row_idx) * region.out_s1;
  CopyBf16Row(src, region.k_s2, dst, region.out_s2, region.head_dim);
}

inline void WriteCombinedIndexRow(const int32_t* topk_row, int64_t topk_s1, int32_t* row, int64_t out_s1,
                                  int64_t topk_len, int64_t swa_len, int32_t topk_addend, int32_t swa_base) {
  if (topk_len > 0) {
...
void RunGatherRegions(const std::vector<GatherRegion>& regions) {
  ...
  at::parallel_for(0, total_rows, 16, [&](int64_t begin, int64_t end) {
    size_t region_idx = static_cast<size_t>(std::upper_bound(starts.begin(), starts.end(), begin) - starts.begin() - 1);
    ...
    while (cursor < end) {
      ...
      for (int64_t row = cursor - starts[region_idx]; row < region_end - starts[region_idx]; ++row) {
...
void AppendGatherRegions(std::vector<GatherRegion>& regions, const at::Tensor& out, const at::Tensor& k_cache,
                         const at::Tensor& seq_lens, const c10::optional<at::Tensor>& gather_lens,
                         const at::Tensor& block_table, int64_t block_size, int64_t offset, const char* op_name) {
  ...
  TORCH_CHECK(k_cache.dim() == 3, "k_cache must be 3-D [num_blocks, block_size, head_dim], got ", k_cache.dim(), "-D");
  ...
  TORCH_CHECK(out.scalar_type() == at::kBFloat16, "out must be torch.bfloat16, got ", out.scalar_type());
  TORCH_CHECK(k_cache.scalar_type() == at::kBFloat16, "k_cache must be torch.bfloat16, got ", k_cache.scalar_type());
  ...
  TORCH_CHECK(k_cache.size(1) == block_size, "block_size mismatch: argument block_size=", block_size,
              " k_cache.size(1)=", k_cache.size(1));
  TORCH_CHECK(out.size(2) == k_cache.size(2), "head_dim mismatch: out=", out.size(2), " k_cache=", k_cache.size(2));

  const int64_t chunk_size = seq_lens.size(0);
  TORCH_CHECK(out.size(0) >= chunk_size, "out first dimension must cover seq_lens: out.size(0)=", out.size(0),
              " seq_lens=", chunk_size);
  TORCH_CHECK(block_table.size(0) >= chunk_size,
              "block_table first dimension must cover seq_lens: block_table.size(0)=", block_table.size(0),
              " seq_lens=", chunk_size);
  ...
  if (gather_lens.has_value()) {
    ...
    TORCH_CHECK(gather_lens_tensor.dim() == 1, "gather_lens must be 1-D, got ", gather_lens_tensor.dim(), "-D");
    TORCH_CHECK(gather_lens_tensor.size(0) >= chunk_size,
                "gather_lens must cover seq_lens: gather_lens.size(0)=", gather_lens_tensor.size(0),
                " seq_lens=", chunk_size);
  }
  ...
  for (int64_t batch_idx = 0; batch_idx < chunk_size; ++batch_idx) {
    ...
    const int64_t gather_len = gather_lens.has_value() ? ReadIndex1d(gather_lens_tensor, batch_idx) : seq_len;
    TORCH_CHECK(gather_len >= 0, "gather_lens[", batch_idx, "] must be non-negative, got ", gather_len);
    TORCH_CHECK(gather_len <= seq_len, "gather_lens[", batch_idx, "]=", gather_len, " exceeds seq_lens[", batch_idx,
                "]=", seq_len);
    if (gather_len == 0) {
      continue;
    }
    TORCH_CHECK(offset + gather_len <= out.size(1), "out M dimension is too small: offset=", offset,
                " gather_len=", gather_len, " out.size(1)=", out.size(1));

    const int64_t needed_blocks = (seq_len + block_size - 1) / block_size;
    TORCH_CHECK(needed_blocks <= block_table.size(1), "block_table row is too short: need ", needed_blocks,
...
void GatherKCacheImpl(const at::Tensor& out, const at::Tensor& k_cache, const at::Tensor& seq_lens,
                      const c10::optional<at::Tensor>& gather_lens, const at::Tensor& block_table, int64_t block_size,
                      int64_t offset, const char* op_name) {
  std::vector<GatherRegion> regions;
  AppendGatherRegions(regions, out, k_cache, seq_lens, gather_lens, block_table, block_size, offset, op_name);
...
void deepseek_v4_dequantize_and_gather_dual_k_cache(at::Tensor out, at::Tensor compressed_k_cache,
  ...
  std::vector<GatherRegion> regions;
  if (has_compressed) {
    AppendGatherRegions(regions, out, compressed_k_cache, compressed_seq_lens, c10::nullopt, compressed_block_table,
                        compressed_block_size, compressed_offset,
  ...
  AppendGatherRegions(regions, out, swa_k_cache, swa_seq_lens, swa_gather_lens_opt, swa_block_table, swa_block_size,
                      swa_offset, "deepseek_v4_dequantize_and_gather_dual_k_cache swa");
  RunGatherRegions(regions);
}
...
std::tuple<at::Tensor, at::Tensor> deepseek_v4_combine_topk_swa_indices(at::Tensor topk_indices,
  ...
  TORCH_CHECK(topk_indices.dim() == 2, "topk_indices must be 2-D [num_tokens, K], got ", topk_indices.dim(), "-D");
  TORCH_CHECK(topk_indices.scalar_type() == at::kInt, "topk_indices must be torch.int32, got ",
              topk_indices.scalar_type());
  TORCH_CHECK(query_start_loc.dim() == 1, "query_start_loc must be 1-D, got ", query_start_loc.dim(), "-D");
  TORCH_CHECK(seq_lens.dim() == 1, "seq_lens must be 1-D, got ", seq_lens.dim(), "-D");
  TORCH_CHECK(gather_lens.dim() == 1, "gather_lens must be 1-D, got ", gather_lens.dim(), "-D");
  ...
  TORCH_CHECK(gather_lens.size(0) >= num_reqs,
              "gather_lens must cover seq_lens: gather_lens.size(0)=", gather_lens.size(0), " seq_lens=", num_reqs);
  TORCH_CHECK(query_start_loc.size(0) >= num_reqs + 1,
              "query_start_loc must have num_reqs + 1 entries: query_start_loc.size(0)=", query_start_loc.size(0),
              " num_reqs=", num_reqs);
  TORCH_CHECK(topk <= topk_width, "topk=", topk, " exceeds topk_indices width=", topk_width);

  const int64_t combined_topk = AlignUp(topk + window_size, kSparsePrefillTopKAlignment);
  at::Tensor combined_indices = at::full({num_tokens, combined_topk}, -1, topk_indices.options().dtype(at::kInt));
  at::Tensor combined_lens = at::zeros({num_tokens}, topk_indices.options().dtype(at::kInt));
  if (num_tokens == 0) {
  ...
  for (int64_t batch_idx = 0; batch_idx < num_reqs; ++batch_idx) {
    ...
    TORCH_CHECK(query_start >= 0 && query_start <= query_end && query_end <= num_tokens,
                "invalid chunk-local query range for batch ", batch_idx, ": [", query_start, ", ", query_end,
    ...
    TORCH_CHECK(seq_len >= 0, "seq_lens[", batch_idx, "] must be non-negative, got ", seq_len);
    TORCH_CHECK(gather_len >= 0, "gather_lens[", batch_idx, "] must be non-negative, got ", gather_len);
    TORCH_CHECK(query_len <= seq_len, "query_len=", query_len, " exceeds seq_lens[", batch_idx, "]=", seq_len);
    TORCH_CHECK(gather_len <= seq_len, "gather_lens[", batch_idx, "]=", gather_len, " exceeds seq_lens[", batch_idx,
                "]=", seq_len);
  ...
  at::parallel_for(0, total_query_tokens, 64, [&](int64_t begin, int64_t end) {
    ...
    while (cursor < end) {
      ...
      for (int64_t local = cursor - starts[region_idx]; local < region_end - starts[region_idx]; ++local) {
        const int64_t token_idx = region.query_start + local;
        const int64_t pos = region.start_pos + local;
        const int64_t topk_len = compress_ratio > 0 ? std::min((pos + 1) / compress_ratio, topk) : 0;
        ...
        if (combined_len > 0) {
          ...
          const auto* topk_row = topk_len > 0 ? topk_ptr + token_idx * topk_s0 : topk_ptr;
          const int32_t swa_base =
              static_cast<int32_t>(M * region.batch_idx + N + pos - swa_len + 1 - region.gather_start);

          WriteCombinedIndexRow(topk_row, topk_s1, row, out_s1, topk_len, swa_len, topk_addend, swa_base);
        }
