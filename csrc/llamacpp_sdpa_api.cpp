...
const float* ptr_at(const float* base, int64_t nb0, int64_t nb1, int64_t nb2, int64_t nb3, int64_t i0, int64_t i1,
                    int64_t i2, int64_t i3) {
  return reinterpret_cast<const float*>(reinterpret_cast<const char*>(base) + i0 * nb0 + i1 * nb1 + i2 * nb2 +
                                        i3 * nb3);
}

float* ptr_at(float* base, int64_t nb0, int64_t nb1, int64_t nb2, int64_t nb3, int64_t i0, int64_t i1, int64_t i2,
              int64_t i3) {
  return reinterpret_cast<float*>(reinterpret_cast<char*>(base) + i0 * nb0 + i1 * nb1 + i2 * nb2 + i3 * nb3);
}
...
extern "C" FUSED_CPP_LLAMA_API int fused_cpp_sdpa_flash2_neon_l3kv_packqkv_pbf16pv_fp32_llamacpp(
  ...
  try {
    ...
    const float effective_scale = scale > 0.0f ? scale : 1.0f / std::sqrt(static_cast<float>(D));
