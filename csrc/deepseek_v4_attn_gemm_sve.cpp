...
inline int round_up(int value, int q) {
    return ((value + q - 1) / q) * q;
}
...
inline void pack_a_block_sve(const uint16_t* A,
                             uint16_t* packed,
                             int rows,
                             int K,
                             int lda) {
    for (int kb = 0; kb < K; kb += 4) {
        for (int rp = 0; rp < 4; ++rp) {
            ...
            const int r1 = r0 + 1;
            uint16_t* dst =
                packed + static_cast<int64_t>(kb / 4) * 32 + rp * 8;
            for (int k = 0; k < 4; ++k) {
                dst[k] = r0 < rows
                             ? A[static_cast<int64_t>(r0) * lda + kb + k]
                             : static_cast<uint16_t>(0);
                dst[4 + k] = r1 < rows
                                 ? A[static_cast<int64_t>(r1) * lda + kb + k]
                                 : static_cast<uint16_t>(0);
            }
        }
    }
...
inline void store_rowpair_f32(float* C,
                              int ldc,
                              int rows,
                              int n_tile_base,
                              int rp,
...
inline void store_rowpair_bf16(uint16_t* C,
                               int ldc,
                               int rows,
                               int n_tile_base,
                               int rp,
    ...
    for (int cp = 0; cp < 4; ++cp) {
        ...
        for (int lane = 0; lane < lanes; ++lane) {
            ...
            C[static_cast<int64_t>(row) * ldc + col] =
                bf16_bits_from_float(tmp[lane]);
        }
    }
}

template <typename StoreFn, typename CPtr>
void gemm_packed_block(const uint16_t* packed_A,
...
bool enabled_by_env() {
    return available() && env_true("FUSED_CPP_ATTN_GEMM_SVE");
}
...
void dispatch_f32(const uint16_t* A,
                  const uint16_t* B_reo,
                  float* C,
                  uint16_t* A_reorder,
                  int M,
...
void dispatch_bf16(const uint16_t* A,
                   const uint16_t* B_reo,
                   uint16_t* C,
                   uint16_t* A_reorder,
                   int M,
