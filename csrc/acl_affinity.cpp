...
void set_acl_thread_affinity(int64_t core_start, int64_t core_end,
    ...
    if (ret != 0) {
        fprintf(stderr,
                "WARNING: pthread_setaffinity_np(cores %ld-%ld) 失败: %s\n",
                static_cast<long>(core_start),
                static_cast<long>(core_end - 1),
                strerror(ret));
        return;
    }
...
std::tuple<int64_t, int64_t, int64_t> get_acl_thread_affinity() {
    ...
    return std::make_tuple(
        static_cast<int64_t>(g_core_start),
        static_cast<int64_t>(g_core_end),
