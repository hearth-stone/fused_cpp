...
inline void store_w13_rowpair_rowmajor(uint16_t* C, int ldc, int rows, int n_tile_base, int rp, svfloat32_t g0,
  ...
  for (int lane = 0; lane < lanes; ++lane) {
    ...
    C[static_cast<int64_t>(row) * ldc + f0] = bf16_bits_from_float(out0[lane]);
    C[static_cast<int64_t>(row) * ldc + f1] = bf16_bits_from_float(out1[lane]);
  }
}

inline void store_w13_rowpair_packc(uint16_t* C, int rows, int n_tile_base, int rp, svfloat32_t g0, svfloat32_t g1,
                                    svfloat32_t u0, svfloat32_t u1, int64_t degree) {
  constexpr int kMaxLanes = 64;
  alignas(64) float out0[kMaxLanes];
  alignas(64) float out1[kMaxLanes];
  const int lanes = lane_count();
...
inline void store_w2_rowpair(float* C, int ldc, int rows, int n_tile_base, int rp, svfloat32_t c0, svfloat32_t c1,
...
void gemm_packed_block(const uint16_t* packed_A, const uint16_t* B_reo, void* C, int rows, int K, int N, int ldc,
  ...
  for (int nb = 0; nb < N; nb += nt) {
    ...
    if (kind == StoreKind::kW13Rowmajor) {
      ...
      store_w13_rowpair_rowmajor(out, ldc, rows, nb, 0, c00, c01, c02, c03, degree);
      store_w13_rowpair_rowmajor(out, ldc, rows, nb, 1, c10, c11, c12, c13, degree);
      store_w13_rowpair_rowmajor(out, ldc, rows, nb, 2, c20, c21, c22, c23, degree);
      store_w13_rowpair_rowmajor(out, ldc, rows, nb, 3, c30, c31, c32, c33, degree);
    } else if (kind == StoreKind::kW13PackC) {
      uint16_t* out = static_cast<uint16_t*>(C);
      store_w13_rowpair_packc(out, rows, nb, 0, c00, c01, c02, c03, degree);
      store_w13_rowpair_packc(out, rows, nb, 1, c10, c11, c12, c13, degree);
      store_w13_rowpair_packc(out, rows, nb, 2, c20, c21, c22, c23, degree);
      store_w13_rowpair_packc(out, rows, nb, 3, c30, c31, c32, c33, degree);
    } else {
    ...
    for (int kb = 0; kb < K_pad; kb += 4) {
      for (int rp = 0; rp < 4; ++rp) {
        for (int row_bit = 0; row_bit < 2; ++row_bit) {
          ...
          const int gr = mb * 8 + r;
          uint16_t* dst = block + static_cast<int64_t>(kb / 4) * 32 + rp * 8 + row_bit * 4;
          ...
          for (int k = 0; k < 4; ++k) {
            const int col = kb + k;
            dst[k] = (src != nullptr && col < H) ? src[col] : static_cast<uint16_t>(0);
          }
        }
      }
    }
...
void w13_silu_rowmajor(const uint16_t* A, const uint16_t* B_reo, uint16_t* C, uint16_t* A_reorder,
  ...
  for (int mb = 0; mb < M; mb += 8) {
    ...
    pack_a_block_sve(A + static_cast<int64_t>(mb) * params->lda, packed, rows, K, params->lda);
    gemm_packed_block(packed, B_reo, C + static_cast<int64_t>(mb) * ldc, rows, K, N, ldc, StoreKind::kW13Rowmajor,
                      degree);
  }
#else
  (void)A;
...
void w13_silu_packed(const uint16_t* packed_A, const uint16_t* B_reo, uint16_t* C, const gemm_params_t* params,
  ...
  for (int mb = 0; mb < M; mb += 8) {
    ...
    gemm_packed_block(block, B_reo, C + static_cast<int64_t>(mb) * ldc, rows, K, N, ldc, StoreKind::kW13Rowmajor,
                      degree);
  }
#else
  (void)packed_A;
...
void w13_silu_packc(const uint16_t* packed_A, const uint16_t* B_reo, uint16_t* C, const gemm_params_t* params,
  ...
  for (int mb = 0; mb < rows_total; mb += 8) {
    ...
    gemm_packed_block(block, B_reo, Cb, rows, K, N, ldc, StoreKind::kW13PackC, degree);
  }
#else
  (void)packed_A;
  (void)B_reo;
...
void w2_packed(const uint16_t* packed_A, const uint16_t* B_reo, float* C, const gemm_params_t* params,
  ...
  for (int mb = 0; mb < rows_total; mb += 8) {
    ...
    gemm_packed_block(block, B_reo, Cb, rows, K, N, ldc, StoreKind::kW2Rowmajor, 0);
  }
#else
  (void)packed_A;
  (void)B_reo;
...
void w2_rowmajor(const uint16_t* A, const uint16_t* B_reo, float* C, uint16_t* A_reorder, const gemm_params_t* params) {
  ...
  for (int mb = 0; mb < M; mb += 8) {
    ...
    uint16_t* packed = A_reorder + static_cast<int64_t>(mb) * K;
    pack_a_block_sve(A + static_cast<int64_t>(mb) * params->lda, packed, rows, K, params->lda);
    gemm_packed_block(packed, B_reo, C + static_cast<int64_t>(mb) * ldc, rows, K, N, ldc, StoreKind::kW2Rowmajor, 0);
  }
#else
  (void)A;
  (void)B_reo;
