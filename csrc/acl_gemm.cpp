...
static DataType torch_dtype_to_acl(at::ScalarType dtype) {
  switch (dtype) {
    ...
    default:
      throw std::runtime_error("ACL GEMM: 不支持的数据类型，仅支持 float32 和 bfloat16");
  }
}
...
int64_t create_acl_gemm_handler(at::Tensor weight, int64_t num_threads, bool fast_math) {
  TORCH_CHECK(weight.dim() == 2, "ACL GEMM: 权重张量必须为 2D，当前维度: ", weight.dim());
  ...
  handler->weight_tensor.allocator()->init(
      TensorInfo(TensorShape(static_cast<unsigned int>(N), static_cast<unsigned int>(K)), 1, acl_dtype));

  // 使用 M=1 作为初始占位来 configure NEGEMM
  // 后续 run 时如果 M 变化会 reconfigure
  handler->input_tensor.allocator()->init(TensorInfo(TensorShape(static_cast<unsigned int>(K), 1u), 1, acl_dtype));
  handler->output_tensor.allocator()->init(TensorInfo(TensorShape(static_cast<unsigned int>(N), 1u), 1, acl_dtype));

  GEMMInfo gemm_info;
  gemm_info.set_fast_math(fast_math);

  // 先 validate，避免不支持的配置导致 segfault
  auto status = NEGEMM::validate(handler->input_tensor.info(), handler->weight_tensor.info(), nullptr,
                                 handler->output_tensor.info(), 1.0f, 0.0f, gemm_info);
  if (status.error_code() != ErrorCode::OK) {
    delete handler;
    TORCH_CHECK(false, "ACL GEMM: 当前平台不支持该配置 (dtype=", weight.scalar_type(), ", K=", K, ", N=", N,
                "): ", status.error_description());
  }

  handler->gemm.configure(&handler->input_tensor, &handler->weight_tensor, nullptr, &handler->output_tensor, 1.0f, 0.0f,
                          gemm_info);
...
void acl_gemm(at::Tensor output, at::Tensor input, c10::optional<at::Tensor> bias, int64_t handler_ptr) {
  ...
  TORCH_CHECK(K == handler->K, "ACL GEMM: 输入 K 维度不匹配，期望 ", handler->K, "，实际 ", K);
  ...
  if (M != handler->cached_M) {
    ...
    handler->input_tensor.allocator()->init(
        TensorInfo(TensorShape(static_cast<unsigned int>(K), static_cast<unsigned int>(M)), 1, handler->acl_dtype));
    handler->output_tensor.allocator()->init(
        TensorInfo(TensorShape(static_cast<unsigned int>(N), static_cast<unsigned int>(M)), 1, handler->acl_dtype));

    GEMMInfo gemm_info;
    gemm_info.set_fast_math(handler->fast_math);

    handler->gemm.configure(&handler->input_tensor, &handler->weight_tensor, nullptr, &handler->output_tensor, 1.0f,
                            0.0f, gemm_info);
...
int64_t create_acl_gemm_handler(at::Tensor weight, int64_t num_threads, bool fast_math) {
  throw std::runtime_error("ACL GEMM: 仅在 AArch64 平台上可用");
}
...
void release_acl_gemm_handler(int64_t handler_ptr) { throw std::runtime_error("ACL GEMM: 仅在 AArch64 平台上可用"); }
