...
// Compute two independent 8-head query tokens against one half of the packed
// 2VL K tile. M=16 and N=VL keep the same 16-accumulator budget as 8x2VL while
// reusing each B vector across both tokens. column_pair_base selects pairs 0/1
// or 2/3 of the existing packed layout and preserves the original score order.
__attribute__((noinline)) inline void qkt_16xvl_bf16(
    const uint16_t* packed_q0, const uint16_t* packed_q1,
    const uint16_t* packed_k, int64_t reduction, int column_pair_base,
    float scale, float* scores0, float* scores1) {
  svfloat32_t c00 = svdup_f32(0.0f), c01 = svdup_f32(0.0f);
  svfloat32_t c10 = svdup_f32(0.0f), c11 = svdup_f32(0.0f);
  svfloat32_t c20 = svdup_f32(0.0f), c21 = svdup_f32(0.0f);
  svfloat32_t c30 = svdup_f32(0.0f), c31 = svdup_f32(0.0f);
  svfloat32_t c40 = svdup_f32(0.0f), c41 = svdup_f32(0.0f);
  svfloat32_t c50 = svdup_f32(0.0f), c51 = svdup_f32(0.0f);
  svfloat32_t c60 = svdup_f32(0.0f), c61 = svdup_f32(0.0f);
  svfloat32_t c70 = svdup_f32(0.0f), c71 = svdup_f32(0.0f);

  const svbool_t pg_bf16 = svptrue_b16();
  const int64_t lanes_h = static_cast<int64_t>(svcnth());
  const uint16_t* q0_ptr = packed_q0;
  const uint16_t* q1_ptr = packed_q1;
  const uint16_t* k_ptr = packed_k + column_pair_base * lanes_h;
  for (int64_t reduction_base = 0; reduction_base < reduction;
       reduction_base += 4) {
    const svbfloat16_t b0 = load_bf16(k_ptr + 0 * lanes_h);
    const svbfloat16_t b1 = load_bf16(k_ptr + 1 * lanes_h);
    ...
    const svbfloat16_t a0 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q0_ptr + 0));
    const svbfloat16_t a1 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q0_ptr + 8));
    const svbfloat16_t a2 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q0_ptr + 16));
    const svbfloat16_t a3 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q0_ptr + 24));
    const svbfloat16_t a4 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q1_ptr + 0));
    const svbfloat16_t a5 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q1_ptr + 8));
    const svbfloat16_t a6 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q1_ptr + 16));
    const svbfloat16_t a7 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(q1_ptr + 24));
    q0_ptr += 32;
    q1_ptr += 32;
    ...
    c30 = svbfmmla_f32(c30, a3, b0);
    c31 = svbfmmla_f32(c31, a3, b1);
    c40 = svbfmmla_f32(c40, a4, b0);
    c41 = svbfmmla_f32(c41, a4, b1);
    c50 = svbfmmla_f32(c50, a5, b0);
    c51 = svbfmmla_f32(c51, a5, b1);
    c60 = svbfmmla_f32(c60, a6, b0);
    c61 = svbfmmla_f32(c61, a6, b1);
    c70 = svbfmmla_f32(c70, a7, b0);
    c71 = svbfmmla_f32(c71, a7, b1);
  }

  const int64_t tile = n_tile();
  store_qkt_row_pair(scores0, tile, 0, column_pair_base + 0, scale, c00);
  store_qkt_row_pair(scores0, tile, 0, column_pair_base + 1, scale, c01);
  store_qkt_row_pair(scores0, tile, 1, column_pair_base + 0, scale, c10);
  store_qkt_row_pair(scores0, tile, 1, column_pair_base + 1, scale, c11);
  store_qkt_row_pair(scores0, tile, 2, column_pair_base + 0, scale, c20);
  store_qkt_row_pair(scores0, tile, 2, column_pair_base + 1, scale, c21);
  store_qkt_row_pair(scores0, tile, 3, column_pair_base + 0, scale, c30);
  store_qkt_row_pair(scores0, tile, 3, column_pair_base + 1, scale, c31);
  store_qkt_row_pair(scores1, tile, 0, column_pair_base + 0, scale, c40);
  store_qkt_row_pair(scores1, tile, 0, column_pair_base + 1, scale, c41);
  store_qkt_row_pair(scores1, tile, 1, column_pair_base + 0, scale, c50);
  store_qkt_row_pair(scores1, tile, 1, column_pair_base + 1, scale, c51);
  store_qkt_row_pair(scores1, tile, 2, column_pair_base + 0, scale, c60);
  store_qkt_row_pair(scores1, tile, 2, column_pair_base + 1, scale, c61);
  store_qkt_row_pair(scores1, tile, 3, column_pair_base + 0, scale, c70);
  store_qkt_row_pair(scores1, tile, 3, column_pair_base + 1, scale, c71);
}
...
// Two-token PV companion for qkt_16xvl_bf16. The existing packed P buffers
// remain independent, while each half-tile V vector is reused across 16 rows.
__attribute__((noinline)) inline void pv_16xvl_bf16(
    const uint16_t* packed_p0, const uint16_t* packed_p1,
    const uint16_t* packed_v, int64_t reduction, int column_pair_base,
    int64_t valid_columns, float* output0, float* output1,
  ...
  const svbool_t pg_bf16 = svptrue_b16();
  const int64_t lanes_h = static_cast<int64_t>(svcnth());
  const uint16_t* p0_ptr = packed_p0;
  const uint16_t* p1_ptr = packed_p1;
  const uint16_t* v_ptr = packed_v + column_pair_base * lanes_h;
  for (int64_t reduction_base = 0; reduction_base < reduction;
       reduction_base += 4) {
    const svbfloat16_t b0 = load_bf16(v_ptr + 0 * lanes_h);
    const svbfloat16_t b1 = load_bf16(v_ptr + 1 * lanes_h);
    ...
    const svbfloat16_t a0 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p0_ptr + 0));
    const svbfloat16_t a1 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p0_ptr + 8));
    const svbfloat16_t a2 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p0_ptr + 16));
    const svbfloat16_t a3 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p0_ptr + 24));
    const svbfloat16_t a4 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p1_ptr + 0));
    const svbfloat16_t a5 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p1_ptr + 8));
    const svbfloat16_t a6 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p1_ptr + 16));
    const svbfloat16_t a7 = svld1rq_bf16(
        pg_bf16, reinterpret_cast<const __bf16*>(p1_ptr + 24));
    p0_ptr += 32;
    p1_ptr += 32;
  ...
  add_pv_row_pair(output0, output_row_stride, 0, column_pair_base + 0,
                  valid_columns, c00);
  add_pv_row_pair(output0, output_row_stride, 0, column_pair_base + 1,
                  valid_columns, c01);
  add_pv_row_pair(output0, output_row_stride, 1, column_pair_base + 0,
                  valid_columns, c10);
  add_pv_row_pair(output0, output_row_stride, 1, column_pair_base + 1,
                  valid_columns, c11);
  add_pv_row_pair(output0, output_row_stride, 2, column_pair_base + 0,
                  valid_columns, c20);
  add_pv_row_pair(output0, output_row_stride, 2, column_pair_base + 1,
                  valid_columns, c21);
  add_pv_row_pair(output0, output_row_stride, 3, column_pair_base + 0,
                  valid_columns, c30);
  add_pv_row_pair(output0, output_row_stride, 3, column_pair_base + 1,
                  valid_columns, c31);
  add_pv_row_pair(output1, output_row_stride, 0, column_pair_base + 0,
                  valid_columns, c40);
  add_pv_row_pair(output1, output_row_stride, 0, column_pair_base + 1,
                  valid_columns, c41);
  add_pv_row_pair(output1, output_row_stride, 1, column_pair_base + 0,
                  valid_columns, c50);
  add_pv_row_pair(output1, output_row_stride, 1, column_pair_base + 1,
                  valid_columns, c51);
  add_pv_row_pair(output1, output_row_stride, 2, column_pair_base + 0,
                  valid_columns, c60);
  add_pv_row_pair(output1, output_row_stride, 2, column_pair_base + 1,
                  valid_columns, c61);
  add_pv_row_pair(output1, output_row_stride, 3, column_pair_base + 0,
                  valid_columns, c70);
  add_pv_row_pair(output1, output_row_stride, 3, column_pair_base + 1,
                  valid_columns, c71);
}
