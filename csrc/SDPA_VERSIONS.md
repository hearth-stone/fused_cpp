...
| 日期 | 改动概述 | 受影响文件 |
|---|---|---|
| 2026-07-26 | **x86 AVX-512 BF16 MoE 完成 K-loop scheduling / packed-B prefetch**：Xbyak W13/W2 新增 cache-key 隔离的 `baseline/no_prefetch/unroll2/unroll2_t0/unroll2_t1`；双 K-pair 路径提前加载两组 B、把 loop branch 减半，并在 ZMM 允许时使用独立累加链，奇数 K-pair 保留单 pair tail。C8i8 W2 M12/K512/N4096 的 T0 路径 cycles -4.2%、branches -31.1%、L1D misses -46.3%，118.17→123.24 GFLOP/s；W13 去掉原 T0 在 K4096 退化 4.4%，T1 未胜 T0。stage/dimension-aware auto 因而让 W13 在 M96--511 用 T0、M>=512 用无显式 prefetch unroll，让 W2 在常规 H/F 的 M>=12 用 T0，generic profile 与小 W13 保留 baseline。H4096/F512/M2048 端到端 1T/8T 提升 2.78%/3.56%，并在 worker 启动前按 H/F/team 预生成全部 auto JIT；旧路径和全部负向 control 继续可选。 | 改 `csrc/moe/x86/avx512_bf16/{executor.cpp,jit_kernels.cpp,kernels.h,policy.cpp,policy.h}`、`tests/test_moe_avx512_bf16.py`、`benchmarks/{bench_avx512_bf16_gemm.cpp,bench_avx512_bf16_w13.cpp,README.md}`；新建 `benchmarks/{bench_avx512_bf16_w2.cpp,bench_avx512_k_loop.py}` 与 `optimizations/fused_moe_avx512/results/amazon_c8i_8core_avx512_k_loop_20260726.md`；改 optimization README/TODO/manifest、`csrc/SDPA_VERSIONS.md` |
| 2026-07-26 | **x86 AVX-512 BF16 MoE 完成 JIT-internal Bulk-MN loop**：W13/W2 的 full-M12 路径新增 N-block 外循环与 M-panel 内循环，一次 JIT 调用覆盖整个 cache window，共享 callee-save frame、常量/stride setup、scratch 与 `vzeroupper`；M1--11、F16/N32 tail 继续走现有 exact kernel。新增 `auto/baseline/w13/w2/bulk_mn` 同 binary 控制，强制路径与自动路径各通过 269 项测试。C8i8 H4096/F512 M12--256 仅 -0.2%~+0.2%，M2048 +0.1%；H64/F2048 的收益集中在 W13，约 +0.2%~+0.5%，W2 基本持平。因此 auto 只在 C8i W13、H<=64、F>=1024、M>=48、team<=4 时启用，常规形状保留旧路径。 | 改 `csrc/moe/x86/avx512_bf16/{jit_kernels.cpp,policy.cpp,policy.h}`、`tests/test_moe_avx512_bf16.py`；新建 `benchmarks/bench_avx512_bulk_mn.py` 与 `optimizations/fused_moe_avx512/results/amazon_c8i_8core_avx512_bulk_mn_20260726.md`；改 benchmark/optimization docs、manifest、TODO、`csrc/SDPA_VERSIONS.md` |
| 2026-07-26 | **x86 AVX-512 BF16 MoE 完成 Small-M multi-N JIT**：M1--3 W13 在一次 A K-pair 广播中累计 4 个相邻 F16 block，M4 累计 2 个；W2 M1--2 使用 N128、M3--4 使用 N64，继续直接读取现有 VNNI2/N32 权重并组合 2/1-block 与 N tail。新增 `auto/baseline/w13/w2/multi_n` 同 binary 控制和 C8i stage/dimension/team-width 策略；generic profile 与 8-worker expert team 保留旧 kernel。C8i8 单核 H4096/F512 M1--4 提升 6.3%--16.1%，H4096/F2048 提升 7.0%--15.9%；2/4T 保留场景最高 +14.6%/+9.6%，8T 负向 control 最慢 -13.8% 后由 auto 回退。 | 改 `csrc/moe/{module.cpp,x86/avx512_bf16/{executor.cpp,jit_kernels.cpp,kernels.h,policy.cpp,policy.h}}`、`tests/test_moe_avx512_bf16.py`；新建 `benchmarks/bench_avx512_small_m_multi_n.py` 与 `optimizations/fused_moe_avx512/results/amazon_c8i_8core_avx512_small_m_multi_n_20260726.md`；改 benchmark/optimization docs、manifest、TODO、`csrc/SDPA_VERSIONS.md` |
| 2026-07-26 | **x86 BF16 MoE 引入 dimension-aware 自动策略**：新增 backend ID 104 `x86_bf16_auto` 与 CPUID versioned profile；Amazon C8i 按 `max(M)*H*F` 在 AVX-512/AMX 间 dispatch，按 aggregate work 收缩小调用线程，按 H/F/M 选择 AMX `m2n2/m1n4`、cache window，并按 `H*F` 缩放 skew-wave target。ID 104 的单份 K32/N32 packed weights 可被两条 ISA 路径复用，显式 backend 和环境变量保留为验证 control。C8i8 focused suite 226 passed/3 skipped；H64/F16/M1 8-worker auto 5.941 us，forced AVX-512/AMX 为 12.965/19.078 us；H4096/F512/M64 1T auto 相对 forced AMX regret 0.068%。 | 新建 `csrc/moe/x86/avx512_bf16/{policy.cpp,policy.h}`、`benchmarks/bench_x86_bf16_policy.py`、`optimizations/fused_moe_avx512/results/amazon_c8i_8core_dimension_aware_policy_20260726.md`；改 x86 MoE backend/executor/JIT/Python API、tests、benchmark/optimization docs、`cpu_moe_schedule_optimization/MATHEMATICAL_MODEL.md`、`csrc/SDPA_VERSIONS.md` |
