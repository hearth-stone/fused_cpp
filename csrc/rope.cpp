...
at::Tensor apply_rope(at::Tensor x, at::Tensor cos_sin_cache,
                      at::Tensor positions, bool is_neox_style) {
    torch::NoGradGuard no_grad;

    TORCH_CHECK(x.dim() == 3,
                "apply_rope: x must be 3-D [num_tokens, num_heads, rot_dim], got dim=", x.dim());
    TORCH_CHECK(cos_sin_cache.dim() == 2,
                "apply_rope: cos_sin_cache must be 2-D [max_pos, rot_dim]");
    TORCH_CHECK(positions.dim() == 1,
                "apply_rope: positions must be 1-D [num_tokens]");
    TORCH_CHECK(x.size(0) == positions.size(0),
                "apply_rope: x num_tokens (", x.size(0),
                ") must match positions size (", positions.size(0), ")");

    auto rot_dim = x.size(-1);
    TORCH_CHECK(rot_dim % 2 == 0,
                "apply_rope: rot_dim must be even, got ", rot_dim);
    TORCH_CHECK(cos_sin_cache.size(-1) == rot_dim,
                "apply_rope: cos_sin_cache last dim (", cos_sin_cache.size(-1),
