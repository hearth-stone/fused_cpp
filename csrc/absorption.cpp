...
std::tuple<at::Tensor, at::Tensor> build_absorption_matrices(at::Tensor kv_b_proj_weight, int64_t num_heads,
  ...
  TORCH_CHECK(kv_b_proj_weight.size(0) == expected_out, "build_absorption_matrices: kv_b_proj_weight rows (",
              kv_b_proj_weight.size(0), ") must equal num_heads * (qk_nope_head_dim + v_head_dim) = ", expected_out);
  TORCH_CHECK(kv_b_proj_weight.size(1) == kv_lora_rank, "build_absorption_matrices: kv_b_proj_weight cols (",
              kv_b_proj_weight.size(1), ") must equal kv_lora_rank (", kv_lora_rank, ")");
