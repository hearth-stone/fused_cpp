...
static at::Tensor bf16_to_fp32(const at::Tensor& weight_bf16) { return weight_bf16.to(at::kFloat).contiguous(); }
...
class KAIThreadPool {
 public:
  ...
  //          把调用线程绑到那个核。
  KAIThreadPool(int num_workers, std::vector<int> cpu_ids) : num_workers_(num_workers), cpu_ids_(std::move(cpu_ids)) {
    if (!cpu_ids_.empty() && static_cast<int>(cpu_ids_.size()) != num_workers_ + 1) {
      throw std::runtime_error("KAIThreadPool: cpu_ids 长度必须为 0 或 num_workers+1");
    }
    const std::size_t num_threads = static_cast<std::size_t>(num_workers_) + 1;
    ...
    for (int i = 0; i < num_workers_; ++i) {
      ...
      workers_.emplace_back([this, i, worker_cpu]() { this->worker_loop(i + 1, worker_cpu); });
    }
  }

  explicit KAIThreadPool(int num_workers) : KAIThreadPool(num_workers, std::vector<int>{}) {}
  ...
  void parallel_for(int num_tasks, const std::function<void(int, int)>& fn) {
    ...
    std::unique_lock<std::mutex> lk(mu_);
    done_cv_.wait(lk, [this]() { return done_count_.load(std::memory_order_acquire) == num_workers_; });
    current_fn_ = nullptr;
  }
  ...
  std::uint8_t* LhsScratch(int thread_id) { return lhs_scratch_[static_cast<std::size_t>(thread_id)].data(); }

  // 取 thread_id 对应的 FP32 tile scratch 指针（仅 BF16 输出路径使用）。
  float* Fp32TileScratch(int thread_id) { return fp32_scratch_[static_cast<std::size_t>(thread_id)].data(); }
 ...
 private:
  void worker_loop(int thread_id, int cpu_id) {
    ...
    while (true) {
      {
        std::unique_lock<std::mutex> lk(mu_);
        cv_.wait(lk, [this, &last_epoch]() { return stop_ || job_epoch_ != last_epoch; });
        if (stop_) return;
        last_epoch = job_epoch_;
      }
      run_until_drained(thread_id);
      if (done_count_.fetch_add(1, std::memory_order_acq_rel) + 1 == num_workers_) {
        std::unique_lock<std::mutex> lk(mu_);
        done_cv_.notify_all();
      }
    }
...
at::Tensor kai_gemm_prepare(at::Tensor weight, c10::optional<at::Tensor> bias) {
  TORCH_CHECK(weight.dim() == 2, "KAI GEMM prepare: weight 必须为 2D [K, N]，当前 dim=", weight.dim());
  TORCH_CHECK(weight.scalar_type() == at::kFloat || weight.scalar_type() == at::kBFloat16,
              "KAI GEMM prepare: weight 仅支持 float32 / bfloat16");
  TORCH_CHECK(weight.is_contiguous(), "KAI GEMM prepare: weight 必须 contiguous（行主序）");

  const int64_t K = weight.size(0);
  const int64_t N = weight.size(1);
  TORCH_CHECK(K > 0 && N > 0, "KAI GEMM prepare: K、N 必须为正，K=", K, " N=", N);
  ...
  if (bias.has_value() && bias.value().defined()) {
    ...
    TORCH_CHECK(b.dim() == 1, "KAI GEMM prepare: bias 必须为 1D [N]，当前 dim=", b.dim());
    TORCH_CHECK(b.size(0) == N, "KAI GEMM prepare: bias 长度必须等于 N=", N, "，实际 ", b.size(0));
    TORCH_CHECK(b.scalar_type() == at::kFloat, "KAI GEMM prepare: bias dtype 必须为 float32");
  ...
  const std::size_t packed_size = kai_get_rhs_packed_size_rhs_quant_pack_kxn_bf16p12x4biasf32_f32_neon(
      static_cast<std::size_t>(N), static_cast<std::size_t>(K), nr, kr);
  TORCH_CHECK(packed_size > 0, "KAI GEMM prepare: packed_size 计算失败（为 0）");

  // 分配一维 uint8 张量用于承载 packed RHS。
  at::Tensor packed = at::empty({static_cast<int64_t>(packed_size)}, at::TensorOptions().dtype(at::kByte));

  // rhs_stride: 行主序 [K, N]，每行 N 个 fp32 元素。
  const std::size_t rhs_stride = static_cast<std::size_t>(N) * sizeof(float);

  kai_run_rhs_quant_pack_kxn_bf16p12x4biasf32_f32_neon(
      /*num_groups=*/1, static_cast<std::size_t>(N), static_cast<std::size_t>(K), nr, kr, sr, rhs_stride,
      weight_f32.data_ptr<float>(),
      bias_ptr,  // 可能为 nullptr；KleidiAI 内部会用 0 填充 bias 区
      /*scale=*/nullptr, packed.data_ptr(),
      /*extra_bytes=*/0,
      /*params=*/nullptr);
...
int64_t create_kai_thread_pool(std::vector<int64_t> cpu_ids) {
  ...
  for (int64_t c : cpu_ids) {
    TORCH_CHECK(c >= 0, "KAIThreadPool: cpu_ids 元素必须 >= 0，实际 ", c);
  ...
  if (cpus.empty()) {
    ...
    pool = std::make_unique<fused_cpp::kai_gemm::KAIThreadPool>(num_workers, std::move(cpus));
  }
  return fused_cpp::kai_gemm::PoolRegistry::Instance().Register(std::move(pool));
}
...
int64_t create_kai_gemm_handler(at::Tensor packed_weight, int64_t K, int64_t N) {
  TORCH_CHECK(packed_weight.dim() == 1, "KAI GEMM handler: packed_weight 必须为 1D uint8 张量");
  TORCH_CHECK(packed_weight.scalar_type() == at::kByte, "KAI GEMM handler: packed_weight dtype 必须为 uint8");
  TORCH_CHECK(packed_weight.is_contiguous(), "KAI GEMM handler: packed_weight 必须 contiguous");
  TORCH_CHECK(K > 0 && N > 0, "KAI GEMM handler: K、N 必须为正，K=", K, " N=", N);

  // 校验 packed_weight 大小与 (K, N) 匹配。
  const std::size_t expected = kai_get_rhs_packed_size_rhs_quant_pack_kxn_bf16p12x4biasf32_f32_neon(
      static_cast<std::size_t>(N), static_cast<std::size_t>(K), fused_cpp::kai_gemm::kNr, fused_cpp::kai_gemm::kKr);
  TORCH_CHECK(static_cast<std::size_t>(packed_weight.numel()) == expected,
              "KAI GEMM handler: packed_weight 大小与 (K, N) 不匹配，"
              "期望 ",
              expected, " bytes，实际 ", packed_weight.numel(), " bytes");
...
void release_kai_gemm_handler(int64_t handler_ptr) {
  auto* handler = reinterpret_cast<fused_cpp::kai_gemm::KAIGEMMHandler*>(handler_ptr);
  if (handler != nullptr) {
...
// 两个输出参数互斥：恰好一个为非空。
// ---------------------------------------------------------------------------
static void gemm_mc_block(const float* lhs_base,  // 指向 A 开头 [m0, 0] 的 FP32 指针
                          ...
                          std::size_t mc_rows, std::size_t nc_cols, std::size_t K, float clamp_min, float clamp_max,
  ...
  kai_run_lhs_quant_pack_bf16p8x4_f32_neon(mc_rows, K, kMr, kKr, /*sr=*/1, /*m_idx_start=*/0, lhs_base,
                                           static_cast<std::size_t>(lhs_stride_bytes), lhs_packed_buffer);
  ...
  for (std::size_t j = 0; j < nc_cols; j += kNr) {
    ...
    const std::size_t rhs_off = kai_get_rhs_packed_offset_rhs_quant_pack_kxn_bf16p12x4biasf32_f32_neon(j, K, kNr, kKr);
    const std::uint8_t* rhs_sub = rhs_packed + rhs_off;

    if (dst_f32_base != nullptr) {
      // ---- FP32 输出：微内核直接写用户 output ----
      float* dst_sub = dst_f32_base + j;
      const std::size_t dst_row_bytes = dst_f32_stride_elems * sizeof(float);
      kai_run_matmul_clamp_f32_bf16p8x4_bf16p12x4b_8x12_neon_mmla(
          mc_rows, n_block, K, lhs_packed_buffer, rhs_sub, dst_sub, dst_row_bytes, sizeof(float), clamp_min, clamp_max);
    } else {
      ...
      kai_run_matmul_clamp_f32_bf16p8x4_bf16p12x4b_8x12_neon_mmla(mc_rows, n_block, K, lhs_packed_buffer, rhs_sub,
                                                                  fp32_tile, tile_row_bytes, sizeof(float), clamp_min,
                                                                  clamp_max);
      ...
      for (std::size_t i = 0; i < mc_rows; ++i) {
        ...
        c10::BFloat16* dst_row = dst_col_base + i * bf16_dst_stride_elems;
...
static void run_gemm_impl(KAIGEMMHandler& handler, KAIThreadPool* pool, const float* lhs_ptr, int64_t lhs_stride_bytes,
                          void* dst_ptr, int64_t dst_stride_row_bytes, bool dst_is_bf16, std::size_t M, std::size_t N,
                          std::size_t K) {
  const auto* rhs_packed = static_cast<const std::uint8_t*>(handler.packed_weight_ptr);
  ...
  const std::size_t lhs_buf_bytes = kai_get_lhs_packed_size_lhs_quant_pack_bf16p8x4_f32_neon(Mc, K, kMr, kKr, /*sr=*/1);
  ...
  auto get_lhs_buf = [&](int tid) -> std::uint8_t* { return use_pool ? pool->LhsScratch(tid) : local_lhs.data(); };
  auto get_tile_buf = [&](int tid) -> float* { return use_pool ? pool->Fp32TileScratch(tid) : local_tile.data(); };

  // 为单个 Mc 块处理：给定当前 Nc 切片和 m0/mc。
  auto run_one_mc = [&](const std::uint8_t* rhs_n_slice, std::size_t n0, std::size_t nc, std::size_t m0, std::size_t mc,
                        int tid) {
    std::uint8_t* lhs_buf = get_lhs_buf(tid);
    const float* lhs_base = lhs_ptr + m0 * (lhs_stride_bytes / sizeof(float));

    if (dst_is_bf16) {
      float* tile = get_tile_buf(tid);
      auto* dst_bytes = static_cast<std::uint8_t*>(dst_ptr) + m0 * dst_stride_row_bytes + n0 * sizeof(c10::BFloat16);
      gemm_mc_block(lhs_base, lhs_stride_bytes, rhs_n_slice,
                    /*dst_f32_base=*/nullptr, /*dst_f32_stride_elems=*/0, reinterpret_cast<c10::BFloat16*>(dst_bytes),
                    static_cast<std::size_t>(dst_stride_row_bytes / sizeof(c10::BFloat16)), mc, nc, K, kClampMin,
                    kClampMax, lhs_buf, tile);
    } else {
      auto* dst_bytes = static_cast<std::uint8_t*>(dst_ptr) + m0 * dst_stride_row_bytes + n0 * sizeof(float);
      gemm_mc_block(lhs_base, lhs_stride_bytes, rhs_n_slice, reinterpret_cast<float*>(dst_bytes),
                    static_cast<std::size_t>(dst_stride_row_bytes / sizeof(float)),
                    /*bf16_dst_base=*/nullptr, /*bf16_dst_stride_elems=*/0, mc, nc, K, kClampMin, kClampMax, lhs_buf,
                    /*fp32_tile=*/nullptr);
    }
  };
  ...
  for (std::size_t n0 = 0; n0 < N; n0 += Nc) {
    ...
    const std::size_t rhs_off_nc =
        kai_get_rhs_packed_offset_rhs_quant_pack_kxn_bf16p12x4biasf32_f32_neon(n0, K, kNr, kKr);
    ...
    if (num_mc >= static_cast<std::size_t>(pool_threads)) {
      // 够喂饱：按 Mc 派发。
      pool->parallel_for(static_cast<int>(num_mc), [&](int tid, int task_idx) {
        const std::size_t m0 = static_cast<std::size_t>(task_idx) * Mc;
        const std::size_t mc = std::min(Mc, M - m0);
        run_one_mc(rhs_n_slice, n0, nc, m0, mc, tid);
      });
    } else {
      // 不够：降级到 Mr 粒度派发。
      const std::size_t num_mr = (M + kMr - 1) / kMr;
      pool->parallel_for(static_cast<int>(num_mr), [&](int tid, int task_idx) {
        const std::size_t m0 = static_cast<std::size_t>(task_idx) * kMr;
        const std::size_t mc = std::min(static_cast<std::size_t>(kMr), M - m0);
        run_one_mc(rhs_n_slice, n0, nc, m0, mc, tid);
      });
    }
  }
...
void kai_gemm(at::Tensor output, at::Tensor input, int64_t handler_ptr, int64_t pool_handle) {
  auto* handler_raw = reinterpret_cast<fused_cpp::kai_gemm::KAIGEMMHandler*>(handler_ptr);
  TORCH_CHECK(handler_raw != nullptr, "KAI GEMM: handler 指针为空");
  auto& handler = *handler_raw;

  TORCH_CHECK(input.dim() == 2, "KAI GEMM: input 必须为 2D [M, K]，实际 dim=", input.dim());
  TORCH_CHECK(output.dim() == 2, "KAI GEMM: output 必须为 2D [M, N]，实际 dim=", output.dim());
  TORCH_CHECK(input.scalar_type() == at::kFloat, "KAI GEMM: input dtype 必须为 float32");
  TORCH_CHECK(input.is_contiguous(), "KAI GEMM: input 必须 contiguous");
  TORCH_CHECK(output.is_contiguous(), "KAI GEMM: output 必须 contiguous");

  const int64_t M = input.size(0);
  const int64_t K = input.size(1);
  const int64_t N = output.size(1);
  TORCH_CHECK(K == handler.K, "KAI GEMM: input K 与 handler 不匹配，期望 ", handler.K, "，实际 ", K);
  TORCH_CHECK(N == handler.N, "KAI GEMM: output N 与 handler 不匹配，期望 ", handler.N, "，实际 ", N);
  TORCH_CHECK(output.size(0) == M, "KAI GEMM: output M 与 input M 不匹配，期望 ", M, "，实际 ", output.size(0));
  ...
  if (pool_handle != 0) {
    ...
    TORCH_CHECK(pool != nullptr, "KAI GEMM: pool_handle 非法或已销毁，handle=", pool_handle);
  }

  const float* lhs_ptr = input.data_ptr<float>();
  const int64_t lhs_stride_bytes = static_cast<int64_t>(K) * static_cast<int64_t>(sizeof(float));
  void* dst_ptr = output.data_ptr();
  const int64_t dst_stride_row_bytes = static_cast<int64_t>(N) * static_cast<int64_t>(output.element_size());

  if (output.scalar_type() == at::kFloat) {
    fused_cpp::kai_gemm::run_gemm_impl(handler, pool, lhs_ptr, lhs_stride_bytes, dst_ptr, dst_stride_row_bytes,
                                       /*dst_is_bf16=*/false, static_cast<std::size_t>(M), static_cast<std::size_t>(N),
                                       static_cast<std::size_t>(K));
  } else if (output.scalar_type() == at::kBFloat16) {
    fused_cpp::kai_gemm::run_gemm_impl(handler, pool, lhs_ptr, lhs_stride_bytes, dst_ptr, dst_stride_row_bytes,
                                       /*dst_is_bf16=*/true, static_cast<std::size_t>(M), static_cast<std::size_t>(N),
                                       static_cast<std::size_t>(K));
  } else {
    TORCH_CHECK(false, "KAI GEMM: output dtype 仅支持 float32 和 bfloat16");
  }
}
