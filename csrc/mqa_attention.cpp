...
inline void mqa_attention_kernel_tmpl(const scalar_t* q_ptr, const scalar_t* k_ptr, const scalar_t* v_ptr,
                                      float* out_ptr, const MqaParams& p) {
  ...
  auto compute_row = [&](int64_t row_idx, std::vector<float>& scores, std::vector<float>& acc) {
    ...
    float* o_row = out_ptr + b * o_stride_b + n * o_stride_n + l * o_stride_l;
    ...
    for (int64_t s = 0; s < p.S; ++s) {
      if (p.is_causal && s > causal_limit) {
        scores[static_cast<size_t>(s)] = -std::numeric_limits<float>::infinity();
        continue;
      }
      ...
      if (p.mask_ptr != nullptr) {
        score += p.mask_ptr[b * p.mask_stride_b + n * p.mask_stride_n + l * p.mask_stride_l + s * p.mask_stride_s];
      }
    ...
    if (std::isfinite(row_max)) {
      for (int64_t s = 0; s < p.S; ++s) {
        ...
        for (int64_t ev = 0; ev < p.Ev; ++ev) {
          acc[static_cast<size_t>(ev)] += weight * static_cast<float>(v_row[ev]);
        }
      }
    }
...
inline void mqa_attention_bf16_nomask_tiled(const at::BFloat16* q_ptr, const at::BFloat16* k_ptr,
                                            const at::BFloat16* v_ptr, float* out_ptr, const MqaParams& p) {
  ...
  auto compute_tile = [&](int64_t tile_idx, std::vector<float>& scores) {
    ...
    const at::BFloat16* q_tile = q_ptr + b * q_stride_b + n * q_stride_n + l0 * q_stride_l;
    const at::BFloat16* k_base = k_ptr + b * k_stride_b;
    const at::BFloat16* v_base = v_ptr + b * v_stride_b;
    float* o_tile = out_ptr + b * o_stride_b + n * o_stride_n + l0 * o_stride_l;

    for (int64_t s0 = 0; s0 < p.S; s0 += kTileS) {
      const int sk = static_cast<int>(std::min<int64_t>(kTileS, p.S - s0));
      ...
      if (lq == kTileL && sk == kTileS) {
        ...
        MK::qkt_8x8(q_tile, q_stride_l, k_tile, k_stride_s, p.E, p.scale, tmp_qkt);
        for (int i = 0; i < kTileL; ++i) {
          std::copy_n(tmp_qkt + i * kTileS, kTileS, scores.data() + i * p.S + s0);
        }
      } else if (lq == kTileL && sk == 4) {
        MK::qkt_8x4(q_tile, q_stride_l, k_tile, k_stride_s, p.E, p.scale, score_tile, p.S);
      } else {
        MK::qkt_tail(q_tile, q_stride_l, k_tile, k_stride_s, p.E, p.scale, score_tile, p.S, lq, sk);
      }
    }
    ...
    for (int64_t ev0 = 0; ev0 < p.Ev; ev0 += kTileEv) {
      const int ev = static_cast<int>(std::min<int64_t>(kTileEv, p.Ev - ev0));
      const at::BFloat16* v_tile = v_base + ev0;
      float* o_block = o_tile + ev0;

      if (lq == kTileL && ev == kTileEv) {
        MK::pv_8x8(scores.data(), p.S, v_tile, v_stride_s, p.S, o_block, o_stride_l);
      } else {
        MK::pv_tail(scores.data(), p.S, v_tile, v_stride_s, p.S, o_block, o_stride_l, lq, ev);
      }
    }
  };
...
inline at::Tensor normalize_mqa_kv(const at::Tensor& tensor, const char* name, int64_t B, int64_t S, int64_t D) {
  TORCH_CHECK(tensor.dim() == 3 || tensor.dim() == 4, "multi_query_attention: ", name,
              " must be [B, S, D] or [B, 1, S, D], got ", tensor.dim(), "-D");
  if (tensor.dim() == 3) {
    TORCH_CHECK(tensor.size(0) == B && tensor.size(1) == S && tensor.size(2) == D, "multi_query_attention: ", name,
                " shape mismatch, expected [", B, ", ", S, ", ", D, "], got ", tensor.sizes());
    return ensure_contiguous(tensor);
  }
  TORCH_CHECK(tensor.size(0) == B && tensor.size(1) == 1 && tensor.size(2) == S && tensor.size(3) == D,
              "multi_query_attention: ", name, " shape mismatch, expected [", B, ", 1, ", S, ", ", D, "], got ",
              tensor.sizes());
  return ensure_contiguous(tensor.select(1, 0));
}

inline void fill_mask_strides(const at::Tensor& mask, int64_t B, int64_t N, int64_t L, int64_t S, MqaParams& p) {
  TORCH_CHECK(mask.dim() == 2 || mask.dim() == 3 || mask.dim() == 4,
              "multi_query_attention: attn_mask must be [L, S], [B, L, S], "
              "[B, 1, L, S], or [B, N, L, S], got ",
              mask.dim(), "-D");

  if (mask.dim() == 2) {
    TORCH_CHECK(mask.size(0) == L && mask.size(1) == S, "multi_query_attention: attn_mask [L, S] shape mismatch, got ",
                mask.sizes());
    p.mask_stride_b = 0;
    p.mask_stride_n = 0;
    p.mask_stride_l = S;
  ...
  if (mask.dim() == 3) {
    TORCH_CHECK((mask.size(0) == B || mask.size(0) == 1) && mask.size(1) == L && mask.size(2) == S,
                "multi_query_attention: attn_mask [B, L, S] shape mismatch, got ", mask.sizes());
  ...
  TORCH_CHECK((mask.size(0) == B || mask.size(0) == 1) && (mask.size(1) == N || mask.size(1) == 1) &&
                  mask.size(2) == L && mask.size(3) == S,
              "multi_query_attention: attn_mask [B, N, L, S] shape mismatch, got ", mask.sizes());
...
at::Tensor multi_query_attention(at::Tensor query, at::Tensor key, at::Tensor value,
  ...
  TORCH_CHECK(query.dim() == 4, "multi_query_attention: query must be [B, N, L, E], got ", query.dim(), "-D");
  ...
  if (dropout_p != 0.0) {
    TORCH_WARN("multi_query_attention: dropout_p=", dropout_p, " is ignored (inference only)");
  }
  ...
  TORCH_CHECK(N > 0 && L > 0 && E > 0, "multi_query_attention: query dimensions must be non-zero");

  int64_t S = 0;
  if (key.dim() == 3) {
    TORCH_CHECK(key.size(0) == B && key.size(2) == E, "multi_query_attention: key must be [B, S, E], got ",
                key.sizes());
    S = key.size(1);
  } else {
    TORCH_CHECK(key.dim() == 4 && key.size(0) == B && key.size(1) == 1 && key.size(3) == E,
  ...
  int64_t Ev = 0;
  if (value.dim() == 3) {
    TORCH_CHECK(value.size(0) == B && value.size(1) == S, "multi_query_attention: value must be [B, S, Ev], got ",
                value.sizes());
    Ev = value.size(2);
  } else {
    TORCH_CHECK(value.dim() == 4 && value.size(0) == B && value.size(1) == 1 && value.size(2) == S,
  ...
  p.E = E;
  p.Ev = Ev;
  p.scale = scale.has_value() ? static_cast<float>(scale.value())
  ...
  if (orig_dtype == at::kBFloat16) {
#if FUSED_CPP_MK_ENABLE_QK_PACKQK_SEQ4_BMAJOR_PV_PQUAD
    if (p.mask_ptr == nullptr) {
      mqa_attention_bf16_nomask_tiled(q.data_ptr<at::BFloat16>(), k.data_ptr<at::BFloat16>(),
                                      v.data_ptr<at::BFloat16>(), output_fp32.data_ptr<float>(), p);
    } else {
      mqa_attention_kernel_tmpl<at::BFloat16>(q.data_ptr<at::BFloat16>(), k.data_ptr<at::BFloat16>(),
                                              v.data_ptr<at::BFloat16>(), output_fp32.data_ptr<float>(), p);
    }
#else
    mqa_attention_kernel_tmpl<at::BFloat16>(q.data_ptr<at::BFloat16>(), k.data_ptr<at::BFloat16>(),
    ...
    mqa_attention_kernel_tmpl<float>(q.data_ptr<float>(), k.data_ptr<float>(), v.data_ptr<float>(),
                                     output_fp32.data_ptr<float>(), p);
  }
