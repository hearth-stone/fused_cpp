...
inline void sdpa_flash2_kernel_impl(
            ...
            for (int64_t l = 0; l < p.L; ++l) {
                ...
                for (int64_t blk = 0; blk < num_blocks; ++blk) {
                    ...
                    const int64_t block_len = std::min(
                        static_cast<int64_t>(BLOCK_S), p.S - block_start);
...
void sdpa_flash2_impl(const SdpaParams& p) {
    ...
    TORCH_CHECK(p.Ev <= MAX_EV,
                "sdpa_flash2_impl: v_head_dim (", p.Ev,
                ") exceeds maximum supported value (", MAX_EV, ")");
...
at::Tensor scaled_dot_product_attention(
    ...
    TORCH_CHECK(query.dim() == 4,
                ...
                "[B, N, L, E], got ", query.dim(), "-D");
    TORCH_CHECK(key.dim() == 4,
                "scaled_dot_product_attention: key must be 4-D "
                "[B, N, S, E], got ", key.dim(), "-D");
    TORCH_CHECK(value.dim() == 4,
                "scaled_dot_product_attention: value must be 4-D "
                "[B, N, S, Ev], got ", value.dim(), "-D");

    TORCH_CHECK(query.size(0) == key.size(0) && query.size(0) == value.size(0),
                "scaled_dot_product_attention: batch size mismatch among Q/K/V (",
                query.size(0), ", ", key.size(0), ", ", value.size(0), ")");
    TORCH_CHECK(query.size(1) == key.size(1) && query.size(1) == value.size(1),
                "scaled_dot_product_attention: num_heads mismatch among Q/K/V (",
                query.size(1), ", ", key.size(1), ", ", value.size(1), ")");
    TORCH_CHECK(key.size(2) == value.size(2),
                "scaled_dot_product_attention: K and V seq_len mismatch (",
                key.size(2), " vs ", value.size(2), ")");
    TORCH_CHECK(query.size(3) == key.size(3),
                "scaled_dot_product_attention: Q and K head_dim mismatch (",
                query.size(3), " vs ", key.size(3), ")");

    // ── 参数处理 ──────────────────────────────────────────────────────────
    TORCH_CHECK(!enable_gqa,
                "scaled_dot_product_attention: enable_gqa=true is not supported");

    if (dropout_p != 0.0) {
        TORCH_WARN("scaled_dot_product_attention: dropout_p=", dropout_p,
                   " is ignored (inference only)");
    }

    double scale_val = scale.has_value()
        ? scale.value()
    ...
    // ── 安全检查：output_acc 栈上缓冲区上限 ─────────────────────────────
    constexpr int64_t MAX_EV = 4096;
    TORCH_CHECK(Ev <= MAX_EV,
                "scaled_dot_product_attention: v_head_dim (", Ev,
                ") exceeds maximum supported value (", MAX_EV, ")");
