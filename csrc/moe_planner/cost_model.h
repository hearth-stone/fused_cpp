...
class CostModel {
 public:
  ...
  static CostModel from_table(std::vector<int> route_buckets, std::vector<int> thread_buckets,
  ...
  static inline double synth_serial(int routes) { return 18000.0 + static_cast<double>(routes) * 1850.0; }
  ...
  static inline int64_t synthetic_ns(int routes, int threads) {
    ...
    return static_cast<int64_t>(synth_serial(routes) / synth_speedup(useful) + synth_overhead(threads));
  }
 ...
 private:
  ...
  int64_t table_ns(int routes, int threads) const {
    ...
    const auto rit = std::lower_bound(route_buckets_.begin(), route_buckets_.end(), rb);
    const auto tit = std::lower_bound(thread_buckets_.begin(), thread_buckets_.end(), tb);
