...
Workload prepare_workload(const int32_t* routes_hist, int num_experts,
    ...
    struct AW { int32_t id; int32_t routes; };
...
#pragma omp parallel for schedule(static) if (parallel)
        ...
        for (int a = 0; a < A; ++a) {
            ...
            for (int t = 1; t <= sat; ++t) {
                row[t - 1] = static_cast<int64_t>(serial / speedup[t - 1] +
                                                  overhead[t - 1]);
            }
            for (int t = sat + 1; t <= C; ++t) {
                row[t - 1] = static_cast<int64_t>(serial / speedup[routes - 1] +
                                                  overhead[t - 1]);
            }
        }
        return w;
...
int64_t emit_sequential_waves(const std::vector<TeamTmp>& teams, int num_cores,
    ...
    for (const auto& t : teams) {
        const bool cur_nonempty =
            static_cast<int>(r.team_expert_ids.size()) > wave_start;
        if (cur_nonempty && used + t.threads > num_cores) {
            r.wave_offsets.push_back(
                static_cast<int32_t>(r.team_expert_ids.size()));
            execute += wave_max;
...
std::vector<std::vector<int32_t>> enumerate_core_group_shapes(int num_cores,
                                                              int max_shapes) {
