...
struct alignas(2) BFloat16 {
    ...
    operator float() const {
        return ::fused_cpp::standalone::bf16_bits_to_fp32(x);
    }
};

static_assert(sizeof(BFloat16) == sizeof(uint16_t),
              "standalone at::BFloat16 must be uint16-compatible");
...
class Tensor {
 public:
    ...
    Tensor(size_t nbytes, ScalarType dtype)
        : storage_(nbytes), dtype_(dtype) {}
    ...
    do {                                                                    \
        if (!(condition)) {                                                  \
            throw std::runtime_error(                                       \
                ::fused_cpp::standalone::make_message(__VA_ARGS__));        \
        }                                                                   \
    } while (0)
