...
#if defined(__ARM_FEATURE_SVE_BF16) || (defined(__ARM_FEATURE_SVE) && defined(__ARM_FEATURE_BF16_VECTOR_ARITHMETIC))
...
inline svfloat32x2_t bf16_load2_evenodd_f32(svbool_t pg, svbool_t pg_h, const uint16_t* ptr) {
  const svuint16x2_t pair = svld2_u16(pg_h, ptr);
  return svcreate2_f32(bf16_u16_to_f32_low(pg, svget2_u16(pair, 0)), bf16_u16_to_f32_low(pg, svget2_u16(pair, 1)));
}

inline svfloat32x2_t bf16_load2_evenodd_f32(svbool_t pg, const uint16_t* ptr) {
  const int64_t active_lanes = static_cast<int64_t>(svcntp_b32(svptrue_b32(), pg));
...
inline void bf16_store_f32(svbool_t pg, uint16_t* ptr, svfloat32_t v) { svst1h_u32(pg, ptr, f32_to_bf16_bits(pg, v)); }
...
template <typename scalar_t>
FUSED_CPP_ALWAYS_INLINE void load_evenodd_contiguous_f32(svbool_t pg, const scalar_t* ptr, svfloat32_t& even,
                                                         svfloat32_t& odd) {
  if constexpr (std::is_same_v<scalar_t, float>) {
...
template <typename scalar_t>
FUSED_CPP_ALWAYS_INLINE void load_evenodd_contiguous_f32(svbool_t pg, svbool_t pg_h, const scalar_t* ptr,
                                                         svfloat32_t& even, svfloat32_t& odd) {
  if constexpr (std::is_same_v<scalar_t, float>) {
    (void)pg_h;
    const svfloat32x2_t pair = svld2_f32(pg, ptr);
    even = svget2_f32(pair, 0);
...
template <typename scalar_t>
FUSED_CPP_ALWAYS_INLINE void store_evenodd_contiguous_f32(svbool_t pg, scalar_t* ptr, svfloat32_t even,
                                                          svfloat32_t odd) {
  if constexpr (std::is_same_v<scalar_t, float>) {
...
FUSED_CPP_ALWAYS_INLINE void compute_kv_rows4_rope_chunk_sve(
    const row_t* row0, const row_t* row1, const row_t* row2, const row_t* row3, const float* cos_row0,
    const float* sin_row0, const float* cos_row1, const float* sin_row1, const float* cos_row2, const float* sin_row2,
    const float* cos_row3, const float* sin_row3, int64_t nope_head_dim, int64_t pair, svbool_t pg, svbool_t pg_h,
    svfloat32x2_t& out0, svfloat32x2_t& out1, svfloat32x2_t& out2, svfloat32x2_t& out3) {
  ...
  out0 = svcreate2_f32(svmul_f32_x(pg, even0, c0), svmul_f32_x(pg, odd0, c0));
  out1 = svcreate2_f32(svmul_f32_x(pg, even1, c1), svmul_f32_x(pg, odd1, c1));
  out2 = svcreate2_f32(svmul_f32_x(pg, even2, c2), svmul_f32_x(pg, odd2, c2));
  out3 = svcreate2_f32(svmul_f32_x(pg, even3, c3), svmul_f32_x(pg, odd3, c3));

  out0 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out0, 0), odd0, s0), svmla_f32_x(pg, svget2_f32(out0, 1), even0, s0));
  out1 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out1, 0), odd1, s1), svmla_f32_x(pg, svget2_f32(out1, 1), even1, s1));
  out2 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out2, 0), odd2, s2), svmla_f32_x(pg, svget2_f32(out2, 1), even2, s2));
  out3 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out3, 0), odd3, s3), svmla_f32_x(pg, svget2_f32(out3, 1), even3, s3));
}
...
void sum_sq_f32_sve_4x4(const float* row0, const float* row1, const float* row2, const float* row3, int64_t head_dim,
...
void sum_sq_bf16_sve_4x4(const uint16_t* row0, const uint16_t* row1, const uint16_t* row2, const uint16_t* row3,
                         int64_t head_dim, float& sum0, float& sum1, float& sum2, float& sum3) {
  ...
  for (; d < head_dim; d += vlh) {
    ...
    const svbfloat16_t v0 = svld1_bf16(pg, reinterpret_cast<const bfloat16_t*>(row0 + d));
    const svbfloat16_t v1 = svld1_bf16(pg, reinterpret_cast<const bfloat16_t*>(row1 + d));
    const svbfloat16_t v2 = svld1_bf16(pg, reinterpret_cast<const bfloat16_t*>(row2 + d));
    const svbfloat16_t v3 = svld1_bf16(pg, reinterpret_cast<const bfloat16_t*>(row3 + d));
...
void process_f32_rope_row_sve(float* row, const float* cos_row, const float* sin_row, int64_t nope_head_dim,
                              int64_t rope_half, float inv_rms);

void process_bf16_rope_row_sve(uint16_t* row, const float* cos_row, const float* sin_row, int64_t nope_head_dim,
                               int64_t rope_half, float inv_rms);

#if !FUSED_CPP_STRICT_MODE
void process_f32_row_sve(float* row, const float* cos_row, const float* sin_row, int64_t head_dim,
                         int64_t nope_head_dim, int64_t rope_half, float inv_rms) {
...
void process_f32_rope_row_sve(float* row, const float* cos_row, const float* sin_row, int64_t nope_head_dim,
  ...
  for (int64_t pair = 0; pair < rope_half; pair += static_cast<int64_t>(svcntw())) {
    ...
    const svfloat32_t out_even = svmls_f32_x(pg, svmul_f32_x(pg, even, c_inv), odd, s_inv);
    const svfloat32_t out_odd = svmla_f32_x(pg, svmul_f32_x(pg, odd, c_inv), even, s_inv);
...
FUSED_CPP_ALWAYS_INLINE void process_f32_rows4_rope_chunk_sve(float* row0, float* row1, float* row2, float* row3,
                                                              const float* cos_row, const float* sin_row,
                                                              int64_t nope_head_dim, int64_t pair, svbool_t pg,
                                                              svfloat32_t inv0, svfloat32_t inv1, svfloat32_t inv2,
                                                              svfloat32_t inv3) {
  const svfloat32_t c = svld1_f32(pg, cos_row + pair);
  const svfloat32_t s = svld1_f32(pg, sin_row + pair);
  ...
  svfloat32x2_t out0 = svcreate2_f32(svmul_f32_x(pg, even0, c_inv0), svmul_f32_x(pg, odd0, c_inv0));
  svfloat32x2_t out1 = svcreate2_f32(svmul_f32_x(pg, even1, c_inv1), svmul_f32_x(pg, odd1, c_inv1));
  svfloat32x2_t out2 = svcreate2_f32(svmul_f32_x(pg, even2, c_inv2), svmul_f32_x(pg, odd2, c_inv2));
  svfloat32x2_t out3 = svcreate2_f32(svmul_f32_x(pg, even3, c_inv3), svmul_f32_x(pg, odd3, c_inv3));

  out0 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out0, 0), odd0, s_inv0),
                       svmla_f32_x(pg, svget2_f32(out0, 1), even0, s_inv0));
  out1 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out1, 0), odd1, s_inv1),
                       svmla_f32_x(pg, svget2_f32(out1, 1), even1, s_inv1));
  out2 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out2, 0), odd2, s_inv2),
                       svmla_f32_x(pg, svget2_f32(out2, 1), even2, s_inv2));
  out3 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out3, 0), odd3, s_inv3),
...
void process_f32_rows4_sve(float* row0, float* row1, float* row2, float* row3, const float* cos_row,
                           const float* sin_row, int64_t nope_head_dim, int64_t rope_half, float inv_rms0,
                           float inv_rms1, float inv_rms2, float inv_rms3) {
  ...
  for (; pair + vl <= rope_half; pair += vl) {
    process_f32_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row, sin_row, nope_head_dim, pair, pg_all, inv0, inv1,
                                     inv2, inv3);
  }
  if (pair < rope_half) {
    const svbool_t pg = svwhilelt_b32(pair, rope_half);
    process_f32_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row, sin_row, nope_head_dim, pair, pg, inv0, inv1,
                                     inv2, inv3);
  }
}
#endif

void process_bf16_row_sve(uint16_t* row, const float* cos_row, const float* sin_row, int64_t head_dim,
                          int64_t nope_head_dim, int64_t rope_half, float inv_rms) {
  const svfloat32_t inv = svdup_f32(inv_rms);
  for (int64_t d = 0; d < nope_head_dim; d += static_cast<int64_t>(svcntw())) {
    const svbool_t pg = svwhilelt_b32(d, nope_head_dim);
...
void process_bf16_rope_row_sve(uint16_t* row, const float* cos_row, const float* sin_row, int64_t nope_head_dim,
  ...
  for (int64_t pair = 0; pair < rope_half; pair += vl) {
    ...
    const svfloat32x2_t q_pair = bf16_load2_evenodd_f32(pg, pg_h, row + nope_head_dim + 2 * pair);
...
FUSED_CPP_ALWAYS_INLINE void process_bf16_rows4_rope_chunk_sve(uint16_t* row0, uint16_t* row1, uint16_t* row2,
                                                               uint16_t* row3, const float* cos_row,
                                                               const float* sin_row, int64_t nope_head_dim,
                                                               int64_t pair, svbool_t pg, svbool_t pg_h,
                                                               svfloat32_t inv0, svfloat32_t inv1, svfloat32_t inv2,
                                                               svfloat32_t inv3) {
  const svfloat32_t c = svld1_f32(pg, cos_row + pair);
  const svfloat32_t s = svld1_f32(pg, sin_row + pair);

  const svfloat32x2_t q_pair0 = bf16_load2_evenodd_f32(pg, pg_h, row0 + nope_head_dim + 2 * pair);
  const svfloat32x2_t q_pair1 = bf16_load2_evenodd_f32(pg, pg_h, row1 + nope_head_dim + 2 * pair);
  const svfloat32x2_t q_pair2 = bf16_load2_evenodd_f32(pg, pg_h, row2 + nope_head_dim + 2 * pair);
  const svfloat32x2_t q_pair3 = bf16_load2_evenodd_f32(pg, pg_h, row3 + nope_head_dim + 2 * pair);
  ...
  svfloat32x2_t out0 = svcreate2_f32(svmul_f32_x(pg, even0, c), svmul_f32_x(pg, odd0, c));
  svfloat32x2_t out1 = svcreate2_f32(svmul_f32_x(pg, even1, c), svmul_f32_x(pg, odd1, c));
  svfloat32x2_t out2 = svcreate2_f32(svmul_f32_x(pg, even2, c), svmul_f32_x(pg, odd2, c));
  svfloat32x2_t out3 = svcreate2_f32(svmul_f32_x(pg, even3, c), svmul_f32_x(pg, odd3, c));

  out0 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out0, 0), odd0, s), svmla_f32_x(pg, svget2_f32(out0, 1), even0, s));
  out1 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out1, 0), odd1, s), svmla_f32_x(pg, svget2_f32(out1, 1), even1, s));
  out2 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out2, 0), odd2, s), svmla_f32_x(pg, svget2_f32(out2, 1), even2, s));
  out3 = svcreate2_f32(svmls_f32_x(pg, svget2_f32(out3, 0), odd3, s), svmla_f32_x(pg, svget2_f32(out3, 1), even3, s));
...
void process_bf16_rows4_sve(uint16_t* row0, uint16_t* row1, uint16_t* row2, uint16_t* row3, const float* cos_row,
                            const float* sin_row, int64_t nope_head_dim, int64_t rope_half, float inv_rms0,
                            float inv_rms1, float inv_rms2, float inv_rms3) {
  const svfloat32_t inv0 = svdup_f32(inv_rms0);
  const svfloat32_t inv1 = svdup_f32(inv_rms1);
  ...
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_bf16_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row, sin_row, nope_head_dim, pair, pg_all, pg_all_h,
                                      inv0, inv1, inv2, inv3);
  }
  if (pair < rope_half) {
    ...
    const svbool_t pg = svwhilelt_b32(static_cast<int64_t>(0), active);
    const svbool_t pg_h = svwhilelt_b16(static_cast<int64_t>(0), active);
    process_bf16_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row, sin_row, nope_head_dim, pair, pg, pg_h, inv0,
                                      inv1, inv2, inv3);
  }
}

FUSED_CPP_ALWAYS_INLINE void process_f32_kv_rows2_rope_chunk_sve(float* row0, float* row1, const float* cos_row0,
                                                                 const float* sin_row0, const float* cos_row1,
                                                                 const float* sin_row1, int64_t nope_head_dim,
                                                                 int64_t pair, svbool_t pg) {
  const svfloat32_t c0 = svld1_f32(pg, cos_row0 + pair);
  const svfloat32_t s0 = svld1_f32(pg, sin_row0 + pair);
  const svfloat32_t c1 = svld1_f32(pg, cos_row1 + pair);
  const svfloat32_t s1 = svld1_f32(pg, sin_row1 + pair);
  ...
  svfloat32x2_t out0 = svcreate2_f32(svmul_f32_x(pg, even0, c0), svmul_f32_x(pg, odd0, c0));
  svfloat32x2_t out1 = svcreate2_f32(svmul_f32_x(pg, even1, c1), svmul_f32_x(pg, odd1, c1));
...
void process_f32_kv_rows2_rope_sve(float* row0, float* row1, const float* cos_row0, const float* sin_row0,
                                   const float* cos_row1, const float* sin_row1, int64_t nope_head_dim,
                                   int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  const svbool_t pg_all = svptrue_b32();
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_f32_kv_rows2_rope_chunk_sve(row0, row1, cos_row0, sin_row0, cos_row1, sin_row1, nope_head_dim, pair,
                                        pg_all);
  }
  if (pair < rope_half) {
    const svbool_t pg = svwhilelt_b32(pair, rope_half);
    process_f32_kv_rows2_rope_chunk_sve(row0, row1, cos_row0, sin_row0, cos_row1, sin_row1, nope_head_dim, pair, pg);
  }
}

FUSED_CPP_ALWAYS_INLINE void process_f32_kv_rows4_rope_chunk_sve(float* row0, float* row1, float* row2, float* row3,
                                                                 const float* cos_row0, const float* sin_row0,
                                                                 const float* cos_row1, const float* sin_row1,
                                                                 const float* cos_row2, const float* sin_row2,
                                                                 const float* cos_row3, const float* sin_row3,
                                                                 int64_t nope_head_dim, int64_t pair, svbool_t pg) {
  svfloat32x2_t out0;
  svfloat32x2_t out1;
  svfloat32x2_t out2;
  svfloat32x2_t out3;
  compute_kv_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2, sin_row2,
                                  cos_row3, sin_row3, nope_head_dim, pair, pg, pg, out0, out1, out2, out3);
  svst2_f32(pg, row0 + nope_head_dim + 2 * pair, out0);
  svst2_f32(pg, row1 + nope_head_dim + 2 * pair, out1);
  svst2_f32(pg, row2 + nope_head_dim + 2 * pair, out2);
  svst2_f32(pg, row3 + nope_head_dim + 2 * pair, out3);
...
void process_f32_kv_rows4_rope_sve(float* row0, float* row1, float* row2, float* row3, const float* cos_row0,
                                   const float* sin_row0, const float* cos_row1, const float* sin_row1,
                                   const float* cos_row2, const float* sin_row2, const float* cos_row3,
                                   const float* sin_row3, int64_t nope_head_dim, int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  const svbool_t pg_all = svptrue_b32();
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_f32_kv_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2,
                                        sin_row2, cos_row3, sin_row3, nope_head_dim, pair, pg_all);
  }
  if (pair < rope_half) {
    const svbool_t pg = svwhilelt_b32(pair, rope_half);
    process_f32_kv_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2,
                                        sin_row2, cos_row3, sin_row3, nope_head_dim, pair, pg);
  }
}

FUSED_CPP_ALWAYS_INLINE void process_bf16_kv_rows2_rope_chunk_sve(uint16_t* row0, uint16_t* row1, const float* cos_row0,
                                                                  const float* sin_row0, const float* cos_row1,
                                                                  const float* sin_row1, int64_t nope_head_dim,
                                                                  int64_t pair, svbool_t pg, svbool_t pg_h) {
  const svfloat32x2_t kv_pair0 = bf16_load2_evenodd_f32(pg, pg_h, row0 + nope_head_dim + 2 * pair);
  const svfloat32x2_t kv_pair1 = bf16_load2_evenodd_f32(pg, pg_h, row1 + nope_head_dim + 2 * pair);
  const svfloat32_t even0 = svget2_f32(kv_pair0, 0);
  const svfloat32_t odd0 = svget2_f32(kv_pair0, 1);
  const svfloat32_t even1 = svget2_f32(kv_pair1, 0);
  const svfloat32_t odd1 = svget2_f32(kv_pair1, 1);
...
void process_bf16_kv_rows2_rope_sve(uint16_t* row0, uint16_t* row1, const float* cos_row0, const float* sin_row0,
                                    const float* cos_row1, const float* sin_row1, int64_t nope_head_dim,
                                    int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  const svbool_t pg_all = svptrue_b32();
  ...
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_bf16_kv_rows2_rope_chunk_sve(row0, row1, cos_row0, sin_row0, cos_row1, sin_row1, nope_head_dim, pair,
                                         pg_all, pg_all_h);
  }
  if (pair < rope_half) {
    const int64_t active = rope_half - pair;
    const svbool_t pg = svwhilelt_b32(static_cast<int64_t>(0), active);
    const svbool_t pg_h = svwhilelt_b16(static_cast<int64_t>(0), active);
    process_bf16_kv_rows2_rope_chunk_sve(row0, row1, cos_row0, sin_row0, cos_row1, sin_row1, nope_head_dim, pair, pg,
                                         pg_h);
  }
}

FUSED_CPP_ALWAYS_INLINE void process_bf16_kv_rows4_rope_chunk_sve(
    uint16_t* row0, uint16_t* row1, uint16_t* row2, uint16_t* row3, const float* cos_row0, const float* sin_row0,
    const float* cos_row1, const float* sin_row1, const float* cos_row2, const float* sin_row2, const float* cos_row3,
    const float* sin_row3, int64_t nope_head_dim, int64_t pair, svbool_t pg, svbool_t pg_h) {
  svfloat32x2_t out0;
  svfloat32x2_t out1;
  svfloat32x2_t out2;
  svfloat32x2_t out3;
  compute_kv_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2, sin_row2,
                                  cos_row3, sin_row3, nope_head_dim, pair, pg, pg_h, out0, out1, out2, out3);
  bf16_scatter_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair, svget2_f32(out0, 0));
  bf16_scatter_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair + 1, svget2_f32(out0, 1));
  bf16_scatter_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair, svget2_f32(out1, 0));
  bf16_scatter_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair + 1, svget2_f32(out1, 1));
...
void process_bf16_kv_rows4_rope_sve(uint16_t* row0, uint16_t* row1, uint16_t* row2, uint16_t* row3,
                                    const float* cos_row0, const float* sin_row0, const float* cos_row1,
                                    const float* sin_row1, const float* cos_row2, const float* sin_row2,
                                    const float* cos_row3, const float* sin_row3, int64_t nope_head_dim,
                                    int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  const svbool_t pg_all = svptrue_b32();
  const svbool_t pg_all_h = svwhilelt_b16(static_cast<int64_t>(0), vl);
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_bf16_kv_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2,
                                         sin_row2, cos_row3, sin_row3, nope_head_dim, pair, pg_all, pg_all_h);
  }
  if (pair < rope_half) {
    const int64_t active = rope_half - pair;
    const svbool_t pg = svwhilelt_b32(static_cast<int64_t>(0), active);
    const svbool_t pg_h = svwhilelt_b16(static_cast<int64_t>(0), active);
    process_bf16_kv_rows4_rope_chunk_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2,
                                         sin_row2, cos_row3, sin_row3, nope_head_dim, pair, pg, pg_h);
  }
}
...
template <typename row_t, typename cache_t>
void process_kv_rope_row_to_cache_sve(const row_t* row, cache_t* cache_row, const float* cos_row, const float* sin_row,
                                      int64_t nope_head_dim, int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  for (int64_t pair = 0; pair < rope_half; pair += vl) {
...
template <typename row_t, typename cache_t>
FUSED_CPP_ALWAYS_INLINE void copy_kv_nope_rows2_to_cache_sve(const row_t* row0, const row_t* row1, cache_t* cache_row0,
                                                             cache_t* cache_row1, int64_t nope_head_dim) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  for (int64_t d = 0; d < nope_head_dim; d += vl) {
    const svbool_t pg = svwhilelt_b32(d, nope_head_dim);
...
template <typename row_t, typename cache_t>
FUSED_CPP_ALWAYS_INLINE void copy_kv_nope_rows4_to_cache_sve(const row_t* row0, const row_t* row1, const row_t* row2,
                                                             const row_t* row3, cache_t* cache_row0,
                                                             cache_t* cache_row1, cache_t* cache_row2,
                                                             cache_t* cache_row3, int64_t nope_head_dim) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  for (int64_t d = 0; d < nope_head_dim; d += vl) {
    const svbool_t pg = svwhilelt_b32(d, nope_head_dim);
    const svfloat32_t v0 = load_contiguous_f32(pg, row0 + d);
...
template <typename row_t, typename cache_t>
FUSED_CPP_ALWAYS_INLINE void process_kv_rows2_rope_to_cache_chunk_sve(const row_t* row0, const row_t* row1,
                                                                      cache_t* cache_row0, cache_t* cache_row1,
                                                                      const float* cos_row0, const float* sin_row0,
                                                                      const float* cos_row1, const float* sin_row1,
                                                                      int64_t nope_head_dim, int64_t pair, svbool_t pg,
                                                                      svbool_t pg_h) {
  ...
  store_evenodd_contiguous_f32(pg, cache_row0 + offset, svget2_f32(out0, 0), svget2_f32(out0, 1));
  store_evenodd_contiguous_f32(pg, cache_row1 + offset, svget2_f32(out1, 0), svget2_f32(out1, 1));
}

template <typename row_t, typename cache_t>
void process_kv_rows2_rope_to_cache_sve(const row_t* row0, const row_t* row1, cache_t* cache_row0, cache_t* cache_row1,
                                        const float* cos_row0, const float* sin_row0, const float* cos_row1,
                                        const float* sin_row1, int64_t nope_head_dim, int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  const svbool_t pg_all = svptrue_b32();
  const svbool_t pg_all_h = svwhilelt_b16(static_cast<int64_t>(0), vl);
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_kv_rows2_rope_to_cache_chunk_sve(row0, row1, cache_row0, cache_row1, cos_row0, sin_row0, cos_row1, sin_row1,
                                             nope_head_dim, pair, pg_all, pg_all_h);
  }
  if (pair < rope_half) {
    const int64_t active = rope_half - pair;
    const svbool_t pg = svwhilelt_b32(static_cast<int64_t>(0), active);
    const svbool_t pg_h = svwhilelt_b16(static_cast<int64_t>(0), active);
    process_kv_rows2_rope_to_cache_chunk_sve(row0, row1, cache_row0, cache_row1, cos_row0, sin_row0, cos_row1, sin_row1,
                                             nope_head_dim, pair, pg, pg_h);
  }
}

template <typename row_t, typename cache_t>
FUSED_CPP_ALWAYS_INLINE void process_kv_rows4_rope_to_cache_chunk_sve(
    const row_t* row0, const row_t* row1, const row_t* row2, const row_t* row3, cache_t* cache_row0,
    cache_t* cache_row1, cache_t* cache_row2, cache_t* cache_row3, const float* cos_row0, const float* sin_row0,
    const float* cos_row1, const float* sin_row1, const float* cos_row2, const float* sin_row2, const float* cos_row3,
    const float* sin_row3, int64_t nope_head_dim, int64_t pair, svbool_t pg, svbool_t pg_h) {
  svfloat32x2_t out0;
  svfloat32x2_t out1;
  ...
  store_evenodd_contiguous_f32(pg, cache_row0 + offset, svget2_f32(out0, 0), svget2_f32(out0, 1));
  store_evenodd_contiguous_f32(pg, cache_row1 + offset, svget2_f32(out1, 0), svget2_f32(out1, 1));
  store_evenodd_contiguous_f32(pg, cache_row2 + offset, svget2_f32(out2, 0), svget2_f32(out2, 1));
  store_evenodd_contiguous_f32(pg, cache_row3 + offset, svget2_f32(out3, 0), svget2_f32(out3, 1));
}

template <typename row_t, typename cache_t>
void process_kv_rows4_rope_to_cache_sve(const row_t* row0, const row_t* row1, const row_t* row2, const row_t* row3,
                                        cache_t* cache_row0, cache_t* cache_row1, cache_t* cache_row2,
                                        cache_t* cache_row3, const float* cos_row0, const float* sin_row0,
                                        const float* cos_row1, const float* sin_row1, const float* cos_row2,
                                        const float* sin_row2, const float* cos_row3, const float* sin_row3,
                                        int64_t nope_head_dim, int64_t rope_half) {
  const int64_t vl = static_cast<int64_t>(svcntw());
  const svbool_t pg_all = svptrue_b32();
  const svbool_t pg_all_h = svwhilelt_b16(static_cast<int64_t>(0), vl);
  int64_t pair = 0;
  for (; pair + vl <= rope_half; pair += vl) {
    process_kv_rows4_rope_to_cache_chunk_sve(row0, row1, row2, row3, cache_row0, cache_row1, cache_row2, cache_row3,
                                             cos_row0, sin_row0, cos_row1, sin_row1, cos_row2, sin_row2, cos_row3,
                                             sin_row3, nope_head_dim, pair, pg_all, pg_all_h);
  }
  if (pair < rope_half) {
    const int64_t active = rope_half - pair;
    const svbool_t pg = svwhilelt_b32(static_cast<int64_t>(0), active);
    ...
    process_kv_rows4_rope_to_cache_chunk_sve(row0, row1, row2, row3, cache_row0, cache_row1, cache_row2, cache_row3,
                                             cos_row0, sin_row0, cos_row1, sin_row1, cos_row2, sin_row2, cos_row3,
                                             sin_row3, nope_head_dim, pair, pg, pg_h);
  }
}

template <typename row_t>
void q_norm_rope_fused_sve_impl(row_t* q_data, const int64_t* pos_data, int64_t scalar_pos, bool scalar_position,
                                const float* cos_sin_data, int64_t cos_sin_stride, int64_t num_tokens,
                                int64_t num_heads, int64_t head_dim, int64_t nope_head_dim, int64_t rope_half,
                                int64_t q_stride_t, int64_t q_stride_h, double eps) {
#if !FUSED_CPP_STRICT_MODE
...
#pragma omp parallel for collapse(2) schedule(static)
    ...
    for (int64_t token = 0; token < num_tokens; ++token) {
      for (int64_t head_group = 0; head_group < head_groups; ++head_group) {
        ...
        process_f32_rows4_sve(row0, row1, row2, row3, cos_row, sin_row, nope_head_dim, rope_half, inv0, inv1, inv2,
                              inv3);
      }
    }
...
#pragma omp parallel for collapse(2) schedule(static)
    ...
    for (int64_t token = 0; token < num_tokens; ++token) {
      for (int64_t head_group = 0; head_group < head_groups; ++head_group) {
        ...
        const float inv2 = 1.0f / std::sqrt(sum2 * scale + eps_f);
        const float inv3 = 1.0f / std::sqrt(sum3 * scale + eps_f);
        process_bf16_rows4_sve(row0, row1, row2, row3, cos_row, sin_row, nope_head_dim, rope_half, inv0, inv1, inv2,
                               inv3);
      }
    }
...
template <typename row_t>
void kv_rope_fused_sve_impl(row_t* kv_data, const int64_t* pos_data, int64_t scalar_pos, bool scalar_position,
                            const float* cos_sin_data, int64_t cos_sin_stride, int64_t num_tokens,
                            int64_t nope_head_dim, int64_t rope_half, int64_t kv_stride_t) {
...
#pragma omp parallel for schedule(static)
  ...
  for (int64_t token_group = 0; token_group < token_groups4; ++token_group) {
    ...
    if constexpr (std::is_same_v<row_t, float>) {
      process_f32_kv_rows4_rope_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2, sin_row2,
                                    cos_row3, sin_row3, nope_head_dim, rope_half);
    } else {
      process_bf16_kv_rows4_rope_sve(row0, row1, row2, row3, cos_row0, sin_row0, cos_row1, sin_row1, cos_row2, sin_row2,
                                     cos_row3, sin_row3, nope_head_dim, rope_half);
    }
  }
  ...
  if (tail_token + 1 < num_tokens) {
    ...
    const float* sin_row1 = cs_row1 + rope_half;
    if constexpr (std::is_same_v<row_t, float>) {
      process_f32_kv_rows2_rope_sve(row0, row1, cos_row0, sin_row0, cos_row1, sin_row1, nope_head_dim, rope_half);
    } else {
      process_bf16_kv_rows2_rope_sve(row0, row1, cos_row0, sin_row0, cos_row1, sin_row1, nope_head_dim, rope_half);
    }
    tail_token += 2;
  }
...
template <typename row_t>
void indexer_q_rope_fused_sve_impl(row_t* q_data, const int64_t* pos_data, int64_t scalar_pos, bool scalar_position,
                                   const float* cos_sin_data, int64_t cos_sin_stride, int64_t num_tokens,
                                   int64_t num_heads, int64_t nope_head_dim, int64_t rope_half, int64_t q_stride_t,
                                   int64_t q_stride_h) {
...
#pragma omp parallel for collapse(2) schedule(static)
  ...
  for (int64_t token = 0; token < num_tokens; ++token) {
    for (int64_t head_group = 0; head_group < head_groups4; ++head_group) {
      ...
      const float* cs_row = cos_sin_data + pos * cos_sin_stride;
      if constexpr (std::is_same_v<row_t, float>) {
        process_f32_kv_rows4_rope_sve(row0, row1, row2, row3, cs_row, cs_row + rope_half, cs_row, cs_row + rope_half,
                                      cs_row, cs_row + rope_half, cs_row, cs_row + rope_half, nope_head_dim, rope_half);
      } else {
        process_bf16_kv_rows4_rope_sve(row0, row1, row2, row3, cs_row, cs_row + rope_half, cs_row, cs_row + rope_half,
                                       cs_row, cs_row + rope_half, cs_row, cs_row + rope_half, nope_head_dim,
                                       rope_half);
      }
    }
  }
...
#pragma omp parallel for schedule(static)
    ...
    for (int64_t token = 0; token < num_tokens; ++token) {
      ...
      const float* cs_row = cos_sin_data + pos * cos_sin_stride;
      if constexpr (std::is_same_v<row_t, float>) {
        process_f32_kv_rows2_rope_sve(row0, row1, cs_row, cs_row + rope_half, cs_row, cs_row + rope_half, nope_head_dim,
                                      rope_half);
      } else {
        process_bf16_kv_rows2_rope_sve(row0, row1, cs_row, cs_row + rope_half, cs_row, cs_row + rope_half,
...
FUSED_CPP_ALWAYS_INLINE cache_t* kv_cache_row_from_slot(cache_t* cache_data, int64_t slot, bool cache_is_2d,
                                                        int64_t cache_stride_slot, int64_t cache_block_size,
                                                        int64_t cache_stride_block, int64_t cache_stride_offset) {
  if (cache_is_2d) {
    return cache_data + slot * cache_stride_slot;
  }
  return cache_data + (slot / cache_block_size) * cache_stride_block + (slot % cache_block_size) * cache_stride_offset;
}

template <typename row_t, typename cache_t>
FUSED_CPP_ALWAYS_INLINE void process_kv_token_to_cache_sve(
    const row_t* kv_data, cache_t* cache_data, const int64_t* slot_data, const int64_t* pos_data, int64_t scalar_pos,
    bool scalar_position, const float* cos_sin_data, int64_t cos_sin_stride, int64_t token, int64_t nope_head_dim,
    int64_t rope_half, int64_t kv_stride_t, bool cache_is_2d, int64_t cache_stride_slot, int64_t cache_block_size,
    int64_t cache_stride_block, int64_t cache_stride_offset) {
  ...
  cache_t* cache_row = kv_cache_row_from_slot(cache_data, slot, cache_is_2d, cache_stride_slot, cache_block_size,
                                              cache_stride_block, cache_stride_offset);
  ...
  const int64_t pos = scalar_position ? scalar_pos : pos_data[token];
  const float* cs_row = cos_sin_data + pos * cos_sin_stride;
  process_kv_rope_row_to_cache_sve(row, cache_row, cs_row, cs_row + rope_half, nope_head_dim, rope_half);
}

template <typename row_t, typename cache_t>
void kv_rope_cache_insert_fused_sve_impl(const row_t* kv_data, cache_t* cache_data, const int64_t* slot_data,
                                         const int64_t* pos_data, int64_t scalar_pos, bool scalar_position,
                                         const float* cos_sin_data, int64_t cos_sin_stride, int64_t num_tokens,
                                         int64_t nope_head_dim, int64_t rope_half, int64_t kv_stride_t,
                                         bool cache_is_2d, int64_t cache_stride_slot, int64_t cache_block_size,
                                         int64_t cache_stride_block, int64_t cache_stride_offset) {
  const int64_t token_groups4 = num_tokens / 4;
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
  ...
  for (int64_t token_group = 0; token_group < token_groups4; ++token_group) {
    ...
    if (slot0 < 0 || slot1 < 0 || slot2 < 0 || slot3 < 0) {
      process_kv_token_to_cache_sve(kv_data, cache_data, slot_data, pos_data, scalar_pos, scalar_position, cos_sin_data,
                                    cos_sin_stride, token0, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
                                    cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
      process_kv_token_to_cache_sve(kv_data, cache_data, slot_data, pos_data, scalar_pos, scalar_position, cos_sin_data,
                                    cos_sin_stride, token1, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
                                    cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
      process_kv_token_to_cache_sve(kv_data, cache_data, slot_data, pos_data, scalar_pos, scalar_position, cos_sin_data,
                                    cos_sin_stride, token2, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
                                    cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
      process_kv_token_to_cache_sve(kv_data, cache_data, slot_data, pos_data, scalar_pos, scalar_position, cos_sin_data,
                                    cos_sin_stride, token3, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
                                    cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
      continue;
    }
    ...
    cache_t* cache_row0 = kv_cache_row_from_slot(cache_data, slot0, cache_is_2d, cache_stride_slot, cache_block_size,
                                                 cache_stride_block, cache_stride_offset);
    cache_t* cache_row1 = kv_cache_row_from_slot(cache_data, slot1, cache_is_2d, cache_stride_slot, cache_block_size,
                                                 cache_stride_block, cache_stride_offset);
    cache_t* cache_row2 = kv_cache_row_from_slot(cache_data, slot2, cache_is_2d, cache_stride_slot, cache_block_size,
                                                 cache_stride_block, cache_stride_offset);
    cache_t* cache_row3 = kv_cache_row_from_slot(cache_data, slot3, cache_is_2d, cache_stride_slot, cache_block_size,
                                                 cache_stride_block, cache_stride_offset);

    copy_kv_nope_rows4_to_cache_sve(row0, row1, row2, row3, cache_row0, cache_row1, cache_row2, cache_row3,
                                    nope_head_dim);
    if (rope_half == 0) {
      continue;
    }
    ...
    const float* cs_row2 = cos_sin_data + pos2 * cos_sin_stride;
    const float* cs_row3 = cos_sin_data + pos3 * cos_sin_stride;
    process_kv_rows4_rope_to_cache_sve(row0, row1, row2, row3, cache_row0, cache_row1, cache_row2, cache_row3, cs_row0,
                                       cs_row0 + rope_half, cs_row1, cs_row1 + rope_half, cs_row2, cs_row2 + rope_half,
                                       cs_row3, cs_row3 + rope_half, nope_head_dim, rope_half);
  }
  ...
  if (tail_token + 1 < num_tokens) {
    ...
    if (slot0 >= 0 && slot1 >= 0) {
      ...
      if (rope_half != 0) {
        ...
        const float* cs_row0 = cos_sin_data + pos0 * cos_sin_stride;
        const float* cs_row1 = cos_sin_data + pos1 * cos_sin_stride;
        process_kv_rows2_rope_to_cache_sve(row0, row1, cache_row0, cache_row1, cs_row0, cs_row0 + rope_half, cs_row1,
                                           cs_row1 + rope_half, nope_head_dim, rope_half);
      }
    } else {
      process_kv_token_to_cache_sve(kv_data, cache_data, slot_data, pos_data, scalar_pos, scalar_position, cos_sin_data,
                                    cos_sin_stride, tail_token, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
                                    cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
      process_kv_token_to_cache_sve(kv_data, cache_data, slot_data, pos_data, scalar_pos, scalar_position, cos_sin_data,
                                    cos_sin_stride, tail_token + 1, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
                                    cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
    }
    tail_token += 2;
  }
...
bool q_norm_rope_fused_sve(const at::Tensor& q, const at::Tensor& positions_long, const at::Tensor& cos_sin_f,
                           double eps) {
#if defined(__ARM_FEATURE_SVE)
#if FUSED_CPP_STRICT_MODE
  TORCH_CHECK(q.scalar_type() == at::kBFloat16,
              "FUSED_CPP_STRICT_MODE q_norm_rope_fused_sve only supports bf16 q, got ", q.scalar_type());
  TORCH_CHECK(q.stride(2) == 1, "FUSED_CPP_STRICT_MODE q stride(2) must be 1, got ", q.stride(2));
  TORCH_CHECK(cos_sin_f.stride(1) == 1, "FUSED_CPP_STRICT_MODE cos_sin_f stride(1) must be 1, got ",
              cos_sin_f.stride(1));
#else
  ...
  if (q.scalar_type() == at::kFloat) {
    q_norm_rope_fused_sve_impl<float>(q.data_ptr<float>(), pos_data, scalar_pos, scalar_position, cos_sin_data,
                                      cos_sin_stride, num_tokens, num_heads, head_dim, nope_head_dim, rope_half,
                                      q_stride_t, q_stride_h, eps);
    return true;
  }
#endif
  if (q.scalar_type() == at::kBFloat16) {
    q_norm_rope_fused_sve_impl<uint16_t>(reinterpret_cast<uint16_t*>(q.data_ptr<at::BFloat16>()), pos_data, scalar_pos,
                                         scalar_position, cos_sin_data, cos_sin_stride, num_tokens, num_heads, head_dim,
                                         nope_head_dim, rope_half, q_stride_t, q_stride_h, eps);
    return true;
  }
#if FUSED_CPP_STRICT_MODE
  TORCH_CHECK(false, "FUSED_CPP_STRICT_MODE q_norm_rope_fused_sve only supports bf16 q, got ", q.scalar_type());
#endif
  return false;
#else
  (void)q;
...
bool indexer_q_rope_fused_sve(const at::Tensor& q, const at::Tensor& positions_long, const at::Tensor& cos_sin_f) {
#if defined(__ARM_FEATURE_SVE)
#if FUSED_CPP_STRICT_MODE
  TORCH_CHECK(q.scalar_type() == at::kBFloat16,
              "FUSED_CPP_STRICT_MODE indexer_q_rope_fused_sve only supports bf16 q, got ", q.scalar_type());
  TORCH_CHECK(q.stride(2) == 1, "FUSED_CPP_STRICT_MODE indexer q stride(2) must be 1, got ", q.stride(2));
  TORCH_CHECK(cos_sin_f.stride(1) == 1, "FUSED_CPP_STRICT_MODE indexer cos_sin_f stride(1) must be 1, got ",
              cos_sin_f.stride(1));
#else
  ...
  indexer_q_rope_fused_sve_impl<uint16_t>(reinterpret_cast<uint16_t*>(q.data_ptr<at::BFloat16>()), pos_data, scalar_pos,
                                          scalar_position, cos_sin_data, cos_sin_stride, num_tokens, num_heads,
                                          nope_head_dim, rope_half, q_stride_t, q_stride_h);
  return true;
#else
  (void)q;
  (void)positions_long;
...
bool kv_rope_fused_sve(const at::Tensor& kv, const at::Tensor& positions_long, const at::Tensor& cos_sin_f) {
#if defined(__ARM_FEATURE_SVE)
#if FUSED_CPP_STRICT_MODE
  TORCH_CHECK(kv.scalar_type() == at::kBFloat16, "FUSED_CPP_STRICT_MODE kv_rope_fused_sve only supports bf16 kv, got ",
              kv.scalar_type());
  TORCH_CHECK(kv.stride(1) == 1, "FUSED_CPP_STRICT_MODE kv stride(1) must be 1, got ", kv.stride(1));
  TORCH_CHECK(cos_sin_f.stride(1) == 1, "FUSED_CPP_STRICT_MODE cos_sin_f stride(1) must be 1, got ",
              cos_sin_f.stride(1));
#else
  ...
  const int64_t rope_half = rope_head_dim / 2;
#if !FUSED_CPP_STRICT_MODE
  const bool supported_dtype = kv.scalar_type() == at::kFloat || kv.scalar_type() == at::kBFloat16;
  if (!supported_dtype) {
    return false;
  }
#endif
  ...
  if (kv.scalar_type() == at::kFloat) {
    kv_rope_fused_sve_impl<float>(kv.data_ptr<float>(), pos_data, scalar_pos, scalar_position, cos_sin_data,
                                  cos_sin_stride, num_tokens, nope_head_dim, rope_half, kv_stride_t);
    return true;
  }
#endif
  if (kv.scalar_type() == at::kBFloat16) {
    kv_rope_fused_sve_impl<uint16_t>(reinterpret_cast<uint16_t*>(kv.data_ptr<at::BFloat16>()), pos_data, scalar_pos,
                                     scalar_position, cos_sin_data, cos_sin_stride, num_tokens, nope_head_dim,
                                     rope_half, kv_stride_t);
    return true;
  }
#if FUSED_CPP_STRICT_MODE
  TORCH_CHECK(false, "FUSED_CPP_STRICT_MODE kv_rope_fused_sve only supports bf16 kv, got ", kv.scalar_type());
#endif
  return false;
#else
  (void)kv;
...
bool kv_rope_cache_insert_fused_sve(const at::Tensor& kv, const at::Tensor& swa_kv_cache,
  ...
  TORCH_CHECK(kv.scalar_type() == at::kBFloat16,
              "FUSED_CPP_STRICT_MODE kv_rope_cache_insert_fused_sve only supports bf16 kv, got ", kv.scalar_type());
...
#endif
#if !FUSED_CPP_STRICT_MODE
  const bool supported_kv_dtype = kv.scalar_type() == at::kFloat || kv.scalar_type() == at::kBFloat16;
...
#if FUSED_CPP_STRICT_MODE
    TORCH_CHECK(false, "FUSED_CPP_STRICT_MODE swa_kv_cache must be 2-D or 3-D, got ", swa_kv_cache.dim(), "-D");
#endif
    return false;
  }
  if (cache_stride_d != 1) {
#if FUSED_CPP_STRICT_MODE
    TORCH_CHECK(false, "FUSED_CPP_STRICT_MODE swa_kv_cache last-dim stride must be 1, got ", cache_stride_d);
#endif
    return false;
  }
  ...
  if (kv.scalar_type() == at::kFloat && swa_kv_cache.scalar_type() == at::kFloat) {
    kv_rope_cache_insert_fused_sve_impl<float, float>(
        kv.data_ptr<float>(), swa_kv_cache.data_ptr<float>(), slot_data, pos_data, scalar_pos, scalar_position,
        cos_sin_data, cos_sin_stride, num_tokens, nope_head_dim, rope_half, kv_stride_t, cache_is_2d, cache_stride_slot,
        cache_block_size, cache_stride_block, cache_stride_offset);
    return true;
  }
  if (kv.scalar_type() == at::kFloat && swa_kv_cache.scalar_type() == at::kBFloat16) {
    kv_rope_cache_insert_fused_sve_impl<float, uint16_t>(
        kv.data_ptr<float>(), reinterpret_cast<uint16_t*>(swa_kv_cache.data_ptr<at::BFloat16>()), slot_data, pos_data,
        scalar_pos, scalar_position, cos_sin_data, cos_sin_stride, num_tokens, nope_head_dim, rope_half, kv_stride_t,
        cache_is_2d, cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
    return true;
  }
  if (kv.scalar_type() == at::kBFloat16 && swa_kv_cache.scalar_type() == at::kFloat) {
    kv_rope_cache_insert_fused_sve_impl<uint16_t, float>(
        reinterpret_cast<const uint16_t*>(kv.data_ptr<at::BFloat16>()), swa_kv_cache.data_ptr<float>(), slot_data,
        pos_data, scalar_pos, scalar_position, cos_sin_data, cos_sin_stride, num_tokens, nope_head_dim, rope_half,
        kv_stride_t, cache_is_2d, cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
    return true;
  }
#endif
  if (kv.scalar_type() == at::kBFloat16 && swa_kv_cache.scalar_type() == at::kBFloat16) {
    kv_rope_cache_insert_fused_sve_impl<uint16_t, uint16_t>(
        ...
        reinterpret_cast<uint16_t*>(swa_kv_cache.data_ptr<at::BFloat16>()), slot_data, pos_data, scalar_pos,
        scalar_position, cos_sin_data, cos_sin_stride, num_tokens, nope_head_dim, rope_half, kv_stride_t, cache_is_2d,
        cache_stride_slot, cache_block_size, cache_stride_block, cache_stride_offset);
    return true;
  }
#if FUSED_CPP_STRICT_MODE
  TORCH_CHECK(false,
              ...
              kv.scalar_type(), " cache=", swa_kv_cache.scalar_type());
#endif
  return false;
#else
  (void)kv;
