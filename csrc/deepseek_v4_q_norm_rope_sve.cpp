...
inline svfloat32_t bf16_gather_evenodd_f32(svbool_t pg, const uint16_t* ptr) {
  const svuint32_t idx = svindex_u32(0, 2);
  const svuint32_t h = svld1uh_gather_u32index_u32(pg, ptr, idx);
  return svreinterpret_f32_u32(svlsl_n_u32_x(pg, h, 16));
}
...
inline svfloat32_t round_f32_to_bf16_then_f32(svbool_t pg, svfloat32_t v) {
  const svuint32_t h = f32_to_bf16_bits(pg, v);
  return svreinterpret_f32_u32(svlsl_n_u32_x(pg, h, 16));
}
...
FUSED_CPP_ALWAYS_INLINE void load_evenodd_contiguous_f32(
  ...
  if constexpr (std::is_same_v<scalar_t, float>) {
    ...
    even = bf16_gather_evenodd_f32(pg, ptr);
    odd = bf16_gather_evenodd_f32(pg, ptr + 1);
  }
}
...
void process_bf16_rope_row_sve(uint16_t* row,
  ...
  for (int64_t pair = 0; pair < rope_half; pair += static_cast<int64_t>(svcntw())) {
    const svbool_t pg = svwhilelt_b32(pair, rope_half);
    const svfloat32_t even = round_f32_to_bf16_then_f32(
        pg, svmul_f32_x(pg, bf16_gather_evenodd_f32(pg, row + nope_head_dim + 2 * pair), inv));
    const svfloat32_t odd = round_f32_to_bf16_then_f32(
        pg, svmul_f32_x(pg, bf16_gather_evenodd_f32(pg, row + nope_head_dim + 2 * pair + 1), inv));
    const svfloat32_t c = svld1_f32(pg, cos_row + pair);
    const svfloat32_t s = svld1_f32(pg, sin_row + pair);
    const svfloat32_t out_even =
...
FUSED_CPP_ALWAYS_INLINE void process_f32_kv_rows4_rope_chunk_sve(
  ...
  const svfloat32x2_t kv_pair0 = svld2_f32(pg, row0 + nope_head_dim + 2 * pair);
  const svfloat32x2_t kv_pair1 = svld2_f32(pg, row1 + nope_head_dim + 2 * pair);
  const svfloat32x2_t kv_pair2 = svld2_f32(pg, row2 + nope_head_dim + 2 * pair);
  const svfloat32x2_t kv_pair3 = svld2_f32(pg, row3 + nope_head_dim + 2 * pair);

  const svfloat32_t even0 = svget2_f32(kv_pair0, 0);
  const svfloat32_t odd0 = svget2_f32(kv_pair0, 1);
  const svfloat32_t even1 = svget2_f32(kv_pair1, 0);
  const svfloat32_t odd1 = svget2_f32(kv_pair1, 1);
  const svfloat32_t even2 = svget2_f32(kv_pair2, 0);
  const svfloat32_t odd2 = svget2_f32(kv_pair2, 1);
  const svfloat32_t even3 = svget2_f32(kv_pair3, 0);
  const svfloat32_t odd3 = svget2_f32(kv_pair3, 1);
...
FUSED_CPP_ALWAYS_INLINE void process_bf16_kv_rows2_rope_chunk_sve(
    ...
    int64_t pair,
    svbool_t pg) {
  const svfloat32_t even0 = bf16_gather_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair);
  const svfloat32_t odd0 = bf16_gather_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair + 1);
  const svfloat32_t even1 = bf16_gather_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair);
  const svfloat32_t odd1 = bf16_gather_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair + 1);
  const svfloat32_t c0 = svld1_f32(pg, cos_row0 + pair);
  const svfloat32_t s0 = svld1_f32(pg, sin_row0 + pair);
  const svfloat32_t c1 = svld1_f32(pg, cos_row1 + pair);
  const svfloat32_t s1 = svld1_f32(pg, sin_row1 + pair);

  const svfloat32_t out_even0 =
      svmls_f32_x(pg, svmul_f32_x(pg, even0, c0), odd0, s0);
  const svfloat32_t out_odd0 =
      svmla_f32_x(pg, svmul_f32_x(pg, odd0, c0), even0, s0);
  const svfloat32_t out_even1 =
      svmls_f32_x(pg, svmul_f32_x(pg, even1, c1), odd1, s1);
  const svfloat32_t out_odd1 =
      svmla_f32_x(pg, svmul_f32_x(pg, odd1, c1), even1, s1);

  bf16_scatter_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair, out_even0);
  bf16_scatter_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair + 1, out_odd0);
  bf16_scatter_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair, out_even1);
  bf16_scatter_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair + 1, out_odd1);
}
...
FUSED_CPP_ALWAYS_INLINE void process_bf16_kv_rows4_rope_chunk_sve(
  ...
  const svfloat32_t even1 = bf16_gather_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair);
  const svfloat32_t odd1 = bf16_gather_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair + 1);
  const svfloat32_t even2 = bf16_gather_evenodd_f32(pg, row2 + nope_head_dim + 2 * pair);
  const svfloat32_t odd2 = bf16_gather_evenodd_f32(pg, row2 + nope_head_dim + 2 * pair + 1);
  const svfloat32_t even3 = bf16_gather_evenodd_f32(pg, row3 + nope_head_dim + 2 * pair);
  const svfloat32_t odd3 = bf16_gather_evenodd_f32(pg, row3 + nope_head_dim + 2 * pair + 1);
  ...
  const svfloat32_t out_odd1 =
      svmla_f32_x(pg, svmul_f32_x(pg, odd1, c1), even1, s1);
  const svfloat32_t out_even2 =
      svmls_f32_x(pg, svmul_f32_x(pg, even2, c2), odd2, s2);
  const svfloat32_t out_odd2 =
      svmla_f32_x(pg, svmul_f32_x(pg, odd2, c2), even2, s2);
  const svfloat32_t out_even3 =
      svmls_f32_x(pg, svmul_f32_x(pg, even3, c3), odd3, s3);
  const svfloat32_t out_odd3 =
      svmla_f32_x(pg, svmul_f32_x(pg, odd3, c3), even3, s3);

  bf16_scatter_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair, out_even0);
  bf16_scatter_evenodd_f32(pg, row0 + nope_head_dim + 2 * pair + 1, out_odd0);
  bf16_scatter_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair, out_even1);
  bf16_scatter_evenodd_f32(pg, row1 + nope_head_dim + 2 * pair + 1, out_odd1);
  bf16_scatter_evenodd_f32(pg, row2 + nope_head_dim + 2 * pair, out_even2);
  bf16_scatter_evenodd_f32(pg, row2 + nope_head_dim + 2 * pair + 1, out_odd2);
  bf16_scatter_evenodd_f32(pg, row3 + nope_head_dim + 2 * pair, out_even3);
  bf16_scatter_evenodd_f32(pg, row3 + nope_head_dim + 2 * pair + 1, out_odd3);
}
...
void process_kv_rope_row_to_cache_sve(const row_t* row,
  ...
  for (int64_t pair = 0; pair < rope_half; pair += vl) {
    ...
    svfloat32_t even;
    svfloat32_t odd;
    load_evenodd_contiguous_f32(pg, row + offset, even, odd);
    const svfloat32_t c = svld1_f32(pg, cos_row + pair);
    const svfloat32_t s = svld1_f32(pg, sin_row + pair);
    const svfloat32_t out_even =
        svmls_f32_x(pg, svmul_f32_x(pg, even, c), odd, s);
...
#pragma omp parallel for schedule(static)
  ...
  for (int64_t token = 0; token < num_tokens; ++token) {
    ...
    process_kv_rope_row_to_cache_sve(
        row, cache_row, cos_row, sin_row, nope_head_dim, rope_half);
  }
}
