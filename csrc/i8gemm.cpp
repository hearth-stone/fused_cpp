...
int64_t round_up_int64(int64_t value, int64_t quantum) {
...
void check_cpu_tensor(const at::Tensor& tensor, const char* name) {
    TORCH_CHECK(tensor.device().is_cpu(),
                "i8gemm: ", name, " must be a CPU tensor");
}

void check_backend_request(const std::string& backend) {
    TORCH_CHECK(backend == "auto" || backend == "neon" || backend == "sve",
                "i8gemm.prepare: backend must be one of auto/neon/sve, got ",
                backend);
#if defined(FUSED_CPP_HAS_I8GEMM)
    if (backend != "auto") {
        TORCH_CHECK(backend == kCompiledBackend,
                    "i8gemm.prepare: requested backend=", backend,
                    " but this extension was built with backend=",
                    kCompiledBackend);
    }
#else
    TORCH_CHECK(backend == "auto",
                "i8gemm.prepare: explicit backend=", backend,
...
at::Tensor prepare_weight_scale(const at::Tensor& weight_scale, int64_t N) {
    ...
    TORCH_CHECK(weight_scale.numel() == 1 || weight_scale.numel() == N,
                "i8gemm.prepare: weight_scale must be scalar or shape [N], got numel=",
                weight_scale.numel(), " for N=", N);
...
#if defined(FUSED_CPP_HAS_I8GEMM)
at::Tensor pack_weight_i8gemm(const at::Tensor& weight, int64_t Kp,
                              int64_t Np) {
    const int64_t N = weight.size(0);
    const int64_t K = weight.size(1);
    at::Tensor weight_kn =
        at::zeros({Kp, Np}, at::TensorOptions().dtype(at::kChar));
    ...
    at::Tensor packed =
        at::empty({Kp * Np}, at::TensorOptions().dtype(at::kChar));
    i8_pack_B(b_pad, packed.data_ptr<int8_t>(),
              static_cast<int>(Kp), static_cast<int>(Np));
    return packed;
}
#else
at::Tensor pack_weight_fallback(const at::Tensor& weight, int64_t Kp,
                                int64_t Np) {
    const int64_t N = weight.size(0);
    const int64_t K = weight.size(1);
...
#if !defined(FUSED_CPP_HAS_I8GEMM)
void fallback_int8_accum_f32(const int8_t* a, const int8_t* b_kn, float* c,
            ...
            for (int64_t n = 0; n < N; ++n) {
                ...
                for (int64_t k = 0; k < K; ++k) {
                    sum += static_cast<int32_t>(a_row[k]) *
                           static_cast<int32_t>(b_kn[k * Np + n]);
                }
...
at::Tensor prepare_bias(c10::optional<at::Tensor> bias, int64_t N) {
    ...
    TORCH_CHECK(b.dim() == 1,
                "i8gemm.dynamic_scaled_mm: bias must be 1D [N], got dim=",
                b.dim());
    TORCH_CHECK(b.size(0) == N,
                "i8gemm.dynamic_scaled_mm: bias length must equal N=", N,
                ", got ", b.size(0));
...
i8gemm_prepare(at::Tensor weight, at::Tensor weight_scale,
    ...
    TORCH_CHECK(weight.dim() == 2,
                "i8gemm.prepare: weight must be 2D [N, K], got dim=",
                weight.dim());
    check_cpu_tensor(weight, "weight");
    TORCH_CHECK(weight.scalar_type() == at::kChar,
                "i8gemm.prepare: weight dtype must be torch.int8");
    TORCH_CHECK(weight.is_contiguous(),
                "i8gemm.prepare: weight must be contiguous in [N, K] layout");

    const int64_t N = weight.size(0);
    const int64_t K = weight.size(1);
    TORCH_CHECK(N > 0 && K > 0,
                "i8gemm.prepare: N and K must be positive, got N=", N,
                " K=", K);
    ...
    TORCH_CHECK(Kp <= std::numeric_limits<int>::max() &&
                    Np <= std::numeric_limits<int>::max(),
...
void i8gemm_dynamic_scaled_mm(at::Tensor output,
    ...
    TORCH_CHECK(input.dim() == 2,
                "i8gemm.dynamic_scaled_mm: input must be 2D [M, K], got dim=",
                input.dim());
    ...
    TORCH_CHECK(input.is_contiguous(),
                "i8gemm.dynamic_scaled_mm: input must be contiguous");
    TORCH_CHECK(output.dim() == 2,
                "i8gemm.dynamic_scaled_mm: output must be 2D [M, N], got dim=",
                output.dim());
    ...
    TORCH_CHECK(output.is_contiguous(),
                "i8gemm.dynamic_scaled_mm: output must be contiguous");
    TORCH_CHECK(packed_weight.scalar_type() == at::kChar &&
                    packed_weight.is_contiguous() &&
                    packed_weight.numel() == Kp * Np,
                "i8gemm.dynamic_scaled_mm: invalid packed_weight");
    TORCH_CHECK(weight_scale.scalar_type() == at::kFloat &&
                    weight_scale.is_contiguous() &&
                    weight_scale.numel() == N,
                "i8gemm.dynamic_scaled_mm: weight_scale must be contiguous float32 [N]");

    const int64_t M = input.size(0);
    TORCH_CHECK(input.size(1) == K,
                "i8gemm.dynamic_scaled_mm: input K mismatch, expected ", K,
                ", got ", input.size(1));
    TORCH_CHECK(output.size(0) == M && output.size(1) == N,
                "i8gemm.dynamic_scaled_mm: output shape must be [", M, ", ",
                N, "], got [", output.size(0), ", ", output.size(1), "]");
    TORCH_CHECK(K > 0 && N > 0 && Kp >= K && Np >= N,
                "i8gemm.dynamic_scaled_mm: invalid K/N metadata");
    TORCH_CHECK(Kp % 16 == 0 && Np % 8 == 0,
                "i8gemm.dynamic_scaled_mm: Kp must be multiple of 16 and Np multiple of 8");
    TORCH_CHECK(Kp <= std::numeric_limits<int>::max() &&
                    Np <= std::numeric_limits<int>::max() &&
    ...
    if (input.scalar_type() == at::kFloat) {
        ...
        at::parallel_for(0, M, 1, [&](int64_t begin, int64_t end) {
            for (int64_t m = begin; m < end; ++m) {
                ...
                for (int64_t k = 0; k < K; ++k) {
                    ...
                    a_row[k] = static_cast<int8_t>(
                        std::max<long>(-128, std::min<long>(127, q)));
                }
            }
        });
    } else {
        ...
        at::parallel_for(0, M, 1, [&](int64_t begin, int64_t end) {
            for (int64_t m = begin; m < end; ++m) {
                ...
                int8_t* a_row = a_ptr + m * Kp;
                for (int64_t k = 0; k < K; ++k) {
                    const long q = static_cast<long>(
                        std::nearbyint(static_cast<float>(x_row[k]) / scale));
                    a_row[k] = static_cast<int8_t>(
                        std::max<long>(-128, std::min<long>(127, q)));
                }
            }
...
#if defined(FUSED_CPP_HAS_I8GEMM)
    i8gemm_mt_dispatch_f(a_ptr, packed_ptr, acc_ptr,
                         static_cast<int>(M), static_cast<int>(Kp),
    ...
    if (output.scalar_type() == at::kFloat) {
        write_scaled_output(output.data_ptr<float>(), acc_ptr, x_scale_ptr,
                            w_scale, bias_ptr, M, N, Np);
    } else {
        write_scaled_output(output.data_ptr<c10::BFloat16>(), acc_ptr,
                            x_scale_ptr, w_scale, bias_ptr, M, N, Np);
    }
}
